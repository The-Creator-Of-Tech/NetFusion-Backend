...
async function main() {
    ...
    // Providers
    const pGroq = await prisma_1.default.provider.findUnique({ where: { id: '419f2e3a-6f0a-4b9a-bbcb-7c73a1d9fa01' } });
    assert(!!pGroq, 'Seeded Groq provider exists');
    assert(pGroq?.providerName === 'groq', 'Seeded Groq name matches');
    const pOllama = await prisma_1.default.provider.findUnique({ where: { id: '419f2e3a-6f0a-4b9a-bbcb-7c73a1d9fa02' } });
    assert(!!pOllama, 'Seeded Ollama provider exists');
    assert(pOllama?.providerName === 'ollama', 'Seeded Ollama name matches');
    // Models
    const mGroq = await prisma_1.default.providerModel.findUnique({ where: { id: '519f2e3a-6f0a-4b9a-bbcb-7c73a1d9fb01' } });
    assert(!!mGroq, 'Seeded Groq model exists');
    assert(mGroq?.providerId === pGroq?.id, 'Seeded Groq model references Groq provider');
    const mOllama = await prisma_1.default.providerModel.findUnique({ where: { id: '519f2e3a-6f0a-4b9a-bbcb-7c73a1d9fb02' } });
    ...
    // Conversation
    const conv = await prisma_1.default.conversation.findUnique({ where: { id: 'a19f2e3a-6f0a-4b9a-bbcb-7c73a1d9f001' } });
    ...
    const msg1 = await prisma_1.default.conversationMessage.findUnique({ where: { id: 'b19f2e3a-6f0a-4b9a-bbcb-7c73a1d9f101' } });
    ...
    const msg2 = await prisma_1.default.conversationMessage.findUnique({ where: { id: 'b19f2e3a-6f0a-4b9a-bbcb-7c73a1d9f102' } });
    ...
    // Session Memory
    const mem = await prisma_1.default.sessionMemory.findUnique({ where: { id: 'c19f2e3a-6f0a-4b9a-bbcb-7c73a1d9f201' } });
    ...
    // Memory Entries
    const mEntries = await prisma_1.default.memoryEntry.findMany({ where: { memoryId: mem?.id } });
    ...
    // Context Window
    const ctx = await prisma_1.default.contextWindow.findUnique({ where: { id: 'e19f2e3a-6f0a-4b9a-bbcb-7c73a1d9f401' } });
    ...
    // Context Entries
    const cEntries = await prisma_1.default.contextEntry.findMany({ where: { contextId: ctx?.id } });
    ...
    // Reasoning Session
    const reason = await prisma_1.default.reasoning.findUnique({ where: { id: '019f2e3a-6f0a-4b9a-bbcb-7c73a1d9f601' } });
    ...
    // Reasoning Steps
    const rSteps = await prisma_1.default.reasoningStep.findMany({ where: { reasoningId: reason?.id } });
    ...
    // Prompt Assembly
    const prompt = await prisma_1.default.promptAssembly.findUnique({ where: { id: '219f2e3a-6f0a-4b9a-bbcb-7c73a1d9f801' } });
    ...
    // Prompt Sections
    const pSections = await prisma_1.default.promptSection.findMany({ where: { promptId: prompt?.id } });
    ...
    // Execution
    const exec = await prisma_1.default.execution.findUnique({ where: { id: '619f2e3a-6f0a-4b9a-bbcb-7c73a1d9fc01' } });
    ...
    // Execution Usage
    const usage = await prisma_1.default.executionUsage.findUnique({ where: { id: '719f2e3a-6f0a-4b9a-bbcb-7c73a1d9fd01' } });
    assert(!!usage, 'Seeded execution usage exists');
    assert(usage?.executionId === exec?.id, 'Seeded usage references execution');
    // Streaming Session
    const stream = await prisma_1.default.streaming.findUnique({ where: { id: '819f2e3a-6f0a-4b9a-bbcb-7c73a1d9fe01' } });
    assert(!!stream, 'Seeded streaming session exists');
    assert(stream?.executionId === exec?.id, 'Seeded streaming references execution');
    // Streaming Chunks
    const sChunks = await prisma_1.default.streamingChunk.findMany({ where: { streamingId: stream?.id } });
    ...
    // ConversationStatus
    await testEnum('ConversationStatus', Object.values(client_1.ConversationStatus), (val) => prisma_1.default.conversation.create({
        data: { projectId: seedProjectId, title: `temp-${val}-${RUN}`, status: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.conversation.findUnique({ where: { id } }), (id, val) => prisma_1.default.conversation.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.conversation.delete({ where: { id } }));
    // ExecutionStatus
    await testEnum('ExecutionStatus', Object.values(client_1.ExecutionStatus), (val) => prisma_1.default.execution.create({
        data: { providerId: pGroq.id, systemPrompt: 's', userPrompt: 'u', status: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.execution.findUnique({ where: { id } }), (id, val) => prisma_1.default.execution.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.execution.delete({ where: { id } }));
    // ProviderStatus
    await testEnum('ProviderStatus', Object.values(client_1.ProviderStatus), (val) => prisma_1.default.provider.create({
    ...
    }), (id) => prisma_1.default.provider.findUnique({ where: { id } }), (id, val) => prisma_1.default.provider.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.provider.delete({ where: { id } }));
    // StreamingStatus
    await testEnum('StreamingStatus', Object.values(client_1.StreamingStatus), (val) => prisma_1.default.streaming.create({
        data: { status: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.streaming.findUnique({ where: { id } }), (id, val) => prisma_1.default.streaming.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.streaming.delete({ where: { id } }));
    // ReasoningStatus
    await testEnum('ReasoningStatus', Object.values(client_1.ReasoningStatus), (val) => prisma_1.default.reasoning.create({
        data: { projectId: seedProjectId, investigationId: seedInvestigationId, status: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.reasoning.findUnique({ where: { id } }), (id, val) => prisma_1.default.reasoning.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.reasoning.delete({ where: { id } }));
    // PromptStatus
    await testEnum('PromptStatus', Object.values(client_1.PromptStatus), (val) => prisma_1.default.promptAssembly.create({
    ...
    }), (id) => prisma_1.default.promptAssembly.findUnique({ where: { id } }), (id, val) => prisma_1.default.promptAssembly.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.promptAssembly.delete({ where: { id } }));
    // MemoryStatus
    await testEnum('MemoryStatus', Object.values(client_1.MemoryStatus), (val) => prisma_1.default.sessionMemory.create({
        data: { projectId: seedProjectId, conversationId: conv.id, status: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.sessionMemory.findUnique({ where: { id } }), (id, val) => prisma_1.default.sessionMemory.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.sessionMemory.delete({ where: { id } }));
    // ContextStatus
    await testEnum('ContextStatus', Object.values(client_1.ContextStatus), (val) => prisma_1.default.contextWindow.create({
        data: { projectId: seedProjectId, status: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.contextWindow.findUnique({ where: { id } }), (id, val) => prisma_1.default.contextWindow.update({ where: { id }, data: { status: val } }), (id) => prisma_1.default.contextWindow.delete({ where: { id } }));
    // ProviderType
    await testEnum('ProviderType', Object.values(client_1.ProviderType), (val) => prisma_1.default.provider.create({
    ...
    }), (id) => prisma_1.default.provider.findUnique({ where: { id } }), (id, val) => prisma_1.default.provider.update({ where: { id }, data: { providerType: val } }), (id) => prisma_1.default.provider.delete({ where: { id } }));
    // ───────────────────────────────────────────────────────────────────────────
    ...
    // 1. Conversation
    await testCRUD('Conversation', () => prisma_1.default.conversation.create({
        data: { projectId: seedProjectId, title: `crud-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.conversation.findUnique({ where: { id } }), (id) => prisma_1.default.conversation.update({
        where: { id },
        data: { title: `crud-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.conversation.delete({ where: { id } }));
    // 2. ConversationMessage
    await testCRUD('ConversationMessage', () => prisma_1.default.conversationMessage.create({
        data: { conversationId: conv.id, role: 'user', content: 'test', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.conversationMessage.findUnique({ where: { id } }), (id) => prisma_1.default.conversationMessage.update({
        where: { id },
        data: { content: 'test-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.conversationMessage.delete({ where: { id } }));
    // 3. SessionMemory
    await testCRUD('SessionMemory', () => prisma_1.default.sessionMemory.create({
        data: { projectId: seedProjectId, conversationId: conv.id, sessionName: `crud-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.sessionMemory.findUnique({ where: { id } }), (id) => prisma_1.default.sessionMemory.update({
        where: { id },
        data: { sessionName: `crud-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.sessionMemory.delete({ where: { id } }));
    // 4. MemoryEntry
    await testCRUD('MemoryEntry', () => prisma_1.default.memoryEntry.create({
    ...
    }), (id) => prisma_1.default.memoryEntry.findUnique({ where: { id } }), (id) => prisma_1.default.memoryEntry.update({
        where: { id },
        data: { content: 'c-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.memoryEntry.delete({ where: { id } }));
    // 5. ContextWindow
    await testCRUD('ContextWindow', () => prisma_1.default.contextWindow.create({
        data: { projectId: seedProjectId, windowName: `crud-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.contextWindow.findUnique({ where: { id } }), (id) => prisma_1.default.contextWindow.update({
        where: { id },
        data: { windowName: `crud-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.contextWindow.delete({ where: { id } }));
    // 6. ContextEntry
    await testCRUD('ContextEntry', () => prisma_1.default.contextEntry.create({
    ...
    }), (id) => prisma_1.default.contextEntry.findUnique({ where: { id } }), (id) => prisma_1.default.contextEntry.update({
        where: { id },
        data: { content: 'c-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.contextEntry.delete({ where: { id } }));
    // 7. PromptAssembly
    await testCRUD('PromptAssembly', () => prisma_1.default.promptAssembly.create({
    ...
    }), (id) => prisma_1.default.promptAssembly.findUnique({ where: { id } }), (id) => prisma_1.default.promptAssembly.update({
        where: { id },
        data: { promptName: `crud-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.promptAssembly.delete({ where: { id } }));
    // 8. PromptSection
    await testCRUD('PromptSection', () => prisma_1.default.promptSection.create({
        data: { promptId: prompt.id, title: 't', content: 'c', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.promptSection.findUnique({ where: { id } }), (id) => prisma_1.default.promptSection.update({
        where: { id },
        data: { content: 'c-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.promptSection.delete({ where: { id } }));
    // 9. Reasoning
    await testCRUD('Reasoning', () => prisma_1.default.reasoning.create({
    ...
    }), (id) => prisma_1.default.reasoning.findUnique({ where: { id } }), (id) => prisma_1.default.reasoning.update({
        where: { id },
        data: { sessionName: `crud-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.reasoning.delete({ where: { id } }));
    // 10. ReasoningStep
    await testCRUD('ReasoningStep', () => prisma_1.default.reasoningStep.create({
    ...
    }), (id) => prisma_1.default.reasoningStep.findUnique({ where: { id } }), (id) => prisma_1.default.reasoningStep.update({
        where: { id },
        data: { outputSummary: 'o-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.reasoningStep.delete({ where: { id } }));
    // 11. Provider
    await testCRUD('Provider', () => prisma_1.default.provider.create({
    ...
    }), (id) => prisma_1.default.provider.findUnique({ where: { id } }), (id) => prisma_1.default.provider.update({
        where: { id },
        data: { displayName: `temp-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.provider.delete({ where: { id } }));
    // 12. ProviderModel
    await testCRUD('ProviderModel', () => prisma_1.default.providerModel.create({
        data: { providerId: pGroq.id, modelName: `crud-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.providerModel.findUnique({ where: { id } }), (id) => prisma_1.default.providerModel.update({
        where: { id },
        data: { alias: `alias-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.providerModel.delete({ where: { id } }));
    // 13. Execution
    await testCRUD('Execution', () => prisma_1.default.execution.create({
        data: { providerId: pGroq.id, systemPrompt: 's', userPrompt: 'u', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.execution.findUnique({ where: { id } }), (id) => prisma_1.default.execution.update({
        where: { id },
        data: { systemPrompt: 's-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.execution.delete({ where: { id } }));
    // 14. ExecutionUsage
    const tempExec = await prisma_1.default.execution.create({
        data: { providerId: pGroq.id, systemPrompt: 's', userPrompt: 'u', createdBy: 't', updatedBy: 't' }
    });
    await testCRUD('ExecutionUsage', () => prisma_1.default.executionUsage.create({
    ...
    }), (id) => prisma_1.default.executionUsage.findUnique({ where: { id } }), (id) => prisma_1.default.executionUsage.update({
        where: { id },
        data: { estimatedCost: 0.2, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.executionUsage.delete({ where: { id } }));
    await prisma_1.default.execution.delete({ where: { id: tempExec.id } });
    // 15. Streaming
    await testCRUD('Streaming', () => prisma_1.default.streaming.create({
        data: { streamName: `crud-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.streaming.findUnique({ where: { id } }), (id) => prisma_1.default.streaming.update({
        where: { id },
        data: { streamName: `crud-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.streaming.delete({ where: { id } }));
    // 16. StreamingChunk
    await testCRUD('StreamingChunk', () => prisma_1.default.streamingChunk.create({
        data: { streamingId: stream.id, sequenceNumber: 10, content: 'c', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.streamingChunk.findUnique({ where: { id } }), (id) => prisma_1.default.streamingChunk.update({
        where: { id },
        data: { content: 'c-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.streamingChunk.delete({ where: { id } }));
    // ───────────────────────────────────────────────────────────────────────────
    ...
    const softDeleteModels = [
        {
            name: 'Conversation',
            createFn: () => prisma_1.default.conversation.create({ data: { projectId: seedProjectId, title: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.conversation.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.conversation.delete({ where: { id } }),
        },
        {
            name: 'ConversationMessage',
            createFn: () => prisma_1.default.conversationMessage.create({ data: { conversationId: conv.id, role: 'user', content: 'soft', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.conversationMessage.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.conversationMessage.delete({ where: { id } }),
        },
        {
            name: 'SessionMemory',
            createFn: () => prisma_1.default.sessionMemory.create({ data: { projectId: seedProjectId, conversationId: conv.id, sessionName: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.sessionMemory.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.sessionMemory.delete({ where: { id } }),
        },
        {
            name: 'MemoryEntry',
            createFn: () => prisma_1.default.memoryEntry.create({ data: { memoryId: mem.id, memoryType: 't', state: 's', title: 't', content: 'c', importanceScore: 1, confidence: 1, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.memoryEntry.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.memoryEntry.delete({ where: { id } }),
        },
        {
            name: 'ContextWindow',
            createFn: () => prisma_1.default.contextWindow.create({ data: { projectId: seedProjectId, windowName: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.contextWindow.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.contextWindow.delete({ where: { id } }),
        },
        {
            name: 'ContextEntry',
            createFn: () => prisma_1.default.contextEntry.create({ data: { contextId: ctx.id, source: 's', priority: 'p', title: 't', content: 'c', referenceId: 'r', importanceScore: 1, confidence: 1, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.contextEntry.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.contextEntry.delete({ where: { id } }),
        },
        {
            name: 'PromptAssembly',
            createFn: () => prisma_1.default.promptAssembly.create({ data: { projectId: seedProjectId, investigationId: seedInvestigationId, systemPrompt: 's', userPrompt: 'u', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.promptAssembly.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.promptAssembly.delete({ where: { id } }),
        },
        {
            name: 'PromptSection',
            createFn: () => prisma_1.default.promptSection.create({ data: { promptId: prompt.id, title: 't', content: 'c', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.promptSection.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.promptSection.delete({ where: { id } }),
        },
        {
            name: 'Reasoning',
            createFn: () => prisma_1.default.reasoning.create({ data: { projectId: seedProjectId, investigationId: seedInvestigationId, sessionName: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.reasoning.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.reasoning.delete({ where: { id } }),
        },
        {
            name: 'ReasoningStep',
            createFn: () => prisma_1.default.reasoningStep.create({ data: { reasoningId: reason.id, stepNumber: 10, stage: 's', inputSummary: 'i', outputSummary: 'o', confidence: 1, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.reasoningStep.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.reasoningStep.delete({ where: { id } }),
        },
        {
            name: 'Provider',
            createFn: () => prisma_1.default.provider.create({ data: { providerName: `soft-${RUN}`, displayName: 't', apiVersion: 'v1', endpoint: 'h', defaultModel: 'm', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.provider.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.provider.delete({ where: { id } }),
        },
        {
            name: 'ProviderModel',
            createFn: () => prisma_1.default.providerModel.create({ data: { providerId: pGroq.id, modelName: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.providerModel.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.providerModel.delete({ where: { id } }),
        },
        {
            name: 'Execution',
            createFn: () => prisma_1.default.execution.create({ data: { providerId: pGroq.id, systemPrompt: 's', userPrompt: 'u', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.execution.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.execution.delete({ where: { id } }),
        },
        {
            name: 'ExecutionUsage',
            createFn: async () => {
                const e = await prisma_1.default.execution.create({ data: { providerId: pGroq.id, systemPrompt: 's', userPrompt: 'u', createdBy: 't', updatedBy: 't' } });
                return prisma_1.default.executionUsage.create({ data: { executionId: e.id, promptTokens: 1, completionTokens: 1, totalTokens: 2, estimatedCost: 0.1, latencyMs: 1, createdBy: 't', updatedBy: 't' } });
            },
            updateFn: (id, d) => prisma_1.default.executionUsage.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: async (id) => {
                const u = await prisma_1.default.executionUsage.findUnique({ where: { id } });
                if (u) {
                    await prisma_1.default.executionUsage.delete({ where: { id } });
                    await prisma_1.default.execution.delete({ where: { id: u.executionId } });
                }
            },
        },
        {
            name: 'Streaming',
            createFn: () => prisma_1.default.streaming.create({ data: { streamName: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.streaming.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.streaming.delete({ where: { id } }),
        },
        {
            name: 'StreamingChunk',
            createFn: () => prisma_1.default.streamingChunk.create({ data: { streamingId: stream.id, sequenceNumber: 10, content: 'c', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.streamingChunk.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.streamingChunk.delete({ where: { id } }),
        },
    ];
    ...
    const populatedConv = await prisma_1.default.conversation.findUnique({
        where: { id: conv.id },
    ...
    const casReasoning = await prisma_1.default.reasoning.create({
        data: { projectId: cascadeProject.id, investigationId: cascadeInvestigation.id, createdBy: 't', updatedBy: 't' }
    });
    const casReasoningStep = await prisma_1.default.reasoningStep.create({
    ...
    const casPrompt = await prisma_1.default.promptAssembly.create({
    ...
    const casPromptSection = await prisma_1.default.promptSection.create({
        data: { promptId: casPrompt.id, title: 't', content: 'c', createdBy: 't', updatedBy: 't' }
    });
    const casExec = await prisma_1.default.execution.create({
    ...
    const casUsage = await prisma_1.default.executionUsage.create({
    ...
    const casStream = await prisma_1.default.streaming.create({
        data: { executionId: casExec.id, projectId: cascadeProject.id, investigationId: cascadeInvestigation.id, createdBy: 't', updatedBy: 't' }
    });
    const casStreamChunk = await prisma_1.default.streamingChunk.create({
    ...
    await prisma_1.default.investigation.delete({ where: { id: cascadeInvestigation.id } });
    // Assert everything cascade-deleted
    assert(await prisma_1.default.conversation.findUnique({ where: { id: casConv.id } }) === null, '[Cascade] Conversation cascade deleted');
    assert(await prisma_1.default.conversationMessage.findUnique({ where: { id: casMsg.id } }) === null, '[Cascade] ConversationMessage cascade deleted');
    assert(await prisma_1.default.sessionMemory.findUnique({ where: { id: casMemory.id } }) === null, '[Cascade] SessionMemory cascade deleted');
    assert(await prisma_1.default.memoryEntry.findUnique({ where: { id: casMemEntry.id } }) === null, '[Cascade] MemoryEntry cascade deleted');
    assert(await prisma_1.default.contextWindow.findUnique({ where: { id: casContext.id } }) === null, '[Cascade] ContextWindow cascade deleted');
    assert(await prisma_1.default.contextEntry.findUnique({ where: { id: casContextEntry.id } }) === null, '[Cascade] ContextEntry cascade deleted');
    assert(await prisma_1.default.reasoning.findUnique({ where: { id: casReasoning.id } }) === null, '[Cascade] Reasoning cascade deleted');
    assert(await prisma_1.default.reasoningStep.findUnique({ where: { id: casReasoningStep.id } }) === null, '[Cascade] ReasoningStep cascade deleted');
    assert(await prisma_1.default.promptAssembly.findUnique({ where: { id: casPrompt.id } }) === null, '[Cascade] PromptAssembly cascade deleted');
    assert(await prisma_1.default.promptSection.findUnique({ where: { id: casPromptSection.id } }) === null, '[Cascade] PromptSection cascade deleted');
    assert(await prisma_1.default.execution.findUnique({ where: { id: casExec.id } }) === null, '[Cascade] Execution cascade deleted');
    assert(await prisma_1.default.executionUsage.findUnique({ where: { id: casUsage.id } }) === null, '[Cascade] ExecutionUsage cascade deleted');
    assert(await prisma_1.default.streaming.findUnique({ where: { id: casStream.id } }) === null, '[Cascade] Streaming cascade deleted');
    assert(await prisma_1.default.streamingChunk.findUnique({ where: { id: casStreamChunk.id } }) === null, '[Cascade] StreamingChunk cascade deleted');
    ...
    const tempContext = await prisma_1.default.contextWindow.create({
        data: { projectId: seedProjectId, windowName: 'temp', createdBy: 't', updatedBy: 't' }
    });
    const tempPrompt = await prisma_1.default.promptAssembly.create({
    ...
    await prisma_1.default.contextWindow.delete({ where: { id: tempContext.id } });
    const checkPrompt = await prisma_1.default.promptAssembly.findUnique({ where: { id: tempPrompt.id } });
    ...
    await prisma_1.default.execution.delete({ where: { id: tempExecForRest.id } });
    await prisma_1.default.provider.delete({ where: { id: tempProvider.id } });
    ...
    try {
        await prisma_1.default.providerModel.create({
    ...
    try {
        await prisma_1.default.provider.create({
    ...
    try {
        await prisma_1.default.executionUsage.create({
    ...
    try {
        await prisma_1.default.streaming.create({
    ...
    try {
        const recordsByUser = await prisma_1.default.conversation.findMany({ where: { userId: seedUserId } });
        assert(recordsByUser.length >= 1, 'Index lookup by userId successful');
        const recordsByProj = await prisma_1.default.conversation.findMany({ where: { projectId: seedProjectId } });
        assert(recordsByProj.length >= 1, 'Index lookup by projectId successful');
        const recordsByInv = await prisma_1.default.conversation.findMany({ where: { investigationId: seedInvestigationId } });
        assert(recordsByInv.length >= 1, 'Index lookup by investigationId successful');
        const recordsByProv = await prisma_1.default.execution.findMany({ where: { providerId: pGroq.id } });
        assert(recordsByProv.length >= 1, 'Index lookup by providerId successful');
        const recordsByConv = await prisma_1.default.sessionMemory.findMany({ where: { conversationId: conv.id } });
        assert(recordsByConv.length >= 1, 'Index lookup by conversationId successful');
        const recordsByExec = await prisma_1.default.streaming.findMany({ where: { executionId: exec.id } });
        assert(recordsByExec.length >= 1, 'Index lookup by executionId successful');
        const recordsByStatus = await prisma_1.default.conversation.findMany({ where: { status: client_1.ConversationStatus.ACTIVE } });
        assert(recordsByStatus.length >= 1, 'Index lookup by status successful');
        const recordsByCreatedAt = await prisma_1.default.conversation.findMany({ where: { createdAt: { lte: new Date() } } });
        assert(recordsByCreatedAt.length >= 1, 'Index lookup by createdAt successful');
        const recordsByUpdatedAt = await prisma_1.default.conversation.findMany({ where: { updatedAt: { lte: new Date() } } });
