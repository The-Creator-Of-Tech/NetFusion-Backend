...
async function main() {
    ...
    // 2 Tags
    const tag1 = await prisma_1.default.tag.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c001' } });
    ...
    const tag2 = await prisma_1.default.tag.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c002' } });
    ...
    // 2 Tag Assignments
    const ta1 = await prisma_1.default.tagAssignment.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c101' } });
    ...
    const ta2 = await prisma_1.default.tagAssignment.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c102' } });
    ...
    // 2 Comments
    const com1 = await prisma_1.default.comment.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c201' } });
    ...
    const com2 = await prisma_1.default.comment.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c202' } });
    ...
    // 2 Attachments
    const att1 = await prisma_1.default.attachment.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c301' } });
    ...
    const att2 = await prisma_1.default.attachment.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c302' } });
    ...
    // 1 Favorite
    const fav1 = await prisma_1.default.favorite.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c401' } });
    ...
    // 2 Notifications
    const not1 = await prisma_1.default.notification.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c501' } });
    ...
    const not2 = await prisma_1.default.notification.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c502' } });
    ...
    // 1 User Preference
    const pref1 = await prisma_1.default.userPreference.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c601' } });
    ...
    // 3 Activity Logs
    const act1 = await prisma_1.default.activityLog.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c701' } });
    ...
    const act2 = await prisma_1.default.activityLog.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c702' } });
    ...
    const act3 = await prisma_1.default.activityLog.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c703' } });
    ...
    // 2 System Settings
    const set1 = await prisma_1.default.systemSetting.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c801' } });
    ...
    const set2 = await prisma_1.default.systemSetting.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c802' } });
    ...
    // 2 API Keys
    const key1 = await prisma_1.default.apiKey.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c901' } });
    ...
    const key2 = await prisma_1.default.apiKey.findUnique({ where: { id: 'c29f2e3a-6f0a-4b9a-bbcb-7c73a1d9c902' } });
    ...
    // AttachmentType
    await testEnum('AttachmentType', Object.values(client_1.AttachmentType), (val) => prisma_1.default.attachment.create({
        data: { projectId, fileName: 't', fileSize: 10, mimeType: 't', storageKey: `t-${val}-${RUN}`, type: val, status: 'ACTIVE', createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.attachment.findUnique({ where: { id } }), (id, val) => prisma_1.default.attachment.update({ where: { id }, data: { type: val, version: 2 } }), (id) => prisma_1.default.attachment.delete({ where: { id } }));
    // AttachmentStatus
    ...
    // ActivityType
    await testEnum('ActivityType', Object.values(client_1.ActivityType), (val) => prisma_1.default.activityLog.create({
        data: { userId: adminUserId, action: 't', type: val, createdBy: 't', updatedBy: 't' }
    }), (id) => prisma_1.default.activityLog.findUnique({ where: { id } }), (id, val) => prisma_1.default.activityLog.update({ where: { id }, data: { type: val, version: 2 } }), (id) => prisma_1.default.activityLog.delete({ where: { id } }));
    // ApiKeyStatus
    ...
    // 1. Tag
    await testCRUD('Tag', () => prisma_1.default.tag.create({
        data: { projectId, name: `tag-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.tag.findUnique({ where: { id } }), (id) => prisma_1.default.tag.update({
        where: { id },
        data: { name: `tag-mod-${RUN}`, version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.tag.delete({ where: { id } }));
    // 2. TagAssignment
    const crudTag = await prisma_1.default.tag.create({
        data: { projectId, name: `temp-tag-${RUN}`, createdBy: 't', updatedBy: 't' }
    });
    await testCRUD('TagAssignment', () => prisma_1.default.tagAssignment.create({
        data: { tagId: crudTag.id, targetId: investigationId, targetType: 'investigation', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.tagAssignment.findUnique({ where: { id } }), (id) => prisma_1.default.tagAssignment.update({
        where: { id },
        data: { targetType: 'investigation-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.tagAssignment.delete({ where: { id } }));
    await prisma_1.default.tag.delete({ where: { id: crudTag.id } });
    // 3. Comment
    await testCRUD('Comment', () => prisma_1.default.comment.create({
        data: { userId: adminUserId, projectId, content: 't', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.comment.findUnique({ where: { id } }), (id) => prisma_1.default.comment.update({
        where: { id },
        data: { content: 't-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.comment.delete({ where: { id } }));
    // 4. Attachment
    await testCRUD('Attachment', () => prisma_1.default.attachment.create({
        data: { projectId, fileName: 't', fileSize: 10, mimeType: 't', storageKey: `s-${RUN}`, type: 'FILE', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.attachment.findUnique({ where: { id } }), (id) => prisma_1.default.attachment.update({
        where: { id },
        data: { fileName: 't-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.attachment.delete({ where: { id } }));
    // 5. Favorite
    await testCRUD('Favorite', () => prisma_1.default.favorite.create({
        data: { userId: adminUserId, targetId: randomTargetId(), type: 'INVESTIGATION', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.favorite.findUnique({ where: { id } }), (id) => prisma_1.default.favorite.update({
        where: { id },
        data: { type: 'PROJECT', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.favorite.delete({ where: { id } }));
    // 6. Notification
    await testCRUD('Notification', () => prisma_1.default.notification.create({
        data: { userId: adminUserId, title: 't', message: 'm', type: 'SYSTEM', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.notification.findUnique({ where: { id } }), (id) => prisma_1.default.notification.update({
        where: { id },
        data: { title: 't-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.notification.delete({ where: { id } }));
    // 7. UserPreference
    await testCRUD('UserPreference', () => prisma_1.default.userPreference.create({
        data: { userId: adminUserId, key: `k-${RUN}`, value: 'v', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.userPreference.findUnique({ where: { id } }), (id) => prisma_1.default.userPreference.update({
        where: { id },
        data: { value: 'v-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.userPreference.delete({ where: { id } }));
    // 8. ActivityLog
    await testCRUD('ActivityLog', () => prisma_1.default.activityLog.create({
        data: { userId: adminUserId, action: 't', type: 'OTHER', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.activityLog.findUnique({ where: { id } }), (id) => prisma_1.default.activityLog.update({
        where: { id },
        data: { action: 't-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.activityLog.delete({ where: { id } }));
    // 9. SystemSetting
    await testCRUD('SystemSetting', () => prisma_1.default.systemSetting.create({
        data: { key: `k-${RUN}`, value: 'v', createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.systemSetting.findUnique({ where: { id } }), (id) => prisma_1.default.systemSetting.update({
        where: { id },
        data: { value: 'v-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.systemSetting.delete({ where: { id } }));
    // 10. ApiKey
    await testCRUD('ApiKey', () => prisma_1.default.apiKey.create({
        data: { userId: adminUserId, name: 't', keyHash: `h-${RUN}`, createdBy: 'crud_test', updatedBy: 'crud_test' }
    }), (id) => prisma_1.default.apiKey.findUnique({ where: { id } }), (id) => prisma_1.default.apiKey.update({
        where: { id },
        data: { name: 't-mod', version: 2, updatedBy: 'crud_test_updated' }
    }), (id) => prisma_1.default.apiKey.delete({ where: { id } }));
    // ───────────────────────────────────────────────────────────────────────────
    ...
    const softDeleteModels = [
        {
            name: 'Tag',
            createFn: () => prisma_1.default.tag.create({ data: { projectId, name: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.tag.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.tag.delete({ where: { id } }),
        },
        {
            name: 'TagAssignment',
            createFn: async () => {
                const t = await prisma_1.default.tag.create({ data: { projectId, name: `soft-tag-${RUN}`, createdBy: 't', updatedBy: 't' } });
                return prisma_1.default.tagAssignment.create({ data: { tagId: t.id, targetId: investigationId, targetType: 'investigation', createdBy: 't', updatedBy: 't' } });
            },
            updateFn: (id, d) => prisma_1.default.tagAssignment.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: async (id) => {
                const record = await prisma_1.default.tagAssignment.findUnique({ where: { id } });
                if (record) {
                    await prisma_1.default.tagAssignment.delete({ where: { id } });
                    await prisma_1.default.tag.delete({ where: { id: record.tagId } });
                }
            },
        },
        {
            name: 'Comment',
            createFn: () => prisma_1.default.comment.create({ data: { userId: adminUserId, projectId, content: 't', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.comment.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.comment.delete({ where: { id } }),
        },
        {
            name: 'Attachment',
            createFn: () => prisma_1.default.attachment.create({ data: { projectId, fileName: 't', fileSize: 10, mimeType: 't', storageKey: `soft-${RUN}`, type: 'FILE', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.attachment.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.attachment.delete({ where: { id } }),
        },
        {
            name: 'Favorite',
            createFn: () => prisma_1.default.favorite.create({ data: { userId: adminUserId, targetId: randomTargetId(), type: 'INVESTIGATION', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.favorite.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.favorite.delete({ where: { id } }),
        },
        {
            name: 'Notification',
            createFn: () => prisma_1.default.notification.create({ data: { userId: adminUserId, title: 't', message: 'm', type: 'SYSTEM', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.notification.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.notification.delete({ where: { id } }),
        },
        {
            name: 'UserPreference',
            createFn: () => prisma_1.default.userPreference.create({ data: { userId: adminUserId, key: `soft-${RUN}`, value: 'v', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.userPreference.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.userPreference.delete({ where: { id } }),
        },
        {
            name: 'ActivityLog',
            createFn: () => prisma_1.default.activityLog.create({ data: { userId: adminUserId, action: 't', type: 'OTHER', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.activityLog.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.activityLog.delete({ where: { id } }),
        },
        {
            name: 'SystemSetting',
            createFn: () => prisma_1.default.systemSetting.create({ data: { key: `soft-${RUN}`, value: 'v', createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.systemSetting.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.systemSetting.delete({ where: { id } }),
        },
        {
            name: 'ApiKey',
            createFn: () => prisma_1.default.apiKey.create({ data: { userId: adminUserId, name: 't', keyHash: `soft-${RUN}`, createdBy: 't', updatedBy: 't' } }),
            updateFn: (id, d) => prisma_1.default.apiKey.update({ where: { id }, data: { deletedAt: d } }),
            deleteFn: (id) => prisma_1.default.apiKey.delete({ where: { id } }),
        },
    ];
    ...
    const popTag = await prisma_1.default.tag.findUnique({
        where: { id: tag1.id },
    ...
    const popComment = await prisma_1.default.comment.findUnique({
        where: { id: com1.id },
    ...
    const popAttachment = await prisma_1.default.attachment.findUnique({
        where: { id: att1.id },
    ...
    const casTag = await prisma_1.default.tag.create({
        data: { projectId, name: `cas-tag-${RUN}`, createdBy: 't', updatedBy: 't' }
    });
    const casAssignment = await prisma_1.default.tagAssignment.create({
        data: { tagId: casTag.id, targetId: investigationId, targetType: 'investigation', createdBy: 't', updatedBy: 't' }
    });
    await prisma_1.default.tag.delete({ where: { id: casTag.id } });
    assert(await prisma_1.default.tagAssignment.findUnique({ where: { id: casAssignment.id } }) === null, '[Cascade Tag] TagAssignment is deleted');
    // B. Cascade TagAssignment on Investigation delete
    const casInvUser = await prisma_1.default.user.create({
        data: { email: `cas-user-${RUN}@t.com`, username: `casuser-${RUN}`, displayName: 't', passwordHash: 't' }
    });
    const casInvProject = await prisma_1.default.project.create({
        data: { ownerId: casInvUser.id, name: 't' }
    });
    const casInvestigation = await prisma_1.default.investigation.create({
        data: { projectId: casInvProject.id, ownerId: casInvUser.id, title: 't' }
    });
    const casTag2 = await prisma_1.default.tag.create({
        data: { projectId: casInvProject.id, name: `cas-tag-2-${RUN}`, createdBy: 't', updatedBy: 't' }
    });
    const casAssignment2 = await prisma_1.default.tagAssignment.create({
    ...
    const casLog = await prisma_1.default.activityLog.create({
    ...
    await prisma_1.default.investigation.delete({ where: { id: casInvestigation.id } });
    assert(await prisma_1.default.tagAssignment.findUnique({ where: { id: casAssignment2.id } }) === null, '[Cascade Investigation] TagAssignment is deleted');
    assert(await prisma_1.default.comment.findUnique({ where: { id: casComment.id } }) === null, '[Cascade Investigation] Comment is deleted');
    assert(await prisma_1.default.attachment.findUnique({ where: { id: casAttachment.id } }) === null, '[Cascade Investigation] Attachment is deleted');
    assert(await prisma_1.default.activityLog.findUnique({ where: { id: casLog.id } }) === null, '[Cascade Investigation] ActivityLog is deleted');
    // Clean up
    await prisma_1.default.tag.delete({ where: { id: casTag2.id } });
    await prisma_1.default.project.delete({ where: { id: casInvProject.id } });
    await prisma_1.default.user.delete({ where: { id: casInvUser.id } });
    ...
    await prisma_1.default.systemSetting.create({
        data: { key: settingKey, value: 'v1', createdBy: 't', updatedBy: 't' }
    });
    await assertUniqueConflict(() => prisma_1.default.systemSetting.create({
    ...
    const tagProj = await prisma_1.default.project.create({
    ...
    await assertUniqueConflict(() => prisma_1.default.tag.create({
        data: { projectId: tagProj.id, name: 'unique-tag', createdBy: 't', updatedBy: 't' }
    }), 'Duplicate Tag name under same Project');
    // Clean up
    await prisma_1.default.tag.deleteMany({ where: { projectId: tagProj.id } });
    await prisma_1.default.project.delete({ where: { id: tagProj.id } });
    ...
    try {
        const listTag_Proj = await prisma_1.default.tag.findMany({ where: { projectId } });
        assert(listTag_Proj.length >= 1, 'Index lookup by projectId successful on Tag');
        const listComment_User = await prisma_1.default.comment.findMany({ where: { userId: adminUserId } });
        assert(listComment_User.length >= 1, 'Index lookup by userId successful on Comment');
        const listComment_Proj = await prisma_1.default.comment.findMany({ where: { projectId } });
        assert(listComment_Proj.length >= 1, 'Index lookup by projectId successful on Comment');
        const listComment_Inv = await prisma_1.default.comment.findMany({ where: { investigationId } });
        assert(listComment_Inv.length >= 1, 'Index lookup by investigationId successful on Comment');
        const listAtt_Proj = await prisma_1.default.attachment.findMany({ where: { projectId } });
