...
const ai_2 = require("./services/ai");
...
async function teardown() {
    try {
        ...
        await prisma_1.default.conversation.deleteMany({ where: { projectId } });
        await prisma_1.default.sessionMemory.deleteMany({ where: { projectId } });
        await prisma_1.default.contextWindow.deleteMany({ where: { projectId } });
        await prisma_1.default.promptAssembly.deleteMany({ where: { projectId } });
        await prisma_1.default.reasoning.deleteMany({ where: { projectId } });
        await prisma_1.default.execution.deleteMany({ where: { projectId } });
        await prisma_1.default.streaming.deleteMany({ where: {} }); // streaming has no projectId filter that's reliable
        await prisma_1.default.investigation.deleteMany({ where: { projectId } });
        await prisma_1.default.project.deleteMany({ where: { id: projectId } });
        await prisma_1.default.user.deleteMany({ where: { id: userId } });
    }
    catch (_) { /* best-effort cleanup */ }
}
async function seedProvider() {
    return ai_2.providerService.registerProvider({
...
async function seedConversation(projId) {
    return ai_2.conversationService.createConversation({
...
async function verifyEventInfrastructure() {
    ...
    for (const key of aiEvents) {
        assertDefined(ApplicationEvents_1.APP_EVENTS[key], `APP_EVENTS.${key}`);
        assertString(ApplicationEvents_1.APP_EVENTS[key], `APP_EVENTS.${key} value`);
    }
    ...
    for (const key of invEvents) {
        assertDefined(ApplicationEvents_1.APP_EVENTS[key], `APP_EVENTS.${key} preserved`);
    }
    // Event pub/sub roundtrip for each AI event
    for (const key of aiEvents) {
        const eventName = ApplicationEvents_1.APP_EVENTS[key];
        let received = false;
        const handler = (_) => { received = true; };
        EventPublisher_1.eventPublisher.subscribe(eventName, handler);
        await EventPublisher_1.eventPublisher.publish(eventName, { test: true });
        assert(received, `Event ${eventName} fired and received`);
        EventPublisher_1.eventPublisher.unsubscribe(eventName, handler);
    }
    // Event payload propagation
    let capturedPayload = null;
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_STARTED, (data) => { capturedPayload = data; });
    await EventPublisher_1.eventPublisher.publish(ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_STARTED, { executionId: 'test-exec', detail: 'ok' });
    ...
    EventPublisher_1.eventPublisher.clearAll();
    // Verify createOperationContext shape
    const ctx = (0, BaseApplicationService_1.createOperationContext)(ACTOR, { projectId });
...
async function verifyStartConversation() {
    console.log('\n═══ Section 2: AIOrchestrator.startConversation ═══');
    const result = await ai_1.aiOrchestrator.startConversation({
        projectId,
    ...
    const conv = await ai_2.conversationService.findConversation(result.conversationId);
    ...
    const memories = await ai_2.sessionMemoryService.findByProject(projectId);
    ...
    const windows = await ai_2.contextWindowService.findByConversation(result.conversationId);
    ...
    const result2 = await ai_1.aiOrchestrator.startConversation({
        projectId,
    ...
    const conv2 = await ai_2.conversationService.findConversation(result2.conversationId);
    assertEq(conv2.investigationId, investigationId, 'conv2.investigationId');
    assertArray(conv2.tags, 'conv2.tags');
    // Validation — missing projectId
    await assertThrows(() => ai_1.aiOrchestrator.startConversation({ projectId: 'bad', title: 'x', actor: ACTOR }), 'startConversation rejects invalid projectId UUID');
    // Multiple conversations
    for (let i = 0; i < 5; i++) {
        const r = await ai_1.aiOrchestrator.startConversation({ projectId, title: `Conv ${i}`, actor: ACTOR });
...
async function verifyMemoryManagement() {
    ...
    const loaded = await ai_1.aiOrchestrator.loadMemory({
    ...
    const loaded2 = await ai_1.aiOrchestrator.loadMemory({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
    });
    ...
    // saveMemory
    const saved = await ai_1.aiOrchestrator.saveMemory({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
        entries: [
    ...
    const entries = await ai_2.sessionMemoryService.findEntries(saved.memoryId);
    assertGte(entries.length, 3, 'at least 3 memory entries in DB');
    // saveMemory — incremental saves
    const saved2 = await ai_1.aiOrchestrator.saveMemory({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
        entries: [
    ...
    const entries2 = await ai_2.sessionMemoryService.findEntries(saved2.memoryId);
    assertGte(entries2.length, 4, 'cumulative entries >= 4');
    // Memory stats
    const stats = await ai_2.sessionMemoryService.getMemoryStats(saved.memoryId);
    ...
    // Validation
    await assertThrows(() => ai_1.aiOrchestrator.loadMemory({ conversationId: 'bad-uuid', projectId, actor: ACTOR }), 'loadMemory rejects invalid conversationId');
...
async function verifyBuildContext() {
    ...
    const r1 = await ai_1.aiOrchestrator.buildContext({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
    });
    ...
    const r2 = await ai_1.aiOrchestrator.buildContext({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
        investigationId,
        entries: [
            { source: 'ALERT', priority: 'HIGH', title: 'Alert 1', content: 'Suspicious outbound traffic', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 85, confidence: 0.9 },
            { source: 'FINDING', priority: 'MEDIUM', title: 'Finding 1', content: 'Port scan detected', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 70, confidence: 0.75 },
            { source: 'EVIDENCE', priority: 'LOW', title: 'Evidence 1', content: 'PCAP file captured', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 50, confidence: 0.6 },
        ],
    });
    ...
    const r3 = await ai_1.aiOrchestrator.buildContext({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
    });
    ...
    const entries = await ai_2.contextWindowService.findEntries(r2.contextId);
    assertGte(entries.length, 3, 'entries in DB >= 3');
    // Window stats
    const stats = await ai_2.contextWindowService.getWindowStats(r2.contextId);
    ...
    const ranked = await ai_2.contextWindowService.rankEntriesByImportance(r2.contextId);
    assertArray(ranked, 'rankEntriesByImportance returns array');
    assertGte(ranked.length, 3, 'ranked entries >= 3');
    if (ranked.length >= 2) {
        assert((ranked[0].importanceScore ?? 0) >= (ranked[ranked.length - 1].importanceScore ?? 0), 'entries ranked descending by importance');
    }
    // Validation
    await assertThrows(() => ai_1.aiOrchestrator.buildContext({ conversationId: 'bad', projectId, actor: ACTOR }), 'buildContext rejects invalid conversationId');
...
async function verifyConversationLifecycle() {
    ...
    const startResult = await ai_1.aiOrchestrator.startConversation({
        projectId,
        title: 'Lifecycle Test Conversation',
        actor: ACTOR,
    });
    // Summarize
    const summary = await ai_1.aiOrchestrator.summarizeConversation({
    ...
    const conv = await ai_2.conversationService.findConversation(startResult.conversationId);
    assertDefined(conv.summary, 'conversation.summary in DB');
    assertString(conv.summary, 'conversation.summary is string');
    // Close
    await ai_1.aiOrchestrator.closeConversation({
        conversationId: startResult.conversationId,
        actor: ACTOR,
    });
    const closed = await ai_2.conversationService.findConversation(startResult.conversationId);
    ...
    try {
        await ai_1.aiOrchestrator.closeConversation({ conversationId: startResult.conversationId, actor: ACTOR });
    ...
    // Not found
    await assertThrowsType(() => ai_1.aiOrchestrator.closeConversation({ conversationId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'closeConversation throws NotFound for unknown id');
    await assertThrowsType(() => ai_1.aiOrchestrator.summarizeConversation({ conversationId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'summarizeConversation throws NotFound for unknown id');
...
async function verifyRunPrompt() {
    console.log('\n═══ Section 6: AIOrchestrator.runPrompt ═══');
    const conv = await seedConversation();
    const r = await ai_1.aiOrchestrator.runPrompt({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
        investigationId,
    ...
    // Sections persisted
    const sections = await ai_2.promptAssemblyService.findSections(r.promptId);
    assertArray(sections, 'prompt sections array');
    assertGte(sections.length, 3, 'at least 3 sections saved');
    // Token budget check
    const budget = await ai_2.promptAssemblyService.checkTokenBudget(r.promptId);
    ...
    const r2 = await ai_1.aiOrchestrator.runPrompt({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
        investigationId,
    ...
    await assertThrows(() => ai_1.aiOrchestrator.runPrompt({
        conversationId: conv.id,
...
async function verifyExecutionAndReasoning() {
    ...
    await seedProvider();
    // runReasoning
    const rr = await ai_1.aiOrchestrator.runReasoning({
        projectId,
        investigationId,
        actor: ACTOR,
        userId,
    ...
    // Steps persisted
    const steps = await ai_2.reasoningService.findSteps(rr.reasoningId);
    assertArray(steps, 'reasoning steps array');
    assertGte(steps.length, 4, 'at least 4 steps saved');
    // executeAI
    const ea = await ai_1.aiOrchestrator.executeAI({
        projectId,
        actor: ACTOR,
        systemPrompt: 'You are a NetFusion SOC AI.',
    ...
    // streamResponse
    const sr = await ai_1.aiOrchestrator.streamResponse({
    ...
    const ea2 = await ai_1.aiOrchestrator.executeAI({
        projectId,
        actor: ACTOR,
        systemPrompt: 'sys',
        userPrompt: 'cancel me',
    });
    await ai_1.aiOrchestrator.cancelExecution({ executionId: ea2.executionId, actor: ACTOR });
    const cancelledExec = await ai_2.executionService.findExecution(ea2.executionId);
    // cancelExecution internally uses 'FAILED' status in the execution service
    assert(cancelledExec.status === 'FAILED', 'cancelled execution status=FAILED (cancelled)');
    // cancelExecution with streamingId
    const ea3 = await ai_1.aiOrchestrator.executeAI({ projectId, actor: ACTOR, systemPrompt: 's', userPrompt: 'u' });
    const sr2 = await ai_1.aiOrchestrator.streamResponse({
        executionId: ea3.executionId,
        actor: ACTOR,
        chunks: [{ content: 'partial', sequenceNumber: 1 }],
    });
    await ai_1.aiOrchestrator.cancelExecution({
        executionId: ea3.executionId,
        actor: ACTOR,
        streamingId: sr2.streamingId,
    });
    const cancelledStream = await ai_2.streamingService.findSession(sr2.streamingId);
    assert(cancelledStream.status === 'FAILED', 'streaming session status=FAILED (cancelled)');
    // cancelExecution — not found
    await assertThrowsType(() => ai_1.aiOrchestrator.cancelExecution({ executionId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'cancelExecution throws NotFound for unknown id');
...
async function verifyConversationOrchestrator() {
    ...
    await seedProvider();
    const conv = await seedConversation();
    const turn = await ai_1.conversationOrchestrator.processTurn({
        conversationId: conv.id,
        projectId,
        investigationId,
        actor: ACTOR,
        ...
        contextEntriesToAdd: [
            { source: 'ALERT', priority: 'HIGH', title: 'C2 Alert', content: 'C2 beacon detected', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 95, confidence: 0.95 },
        ],
    ...
    assertUuid(turn.streamingId, 'turn.streamingId valid UUID');
    assertUuid(turn.promptId, 'turn.promptId');
    assertUuid(turn.memoryId, 'turn.memoryId');
    assertUuid(turn.contextId, 'turn.contextId');
    ...
    const convStats = await ai_2.conversationService.getConversationStats(conv.id);
    ...
    // Memory entry saved
    const memories = await ai_2.sessionMemoryService.findByProject(projectId);
    const mem = memories.find((m) => m.id === turn.memoryId);
    assertDefined(mem, 'memory in DB');
    const memEntries = await ai_2.sessionMemoryService.findEntries(mem.id);
    assertGte(memEntries.length, 1, 'memory has at least 1 entry');
    // Context entry added
    const ctxEntries = await ai_2.contextWindowService.findEntries(turn.contextId);
    assertGte(ctxEntries.length, 1, 'context has at least 1 entry');
    // Second turn — no streaming
    const turn2 = await ai_1.conversationOrchestrator.processTurn({
        conversationId: conv.id,
        projectId,
        investigationId,
        actor: ACTOR,
    ...
    const stats2 = await ai_2.conversationService.getConversationStats(conv.id);
    assertGte(stats2.messageCount, 4, 'total messages >= 4 after 2 turns');
    // getHistory
    const hist = await ai_1.conversationOrchestrator.getHistory(conv.id, ACTOR);
    ...
    const buildCtxResult = await ai_1.aiOrchestrator.buildContext({
    ...
    const pruned = await ai_1.conversationOrchestrator.pruneContext({
        conversationId: conv2.id,
        actor: ACTOR,
    ...
    await assertThrowsType(() => ai_1.conversationOrchestrator.pruneContext({ conversationId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'pruneContext throws NotFound for unknown conversation');
    // Validation
    await assertThrows(() => ai_1.conversationOrchestrator.processTurn({
        conversationId: 'bad',
        projectId,
        investigationId,
        actor: ACTOR,
...
async function verifyPromptOrchestrator() {
    ...
    const conv = await seedConversation();
    // Build prompt
    const bp = await ai_1.promptOrchestrator.buildPrompt({
        projectId,
        investigationId,
        conversationId: conv.id,
        actor: ACTOR,
    ...
    const secs = await ai_2.promptAssemblyService.findSections(bp.promptId);
    assertGte(secs.length, 3, 'sections >= 3');
    if (secs.length >= 2) {
        assert(secs[0].priority >= secs[secs.length - 1].priority, 'sections ordered by priority descending');
    }
    // Token estimation
    const te = await ai_1.promptOrchestrator.estimateTokens(bp.promptId, ACTOR);
    ...
    // Cost estimation
    const ce = await ai_1.promptOrchestrator.estimateCost(bp.promptId, ACTOR);
    ...
    // Optimize prompt
    const op = await ai_1.promptOrchestrator.optimizePrompt({ promptId: bp.promptId, actor: ACTOR });
    ...
    // Compress context
    const buildCtx = await ai_1.aiOrchestrator.buildContext({
        conversationId: conv.id,
        projectId,
        actor: ACTOR,
    ...
    const compress = await ai_1.promptOrchestrator.compressContext({
    ...
    await assertThrowsType(() => ai_1.promptOrchestrator.compressContext({ contextId: (0, crypto_1.randomUUID)(), actor: ACTOR, maxTokenBudget: 100 }), BaseApplicationService_1.OrchestrationNotFoundError, 'compressContext throws NotFound');
    // Validation
    await assertThrows(() => ai_1.promptOrchestrator.buildPrompt({
        projectId: 'bad',
        investigationId,
        conversationId: conv.id,
        actor: ACTOR,
...
async function verifyReasoningOrchestrator() {
    ...
    const wf = await ai_1.reasoningOrchestrator.runWorkflow({
        projectId,
        investigationId,
        actor: ACTOR,
        userId,
    ...
    // Session in DB
    const session = await ai_2.reasoningService.findSession(wf.reasoningId);
    assertDefined(session, 'reasoning session in DB');
    assertEq(session.status, 'COMPLETED', 'session.status=COMPLETED');
    // Steps in DB
    const steps = await ai_2.reasoningService.findSteps(wf.reasoningId);
    assertGte(steps.length, 5, 'at least 5 steps in DB');
    // Confidence from stats
    const stats = await ai_1.reasoningOrchestrator.getStats(wf.reasoningId, ACTOR);
    ...
    const wf2 = await ai_1.reasoningOrchestrator.runWorkflow({
        projectId,
        investigationId,
        actor: ACTOR,
        steps: [
    ...
    const wf3 = await ai_1.reasoningOrchestrator.runWorkflow({
        projectId,
        investigationId,
        actor: ACTOR,
        steps: [
    ...
    const conv = await seedConversation();
    const freshWf = await ai_1.reasoningOrchestrator.runWorkflow({
        projectId,
        investigationId,
        actor: ACTOR,
    ...
    // getStats
    const s = await ai_1.reasoningOrchestrator.getStats(freshWf.reasoningId, ACTOR);
    assertNumber(s.overallConfidence, 'getStats.overallConfidence');
    // Validation
    await assertThrows(() => ai_1.reasoningOrchestrator.runWorkflow({
        projectId: 'not-uuid',
        investigationId,
        actor: ACTOR,
        steps: [],
...
async function verifyStreamingOrchestrator() {
    ...
    const exec = await ai_2.executionService.submitExecution({
    ...
    await ai_2.executionService.startExecution(exec.id, ACTOR);
    // startStream
    const ss = await ai_1.streamingOrchestrator.startStream({
        executionId: exec.id,
        actor: ACTOR,
        projectId,
        investigationId,
    ...
    // ingestChunks
    let progressCalls = 0;
    const ic = await ai_1.streamingOrchestrator.ingestChunks({
    ...
    // completeStream
    const cs = await ai_1.streamingOrchestrator.completeStream(ss.streamingId, ACTOR);
    assertEq(cs.status, 'COMPLETED', 'completeStream.status=COMPLETED');
    assertString(cs.reconstructed, 'completeStream.reconstructed');
    assert(cs.reconstructed.includes('Hello'), 'reconstructed includes first chunk');
    assert(cs.reconstructed.includes('NetFusion AI'), 'reconstructed includes last chunk');
    // getProgress
    const gp = await ai_1.streamingOrchestrator.getProgress(ss.streamingId, ACTOR);
    ...
    // reconstruct
    const rc = await ai_1.streamingOrchestrator.reconstruct(ss.streamingId, ACTOR);
    ...
    const exec2 = await ai_2.executionService.submitExecution({
        providerId: (await seedProvider()).id,
        projectId,
    ...
    await ai_2.executionService.startExecution(exec2.id, ACTOR);
    const ss2 = await ai_1.streamingOrchestrator.startStream({ executionId: exec2.id, actor: ACTOR });
    await ai_1.streamingOrchestrator.cancelStream({ streamingId: ss2.streamingId, actor: ACTOR, executionId: exec2.id });
    const cancelledSess = await ai_2.streamingService.findSession(ss2.streamingId);
    assert(cancelledSess.status === 'FAILED', 'cancelStream sets session status to FAILED');
    const cancelledExec2 = await ai_2.executionService.findExecution(exec2.id);
    assert(cancelledExec2.status === 'FAILED', 'cancelStream also sets execution to FAILED');
    // resumeStream — add more chunks after partial ingestion
    const exec3 = await ai_2.executionService.submitExecution({
        providerId: (await seedProvider()).id,
        projectId,
        investigationId: null,
        userId: null,
    ...
    await ai_2.executionService.startExecution(exec3.id, ACTOR);
    const ss3 = await ai_1.streamingOrchestrator.startStream({ executionId: exec3.id, actor: ACTOR });
    // First batch
    await ai_1.streamingOrchestrator.ingestChunks({
        streamingId: ss3.streamingId,
        actor: ACTOR,
        chunks: [{ content: 'Part 1', sequenceNumber: 1 }],
    });
    // Resume with more
    const resumed = await ai_1.streamingOrchestrator.resumeStream({
        streamingId: ss3.streamingId,
        actor: ACTOR,
    ...
    await ai_1.streamingOrchestrator.completeStream(ss3.streamingId, ACTOR);
    await assertThrowsType(() => ai_1.streamingOrchestrator.resumeStream({ streamingId: ss3.streamingId, actor: ACTOR, chunks: [] }), BaseApplicationService_1.OrchestrationError, 'resumeStream throws on COMPLETED stream');
    // Not found cases
    await assertThrowsType(() => ai_1.streamingOrchestrator.ingestChunks({ streamingId: (0, crypto_1.randomUUID)(), actor: ACTOR, chunks: [] }), BaseApplicationService_1.OrchestrationNotFoundError, 'ingestChunks throws NotFound');
    await assertThrowsType(() => ai_1.streamingOrchestrator.completeStream((0, crypto_1.randomUUID)(), ACTOR), BaseApplicationService_1.OrchestrationNotFoundError, 'completeStream throws NotFound');
    await assertThrowsType(() => ai_1.streamingOrchestrator.cancelStream({ streamingId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'cancelStream throws NotFound');
...
async function verifyProviderSelection() {
    ...
    const ollamaProvider = await ai_2.providerService.registerProvider({
    ...
    const deepseekProvider = await ai_2.providerService.registerProvider({
    ...
    const qwenProvider = await ai_2.providerService.registerProvider({
    ...
    // Select by priority
    const byPriority = await ai_2.providerService.selectProvider({ strategy: 'priority' });
    assertDefined(byPriority, 'selectProvider(priority) returns provider');
    // Select by health
    const byHealth = await ai_2.providerService.selectProvider({ strategy: 'health' });
    assertDefined(byHealth, 'selectProvider(health) returns provider');
    assertString(byHealth.providerName, 'byHealth.providerName');
    // Select by random
    const byRandom = await ai_2.providerService.selectProvider({ strategy: 'random' });
    ...
    const withStreaming = await ai_2.providerService.selectProvider({ strategy: 'priority', requireStreaming: true });
    ...
    const withTools = await ai_2.providerService.selectProvider({ strategy: 'priority', requireToolCalling: true });
    // May or may not return a provider depending on capabilities stored in metadata
    assertDefined(byHealth, 'selectProvider(health) still works after tool check');
    // List all enabled providers
    const all = await ai_2.providerService.findEnabled();
    ...
    const stats = await ai_2.providerService.getProviderStats(ollamaProvider.id);
    assertNumber(stats.healthScore, 'providerStats.healthScore');
    assertString(stats.status, 'providerStats.status');
    // Disable provider
    await ai_2.providerService.disableProvider(deepseekProvider.id, ACTOR);
    const disabled = await ai_2.providerService.findProvider(deepseekProvider.id);
    assertEq(disabled.enabled, false, 'provider disabled');
    // No provider available — disable all enabled then attempt
    const allProviders = await ai_2.providerService.findEnabled();
    for (const p of allProviders) {
        try {
            await ai_2.providerService.disableProvider(p.id, ACTOR);
        }
        catch (_) { }
    }
    await assertThrows(() => ai_1.aiOrchestrator.executeAI({ projectId, actor: ACTOR, systemPrompt: 'x', userPrompt: 'y' }), 'executeAI throws when no provider available');
    // Re-enable providers
    for (const p of allProviders) {
        try {
            await ai_2.providerService.enableProvider(p.id, ACTOR);
        }
        catch (_) { }
    }
...
async function verifyTransactionsAndRollbacks() {
    ...
    const goodResult = await ai_1.aiOrchestrator.startConversation({
        projectId,
    ...
    const conv = await ai_2.conversationService.findConversation(goodResult.conversationId);
    ...
    const convsBefore = await ai_2.conversationService.findByProject(projectId);
    const countBefore = convsBefore.length;
    await assertThrows(() => ai_1.aiOrchestrator.startConversation({ projectId: 'invalid-uuid', title: 'Bad', actor: ACTOR }), 'startConversation rolls back on bad projectId');
    const convsAfter = await ai_2.conversationService.findByProject(projectId);
    ...
    const execStats = await ai_2.executionService.aggregateProjectUsage(projectId);
    const execCountBefore = execStats.totalExecutions;
    await assertThrows(() => ai_1.aiOrchestrator.executeAI({ projectId: 'bad-uuid', actor: ACTOR, systemPrompt: 'x', userPrompt: 'y' }), 'executeAI does not create execution with bad projectId');
    const execStatsAfter = await ai_2.executionService.aggregateProjectUsage(projectId);
    ...
    await assertThrows(() => ai_1.aiOrchestrator.runReasoning({
        projectId: 'x',
        investigationId,
        actor: ACTOR,
        steps: [],
    ...
    const ctxA = (0, BaseApplicationService_1.createOperationContext)(ACTOR);
    const ctxB = (0, BaseApplicationService_1.createOperationContext)(ACTOR);
...
async function verifyEventPublishing() {
    ...
    const handlers = {};
    const eventsToTrack = [
        ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_STARTED,
        ApplicationEvents_1.APP_EVENTS.AI_MEMORY_LOADED,
        ApplicationEvents_1.APP_EVENTS.AI_CONTEXT_BUILT,
        ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_STARTED,
        ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_COMPLETED,
        ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_CONTINUED,
        ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_CLOSED,
        ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_SUMMARIZED,
        ApplicationEvents_1.APP_EVENTS.AI_MEMORY_SAVED,
        ApplicationEvents_1.APP_EVENTS.AI_PROMPT_BUILT,
        ApplicationEvents_1.APP_EVENTS.AI_REASONING_STARTED,
        ApplicationEvents_1.APP_EVENTS.AI_REASONING_COMPLETED,
        ApplicationEvents_1.APP_EVENTS.AI_STREAMING_STARTED,
        ApplicationEvents_1.APP_EVENTS.AI_STREAMING_FINISHED,
        ApplicationEvents_1.APP_EVENTS.AI_STREAMING_CANCELLED,
        ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_CANCELLED,
        ApplicationEvents_1.APP_EVENTS.AI_PROVIDER_SELECTED,
    ];
    for (const ev of eventsToTrack) {
        capturedEvents[ev] = [];
        handlers[ev] = (data) => { capturedEvents[ev].push(data); };
        EventPublisher_1.eventPublisher.subscribe(ev, handlers[ev]);
    }
    // startConversation fires AI_CONVERSATION_STARTED
    const startResult = await ai_1.aiOrchestrator.startConversation({ projectId, title: 'Event Test Conv', actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_STARTED].length, 1, 'AI_CONVERSATION_STARTED fired');
    const startEvt = capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_STARTED][0];
    ...
    await ai_1.aiOrchestrator.loadMemory({ conversationId: startResult.conversationId, projectId, actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_MEMORY_LOADED].length, 1, 'AI_MEMORY_LOADED fired');
    // saveMemory fires AI_MEMORY_SAVED
    await ai_1.aiOrchestrator.saveMemory({
        conversationId: startResult.conversationId,
        projectId,
        actor: ACTOR,
        entries: [{ memoryType: 'FACT', state: 'ACTIVE', title: 'T', content: 'C' }],
    });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_MEMORY_SAVED].length, 1, 'AI_MEMORY_SAVED fired');
    // buildContext fires AI_CONTEXT_BUILT
    await ai_1.aiOrchestrator.buildContext({ conversationId: startResult.conversationId, projectId, actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_CONTEXT_BUILT].length, 1, 'AI_CONTEXT_BUILT fired');
    // runPrompt fires AI_PROMPT_BUILT
    await ai_1.aiOrchestrator.runPrompt({
        conversationId: startResult.conversationId,
        projectId,
        investigationId,
        actor: ACTOR,
    ...
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_PROMPT_BUILT].length, 1, 'AI_PROMPT_BUILT fired');
    // runReasoning fires AI_REASONING_STARTED + AI_REASONING_COMPLETED
    await ai_1.aiOrchestrator.runReasoning({
        projectId,
        investigationId,
        actor: ACTOR,
        steps: [{ stage: 'TEST', inputSummary: 'in', outputSummary: 'out', confidence: 0.8 }],
    });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_REASONING_STARTED].length, 1, 'AI_REASONING_STARTED fired');
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_REASONING_COMPLETED].length, 1, 'AI_REASONING_COMPLETED fired');
    // executeAI fires AI_PROVIDER_SELECTED + AI_EXECUTION_STARTED
    const ea = await ai_1.aiOrchestrator.executeAI({ projectId, actor: ACTOR, systemPrompt: 's', userPrompt: 'u' });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_PROVIDER_SELECTED].length, 1, 'AI_PROVIDER_SELECTED fired');
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_STARTED].length, 1, 'AI_EXECUTION_STARTED fired');
    // streamResponse fires AI_STREAMING_STARTED + AI_STREAMING_FINISHED
    await ai_1.aiOrchestrator.streamResponse({
        executionId: ea.executionId,
        actor: ACTOR,
        projectId,
        chunks: [{ content: 'hi', sequenceNumber: 1, finishReason: 'stop' }],
    });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_STREAMING_STARTED].length, 1, 'AI_STREAMING_STARTED fired');
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_STREAMING_FINISHED].length, 1, 'AI_STREAMING_FINISHED fired');
    // continueConversation fires AI_EXECUTION_STARTED + AI_EXECUTION_COMPLETED + AI_CONVERSATION_CONTINUED
    await ai_1.aiOrchestrator.continueConversation({
        conversationId: startResult.conversationId,
        userMessage: 'Next question?',
        actor: ACTOR,
        projectId,
    });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_COMPLETED].length, 1, 'AI_EXECUTION_COMPLETED fired');
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_CONTINUED].length, 1, 'AI_CONVERSATION_CONTINUED fired');
    // closeConversation fires AI_CONVERSATION_CLOSED
    await ai_1.aiOrchestrator.closeConversation({ conversationId: startResult.conversationId, actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_CLOSED].length, 1, 'AI_CONVERSATION_CLOSED fired');
    // cancelExecution fires AI_EXECUTION_CANCELLED
    const ea2 = await ai_1.aiOrchestrator.executeAI({ projectId, actor: ACTOR, systemPrompt: 's', userPrompt: 'u' });
    await ai_1.aiOrchestrator.cancelExecution({ executionId: ea2.executionId, actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_EXECUTION_CANCELLED].length, 1, 'AI_EXECUTION_CANCELLED fired');
    // streamingOrchestrator cancelStream fires AI_STREAMING_CANCELLED
    const exec4 = await ai_2.executionService.submitExecution({
    ...
    await ai_2.executionService.startExecution(exec4.id, ACTOR);
    const ss4 = await ai_1.streamingOrchestrator.startStream({ executionId: exec4.id, actor: ACTOR });
    await ai_1.streamingOrchestrator.cancelStream({ streamingId: ss4.streamingId, actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_STREAMING_CANCELLED].length, 1, 'AI_STREAMING_CANCELLED fired');
    // Summarize conversation — new conv
    const sumConv = await ai_1.aiOrchestrator.startConversation({ projectId, title: 'Sum Conv', actor: ACTOR });
    await ai_1.aiOrchestrator.summarizeConversation({ conversationId: sumConv.conversationId, actor: ACTOR });
    assertGte(capturedEvents[ApplicationEvents_1.APP_EVENTS.AI_CONVERSATION_SUMMARIZED].length, 1, 'AI_CONVERSATION_SUMMARIZED fired');
    // Cleanup
    for (const ev of eventsToTrack) {
...
async function verifyEndToEndWorkflow() {
    ...
    const conv = await ai_1.aiOrchestrator.startConversation({
        projectId,
    ...
    const mem = await ai_1.aiOrchestrator.saveMemory({
    ...
    const ctx = await ai_1.aiOrchestrator.buildContext({
        conversationId: conv.conversationId,
        projectId,
        investigationId,
        actor: ACTOR,
        entries: [
            { source: 'ALERT', priority: 'CRITICAL', title: 'Ransomware Alert', content: 'File encryption detected on workstation-12', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 100, confidence: 0.99 },
            { source: 'FINDING', priority: 'HIGH', title: 'SMB Exploit', content: 'EternalBlue exploit attempted', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 90, confidence: 0.90 },
        ],
    });
    assertGte(ctx.entryCount, 2, 'e2e: context built');
    // Step 4: Assemble prompt
    const prompt = await ai_1.aiOrchestrator.runPrompt({
        conversationId: conv.conversationId,
        projectId,
        investigationId,
        actor: ACTOR,
    ...
    const reasoning = await ai_1.aiOrchestrator.runReasoning({
        projectId,
        investigationId,
        actor: ACTOR,
        steps: [
    ...
    // Step 6: Execute AI
    const exec = await ai_1.aiOrchestrator.executeAI({
        projectId,
        investigationId,
        actor: ACTOR,
    ...
    const stream = await ai_1.aiOrchestrator.streamResponse({
    ...
    const turn = await ai_1.aiOrchestrator.continueConversation({
        conversationId: conv.conversationId,
    ...
    await ai_1.aiOrchestrator.closeConversation({ conversationId: conv.conversationId, actor: ACTOR });
    const closed = await ai_2.conversationService.findConversation(conv.conversationId);
    assertEq(closed.status, 'COMPLETED', 'e2e: conversation closed');
    // Verify overall conversation state
    const finalStats = await ai_2.conversationService.getConversationStats(conv.conversationId);
...
async function verifyBulkOperations() {
    ...
    const convPromises = Array.from({ length: 10 }, (_, i) => ai_1.aiOrchestrator.startConversation({ projectId, title: `Bulk Conv ${i}`, actor: ACTOR }));
    ...
    const memSavePromises = convs.slice(0, 5).map((c) => ai_1.aiOrchestrator.saveMemory({
    ...
    const reasoningPromises = Array.from({ length: 5 }, (_, i) => ai_1.aiOrchestrator.runReasoning({
        projectId,
        investigationId,
        actor: ACTOR,
        steps: [
    ...
    const promptPromises = convs.slice(5).map((c) => ai_1.aiOrchestrator.runPrompt({
        conversationId: c.conversationId,
        projectId,
        investigationId,
        actor: ACTOR,
    ...
    await Promise.all(convs.map((c) => ai_1.aiOrchestrator.closeConversation({ conversationId: c.conversationId, actor: ACTOR })));
    // Verify all closed
    const closedChecks = await Promise.all(convs.map((c) => ai_2.conversationService.findConversation(c.conversationId)));
...
async function verifyContinueConversation() {
    ...
    await seedProvider();
    const conv = await ai_1.aiOrchestrator.startConversation({ projectId, title: 'Continue Test', actor: ACTOR });
    // Basic continue
    const r1 = await ai_1.aiOrchestrator.continueConversation({
        conversationId: conv.conversationId,
    ...
    const r2 = await ai_1.aiOrchestrator.continueConversation({
        conversationId: conv.conversationId,
    ...
    assertUuid(r2.streamingId, 'continue with streaming: streamingId set');
    // Messages persisted — check via stats
    const convMsgStats = await ai_2.conversationService.getConversationStats(conv.conversationId);
    ...
    await assertThrowsType(() => ai_1.aiOrchestrator.continueConversation({
        conversationId: (0, crypto_1.randomUUID)(),
        userMessage: 'hello',
        actor: ACTOR,
        projectId,
    }), BaseApplicationService_1.OrchestrationNotFoundError, 'continueConversation throws NotFound for unknown conversation');
    // Validation — bad conversationId
    await assertThrows(() => ai_1.aiOrchestrator.continueConversation({
        conversationId: 'not-uuid',
        userMessage: 'x',
        actor: ACTOR,
        projectId,
    ...
    // Multiple turns
    for (let i = 0; i < 3; i++) {
        const r = await ai_1.aiOrchestrator.continueConversation({
            conversationId: conv.conversationId,
    ...
    const finalMsgStats = await ai_2.conversationService.getConversationStats(conv.conversationId);
...
async function verifyDeepMemoryCoverage() {
    ...
    for (let c = 0; c < CONV_COUNT; c++) {
        const conv = await seedConversation();
        // Create memory
        const loaded = await ai_1.aiOrchestrator.loadMemory({ conversationId: conv.id, projectId, actor: ACTOR });
        ...
        const saved = await ai_1.aiOrchestrator.saveMemory({ conversationId: conv.id, projectId, actor: ACTOR, entries });
        assertUuid(saved.memoryId, `S18[${c}] saved.memoryId`);
        assertEq(saved.entryCount, ENTRIES_PER, `S18[${c}] saved ${ENTRIES_PER} entries`);
        // Verify in DB
        const dbEntries = await ai_2.sessionMemoryService.findEntries(saved.memoryId);
        ...
        // Idempotent load
        const reloaded = await ai_1.aiOrchestrator.loadMemory({ conversationId: conv.id, projectId, actor: ACTOR });
...
async function verifyDeepContextCoverage() {
    ...
    for (let c = 0; c < CONV_COUNT; c++) {
        ...
        const built = await ai_1.aiOrchestrator.buildContext({ conversationId: conv.id, projectId, actor: ACTOR, investigationId, entries });
        ...
        // Stats
        const stats = await ai_2.contextWindowService.getWindowStats(built.contextId);
        ...
        // Ranking
        const ranked = await ai_2.contextWindowService.rankEntriesByImportance(built.contextId);
        ...
        const built2 = await ai_1.aiOrchestrator.buildContext({ conversationId: conv.id, projectId, actor: ACTOR });
...
async function verifyDeepPromptCoverage() {
    ...
    for (let p = 0; p < PROMPT_COUNT; p++) {
        ...
        // Sections count
        const savedSections = await ai_2.promptAssemblyService.findSections(r.promptId);
...
async function verifyDeepReasoningCoverage() {
    ...
    for (let s = 0; s < SESSION_COUNT; s++) {
        const steps = STAGES.map((stage, i) => ({
            ...
            evidenceIds: [(0, crypto_1.randomUUID)()],
            findingIds: [(0, crypto_1.randomUUID)()],
        }));
        const wf = await ai_1.reasoningOrchestrator.runWorkflow({
            projectId,
            investigationId,
        ...
        // Stats from service
        const stats = await ai_2.reasoningService.getSessionStats(wf.reasoningId);
...
async function verifyDeepStreamingCoverage() {
    ...
    for (let s = 0; s < STREAM_COUNT; s++) {
        ...
        await ai_2.executionService.startExecution(exec.id, ACTOR);
        // Start stream
        const ss = await ai_1.streamingOrchestrator.startStream({ executionId: exec.id, actor: ACTOR, projectId });
        ...
        const ic = await ai_1.streamingOrchestrator.ingestChunks({ streamingId: ss.streamingId, actor: ACTOR, chunks });
        ...
        assertString(cs.reconstructed, `S22[${s}] reconstructed`);
        assert(cs.reconstructed.includes(`chunk-${s}-0`), `S22[${s}] reconstructed has first chunk`);
        // Reconstruct
        const rc = await ai_1.streamingOrchestrator.reconstruct(ss.streamingId, ACTOR);
...
async function verifyDeepConversationTurns() {
    ...
    for (let c = 0; c < CONV_COUNT; c++) {
        ...
        for (let t = 0; t < TURNS_PER; t++) {
            ...
            // Message count grows
            const stats = await ai_2.conversationService.getConversationStats(conv.conversationId);
...
async function verifyDeepProviderCoverage() {
    ...
    for (let p = 0; p < PROVIDER_COUNT; p++) {
        const prov = await ai_2.providerService.registerProvider({
    ...
    for (const strategy of ['priority', 'health', 'random']) {
        const selected = await ai_2.providerService.selectProvider({ strategy });
    ...
    const enabled = await ai_2.providerService.findEnabled();
    ...
    for (let p = 0; p < 5; p++) {
        await ai_2.providerService.updateHealthScore(providers[p].id, 50.0 + p * 5, ACTOR);
        const found = await ai_2.providerService.findProvider(providers[p].id);
        assertDefined(found, `S24 updated provider[${p}] found`);
    }
    // Stats for first 5 providers
    for (let p = 0; p < 5; p++) {
        const stats = await ai_2.providerService.getProviderStats(providers[p].id);
    ...
    for (let p = 0; p < 5; p++) {
        await ai_2.providerService.disableProvider(providers[p].id, ACTOR);
        const dis = await ai_2.providerService.findProvider(providers[p].id);
        assertEq(dis.enabled, false, `S24[${p}] disabled`);
        await ai_2.providerService.enableProvider(providers[p].id, ACTOR);
        const en = await ai_2.providerService.findProvider(providers[p].id);
...
async function verifyDeepExecutionCoverage() {
    ...
    for (let e = 0; e < EXEC_COUNT; e++) {
        const provider = await ai_2.providerService.selectProvider({ strategy: 'priority' });
        if (!provider) {
            assert(false, `S25[${e}] provider available`);
            continue;
        }
        const exec = await ai_2.executionService.submitExecution({
            providerId: provider.id,
        ...
        // Start
        await ai_2.executionService.startExecution(exec.id, ACTOR);
        const active = await ai_2.executionService.findExecution(exec.id);
        assertEq(active.status, 'ACTIVE', `S25[${e}] status=ACTIVE after start`);
        // Record usage
        await ai_2.executionService.recordUsage(exec.id, {
            promptTokens: 100 + e,
        ...
        // Complete
        await ai_2.executionService.completeExecution(exec.id, ACTOR);
        const done = await ai_2.executionService.findExecution(exec.id);
        assertEq(done.status, 'COMPLETED', `S25[${e}] status=COMPLETED`);
        // Usage stats
        const usage = await ai_2.executionService.getUsageStats(exec.id);
    ...
    const agg = await ai_2.executionService.aggregateProjectUsage(projectId);
...
async function verifyFullWorkflowStress() {
    ...
    for (let r = 0; r < RUN_COUNT; r++) {
        ...
        const ctx = await ai_1.aiOrchestrator.buildContext({
            ...
            actor: ACTOR,
            entries: [
                { source: 'ALERT', priority: 'CRITICAL', title: `Alert: ${scenario.name}`, content: scenario.threat, referenceId: (0, crypto_1.randomUUID)(), importanceScore: 100 },
            ],
        });
        ...
        // 8. Summarize
        const summary = await ai_1.aiOrchestrator.summarizeConversation({ conversationId: conv.conversationId, actor: ACTOR });
...
async function verifyExhaustiveValidation() {
    ...
    for (const badId of badUuids) {
        // startConversation
        await assertThrows(() => ai_1.aiOrchestrator.startConversation({ projectId: badId, title: 'x', actor: ACTOR }), `startConversation rejects "${badId}" as projectId`);
        // loadMemory
        await assertThrows(() => ai_1.aiOrchestrator.loadMemory({ conversationId: badId, projectId, actor: ACTOR }), `loadMemory rejects "${badId}" as conversationId`);
        // buildContext
        await assertThrows(() => ai_1.aiOrchestrator.buildContext({ conversationId: badId, projectId, actor: ACTOR }), `buildContext rejects "${badId}" as conversationId`);
        // runReasoning
        await assertThrows(() => ai_1.aiOrchestrator.runReasoning({ projectId: badId, investigationId, actor: ACTOR, steps: [] }), `runReasoning rejects "${badId}" as projectId`);
        // executeAI
        await assertThrows(() => ai_1.aiOrchestrator.executeAI({ projectId: badId, actor: ACTOR, systemPrompt: 's', userPrompt: 'u' }), `executeAI rejects "${badId}" as projectId`);
        // cancelExecution
        await assertThrows(() => ai_1.aiOrchestrator.cancelExecution({ executionId: badId, actor: ACTOR }), `cancelExecution rejects "${badId}" as executionId`);
        // closeConversation
        await assertThrows(() => ai_1.aiOrchestrator.closeConversation({ conversationId: badId, actor: ACTOR }), `closeConversation rejects "${badId}" as conversationId`);
    }
    // Not-found cases across orchestrators
    await assertThrowsType(() => ai_1.aiOrchestrator.closeConversation({ conversationId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'closeConversation NotFound on random UUID');
    await assertThrowsType(() => ai_1.aiOrchestrator.summarizeConversation({ conversationId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'summarizeConversation NotFound on random UUID');
    await assertThrowsType(() => ai_1.aiOrchestrator.cancelExecution({ executionId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'cancelExecution NotFound on random UUID');
    await assertThrowsType(() => ai_1.promptOrchestrator.compressContext({ contextId: (0, crypto_1.randomUUID)(), actor: ACTOR, maxTokenBudget: 100 }), BaseApplicationService_1.OrchestrationNotFoundError, 'compressContext NotFound on random UUID');
    await assertThrowsType(() => ai_1.promptOrchestrator.optimizePrompt({ promptId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'optimizePrompt NotFound on random UUID');
    await assertThrowsType(() => ai_1.streamingOrchestrator.ingestChunks({ streamingId: (0, crypto_1.randomUUID)(), actor: ACTOR, chunks: [] }), BaseApplicationService_1.OrchestrationNotFoundError, 'ingestChunks NotFound on random UUID');
    await assertThrowsType(() => ai_1.streamingOrchestrator.completeStream((0, crypto_1.randomUUID)(), ACTOR), BaseApplicationService_1.OrchestrationNotFoundError, 'completeStream NotFound on random UUID');
    await assertThrowsType(() => ai_1.streamingOrchestrator.cancelStream({ streamingId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'cancelStream NotFound on random UUID');
    await assertThrowsType(() => ai_1.conversationOrchestrator.pruneContext({ conversationId: (0, crypto_1.randomUUID)(), actor: ACTOR }), BaseApplicationService_1.OrchestrationNotFoundError, 'pruneContext NotFound on random UUID');
    // continueConversation on non-existent
    await assertThrowsType(() => ai_1.aiOrchestrator.continueConversation({ conversationId: (0, crypto_1.randomUUID)(), userMessage: 'hi', actor: ACTOR, projectId }), BaseApplicationService_1.OrchestrationNotFoundError, 'continueConversation NotFound on random UUID');
    // processTurn on non-existent
    await assertThrowsType(() => ai_1.conversationOrchestrator.processTurn({ conversationId: (0, crypto_1.randomUUID)(), projectId, investigationId, actor: ACTOR, userMessage: 'hi' }), BaseApplicationService_1.OrchestrationNotFoundError, 'processTurn NotFound on random UUID');
...
async function verifyOperationContextCoverage() {
    ...
    for (let i = 0; i < CONTEXT_COUNT; i++) {
        const ctx = (0, BaseApplicationService_1.createOperationContext)(`actor-${i}`, {
            projectId,
            investigationId,
        });
    ...
    const ctxP = (0, BaseApplicationService_1.createOperationContext)('actor-x', { projectId });
    assertEq(ctxP.projectId, projectId, 'S29 ctx with only projectId');
    assert(ctxP.investigationId === undefined || ctxP.investigationId === null || ctxP.investigationId === '', 'S29 ctx.investigationId not set when not provided');
    // Verify orchestrator responses include correlationId
    const conv = await ai_1.aiOrchestrator.startConversation({ projectId, title: 'Ctx Test', actor: ACTOR });
...
async function verifyServiceLayerIntegration() {
    ...
    await seedProvider();
    // ConversationService
    const conv = await ai_2.conversationService.createConversation({
        projectId,
    ...
    // Add messages
    const msg1 = await ai_2.conversationService.addMessage(conv.id, { role: 'user', content: 'Hello SOC AI', createdBy: ACTOR, updatedBy: ACTOR });
    assertUuid(msg1.id, 'SL msg1.id');
    assertEq(msg1.role, 'user', 'SL msg1.role');
    const msg2 = await ai_2.conversationService.addMessage(conv.id, { role: 'assistant', content: 'Hello! I can help you.', parentMessageId: msg1.id, createdBy: ACTOR, updatedBy: ACTOR });
    assertUuid(msg2.id, 'SL msg2.id');
    assertEq(msg2.role, 'assistant', 'SL msg2.role');
    const stats = await ai_2.conversationService.getConversationStats(conv.id);
    assertNumber(stats.messageCount, 'SL stats.messageCount');
    assertGte(stats.messageCount, 2, 'SL messageCount >= 2');
    // SessionMemoryService
    const mem = await ai_2.sessionMemoryService.createMemory({
        conversationId: conv.id,
        projectId,
        investigationId: null,
        userId: null,
    ...
    await ai_2.sessionMemoryService.addEntry(mem.id, { memoryType: 'FACT', state: 'ACTIVE', title: 'T1', content: 'C1', importanceScore: 80, confidence: 0.8, createdBy: ACTOR, updatedBy: ACTOR });
    await ai_2.sessionMemoryService.addEntry(mem.id, { memoryType: 'CONTEXT', state: 'ACTIVE', title: 'T2', content: 'C2', importanceScore: 70, confidence: 0.7, createdBy: ACTOR, updatedBy: ACTOR });
    const entries = await ai_2.sessionMemoryService.findEntries(mem.id);
    assertGte(entries.length, 2, 'SL memory entries >= 2');
    const memStats = await ai_2.sessionMemoryService.getMemoryStats(mem.id);
    assertNumber(memStats.entryCount, 'SL memStats.entryCount');
    assertGte(memStats.entryCount, 2, 'SL memStats.entryCount >= 2');
    // ContextWindowService
    const win = await ai_2.contextWindowService.createWindow({
        projectId,
        conversationId: conv.id,
        investigationId: null,
        userId: null,
    ...
    await ai_2.contextWindowService.addEntry(win.id, { source: 'ALERT', priority: 'HIGH', title: 'Alert', content: 'Suspicious traffic', referenceId: (0, crypto_1.randomUUID)(), importanceScore: 90, confidence: 0.9, createdBy: ACTOR, updatedBy: ACTOR });
    const winStats = await ai_2.contextWindowService.getWindowStats(win.id);
    assertNumber(winStats.entryCount, 'SL winStats.entryCount');
    assertGte(winStats.entryCount, 1, 'SL winStats.entryCount >= 1');
    // PromptAssemblyService
    const promptRec = await ai_2.promptAssemblyService.createPrompt({
        investigationId,
        projectId,
    ...
    await ai_2.promptAssemblyService.publishPrompt(promptRec.id, ACTOR);
    const assembled = await ai_2.promptAssemblyService.assemblePrompt(promptRec.id);
    assertString(assembled, 'SL assembled prompt');
    const budget = await ai_2.promptAssemblyService.checkTokenBudget(promptRec.id);
    assertNumber(budget.estimatedTokens, 'SL budget.estimatedTokens');
    assertBoolean(budget.withinBudget, 'SL budget.withinBudget');
    // ReasoningService
    const rsess = await ai_2.reasoningService.createSession({
        projectId,
        investigationId,
        userId: null,
        status: 'ACTIVE',
    ...
    await ai_2.reasoningService.addStep(rsess.id, { stepNumber: 1, stage: 'TRIAGE', inputSummary: 'in', outputSummary: 'out', confidence: 0.85, evidenceIds: [], findingIds: [], createdBy: ACTOR, updatedBy: ACTOR });
    await ai_2.reasoningService.completeSession(rsess.id, 'Decision made.', ACTOR);
    const rsStats = await ai_2.reasoningService.getSessionStats(rsess.id);
    assertNumber(rsStats.overallConfidence, 'SL rsStats.overallConfidence');
    assertGte(rsStats.stepCount, 1, 'SL rsStats.stepCount >= 1');
    // StreamingService
    const provider = await ai_2.providerService.selectProvider({ strategy: 'priority' });
    const execR = await ai_2.executionService.submitExecution({
        providerId: provider.id,
        projectId,
        investigationId: null,
        userId: null,
    ...
    await ai_2.executionService.startExecution(execR.id, ACTOR);
    const stream = await ai_2.streamingService.createSession({
    ...
    await ai_2.streamingService.appendChunk(stream.id, { sequenceNumber: 1, content: 'Hello', finishReason: 'stop', createdBy: ACTOR, updatedBy: ACTOR });
    const streamStats = await ai_2.streamingService.getStreamingStats(stream.id);
    ...
    await ai_2.streamingService.completeSession(stream.id, ACTOR);
    const recon = await ai_2.streamingService.reconstructContent(stream.id);
