...
const knowledge_1 = require("./application/knowledge");
const BaseApplicationService_1 = require("./application/base/BaseApplicationService");
const knowledge_2 = require("./services/knowledge");
...
function assertGte(value, min, label) {
    assert(value >= min, `${label} >= ${min}`, `got: ${value}`);
}
function assertLte(value, max, label) {
...
async function assertThrowsType(fn, errorType, label) {
    try {
        await fn();
        failed++;
...
async function setupCore() {
    ...
    const technique = await knowledge_2.mitreService.createTechnique({
        mitreId: `T${techNum}`,
        tacticId: tactic.id,
    ...
    // Sub-technique
    const subTech = await knowledge_2.mitreService.createTechnique({
    ...
    // Mitigation
    const mitigation = await knowledge_2.mitreService.createMitigation({
        mitreId: `M_KNO_${RUN}`,
    ...
    const cve = await knowledge_2.cveService.createCve({
    ...
    const cve2 = await knowledge_2.cveService.createCve({
    ...
    // CVSS
    const cvss = await knowledge_2.cveService.upsertCvss(cve.id, {
        baseScore: 8.5,
    ...
    // Affected product
    const product = await knowledge_2.cveService.addAffectedProduct(cve.id, {
        vendor: 'TestVendor',
        product: 'TestProduct',
    ...
    // Link CVE → technique
    await knowledge_2.cveService.correlateToTechniques(cve.id, [technique.id], ACTOR);
    // IOC 1 (IP)
    const ioc = await knowledge_2.iocService.createIoc({
        iocId: `IOC_IP_${RUN}`,
    ...
    // IOC 2 (hash)
    const ioc2 = await knowledge_2.iocService.createIoc({
        iocId: `IOC_HASH_${RUN}`,
    ...
    // IOC enrichment
    const enrichment = await knowledge_2.iocService.enrichIoc(ioc.id, {
        reputationScore: 85,
        malicious: true,
    ...
    // Threat actor
    const threatActor = await knowledge_2.threatService.createThreatActor({
    ...
    // Campaign
    const campaign = await knowledge_2.threatService.createCampaign({
...
async function teardown(ctx) {
    try {
        await prisma_1.default.threatRelationship.deleteMany({ where: { OR: [{ threatId: ctx.threatActorId }, { cveId: ctx.cveId }] } });
        await prisma_1.default.iOCRelationship.deleteMany({ where: { iocId: { in: [ctx.iocId, ctx.iocId2] } } });
        if (ctx.campaignId)
            await prisma_1.default.threatCampaign.deleteMany({ where: { id: ctx.campaignId } });
        if (ctx.threatActorId)
            await prisma_1.default.threatActor.deleteMany({ where: { id: ctx.threatActorId } });
        if (ctx.enrichmentId)
...
// ─────────────────────────────────────────────────────────────────────────────
async function testEventInfrastructure() {
    section('1. Event Infrastructure — Knowledge events defined');
    // All knowledge APP_EVENTS constants exist
    assertString(ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL, 'APP_EVENTS.FINDING_CORRELATED_FULL defined');
    assertString(ApplicationEvents_1.APP_EVENTS.ASSET_CORRELATED, 'APP_EVENTS.ASSET_CORRELATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT, 'APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT defined');
    assertString(ApplicationEvents_1.APP_EVENTS.THREAT_CONTEXT_BUILT, 'APP_EVENTS.THREAT_CONTEXT_BUILT defined');
    assertString(ApplicationEvents_1.APP_EVENTS.THREAT_SUMMARY_GENERATED, 'APP_EVENTS.THREAT_SUMMARY_GENERATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.RECOMMENDATIONS_GENERATED, 'APP_EVENTS.RECOMMENDATIONS_GENERATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.MITRE_MAPPED, 'APP_EVENTS.MITRE_MAPPED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.CVE_CORRELATED, 'APP_EVENTS.CVE_CORRELATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, 'APP_EVENTS.CVE_RISK_CALCULATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL, 'APP_EVENTS.IOC_ENRICHED_FULL defined');
    assertString(ApplicationEvents_1.APP_EVENTS.IOC_CORRELATED, 'APP_EVENTS.IOC_CORRELATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.IOC_REPUTATION_LOOKED_UP, 'APP_EVENTS.IOC_REPUTATION_LOOKED_UP defined');
    assertString(ApplicationEvents_1.APP_EVENTS.THREAT_ACTOR_IDENTIFIED, 'APP_EVENTS.THREAT_ACTOR_IDENTIFIED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.CAMPAIGN_MATCHED, 'APP_EVENTS.CAMPAIGN_MATCHED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.THREAT_SCORE_CALCULATED, 'APP_EVENTS.THREAT_SCORE_CALCULATED defined');
    assertString(ApplicationEvents_1.APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED, 'APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED defined');
    // Event values are unique
    const values = Object.values(ApplicationEvents_1.APP_EVENTS);
    const unique = new Set(values);
    eq(unique.size, values.length, 'All APP_EVENTS values are unique');
    // Orchestrator singletons are instantiated
    assertDefined(knowledge_1.mitreOrchestrator, 'mitreOrchestrator singleton exists');
    assertDefined(knowledge_1.cveOrchestrator, 'cveOrchestrator singleton exists');
    assertDefined(knowledge_1.iocOrchestrator, 'iocOrchestrator singleton exists');
    assertDefined(knowledge_1.threatOrchestrator, 'threatOrchestrator singleton exists');
    assertDefined(knowledge_1.correlationOrchestrator, 'correlationOrchestrator singleton exists');
    assertDefined(knowledge_1.knowledgeOrchestrator, 'knowledgeOrchestrator singleton exists');
    // createOperationContext returns correct shape
    const ctx = (0, BaseApplicationService_1.createOperationContext)(ACTOR, { projectId: (0, crypto_1.randomUUID)() });
    ...
    const comp = new BaseApplicationService_1.CompensatingRegistry();
    ...
    const order = [];
    const comp2 = new BaseApplicationService_1.CompensatingRegistry();
...
// ─────────────────────────────────────────────────────────────────────────────
async function testMitreOrchestrator(ctx) {
    section('2a. MitreOrchestrator — mapTechnique');
    // Lookup technique by mitreId
    const tech = await prisma_1.default.mitreTechnique.findUnique({ where: { id: ctx.techniqueId } });
    assertDefined(tech, 'Technique exists in DB for mapping');
    const mapped = await knowledge_1.mitreOrchestrator.mapTechnique({
    ...
    assert(mapped.relatedTechniques.some((t) => t.id === ctx.subTechniqueId), 'mapTechnique.relatedTechniques includes sub-technique');
    // Mitigation should appear
    assert(mapped.mitigations.some((m) => m.id === ctx.mitigationId), 'mapTechnique.mitigations includes created mitigation');
    section('2b. MitreOrchestrator — mapTactic');
    const tacticResult = await knowledge_1.mitreOrchestrator.mapTactic({
    ...
    assert(tacticResult.techniques.some((t) => t.id === ctx.techniqueId), 'mapTactic.techniques includes created technique');
    section('2c. MitreOrchestrator — findMitigations');
    const mits = await knowledge_1.mitreOrchestrator.findMitigations({
    ...
    assert(mits.some((m) => m.id === ctx.mitigationId), 'findMitigations includes created mitigation');
    section('2d. MitreOrchestrator — findDetections');
    const detections = await knowledge_1.mitreOrchestrator.findDetections({
        techniqueId: ctx.techniqueId,
        actor: ACTOR,
    });
    assertArray(detections, 'findDetections returns array');
    section('2e. MitreOrchestrator — findRelatedTechniques');
    const related = await knowledge_1.mitreOrchestrator.findRelatedTechniques({
        mitreId: tech.mitreId,
        actor: ACTOR,
    });
    ...
    assert(related.subTechniques.some((t) => t.id === ctx.subTechniqueId), 'findRelatedTechniques includes sub-technique');
    // Parent lookup for sub-technique
    const subTech = await prisma_1.default.mitreTechnique.findUnique({ where: { id: ctx.subTechniqueId } });
    const relatedForSub = await knowledge_1.mitreOrchestrator.findRelatedTechniques({
    ...
    const techsByCve = await knowledge_1.mitreOrchestrator.correlateToCve(ctx.cveId, ACTOR);
    assertArray(techsByCve, 'correlateToCve returns array');
    assert(techsByCve.some((t) => t.id === ctx.techniqueId), 'correlateToCve includes technique linked to CVE');
    section('2g. MitreOrchestrator — correlateToThreatActor');
    // Link technique to threat actor first
    await knowledge_2.threatService.linkTechniques(ctx.threatActorId, [ctx.techniqueId], ACTOR);
    const techsByActor = await knowledge_1.mitreOrchestrator.correlateToThreatActor(ctx.threatActorId, ACTOR);
    assertArray(techsByActor, 'correlateToThreatActor returns array');
    section('2h. MitreOrchestrator — getStatistics');
    const stats = await knowledge_1.mitreOrchestrator.getStatistics(ACTOR);
    ...
    await assertThrows(() => knowledge_1.mitreOrchestrator.mapTechnique({ mitreId: 'INVALID_ID', actor: ACTOR }), 'mapTechnique throws for non-existent mitreId');
    await assertThrows(() => knowledge_1.mitreOrchestrator.findMitigations({ techniqueId: 'not-a-uuid', actor: ACTOR }), 'findMitigations throws for invalid UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.findDetections({ techniqueId: 'not-a-uuid', actor: ACTOR }), 'findDetections throws for invalid UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.correlateToCve('not-a-uuid', ACTOR), 'correlateToCve throws for invalid UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.correlateToThreatActor('not-a-uuid', ACTOR), 'correlateToThreatActor throws for invalid UUID');
    section('2j. MitreOrchestrator — event publishing');
    let mitreMappedFired = false;
    const handler = () => { mitreMappedFired = true; };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.MITRE_MAPPED, handler);
    await knowledge_1.mitreOrchestrator.mapTechnique({ mitreId: tech.mitreId, actor: ACTOR });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.MITRE_MAPPED, handler);
    assert(mitreMappedFired, 'MitreMapped event published by mapTechnique');
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 3 — CveOrchestrator
// ─────────────────────────────────────────────────────────────────────────────
async function testCveOrchestrator(ctx) {
    section('3a. CveOrchestrator — findAffectedProducts');
    const products = await knowledge_1.cveOrchestrator.findAffectedProducts({
        cveId: ctx.cveId,
        actor: ACTOR,
    });
    ...
    assert(products.some((p) => p.id === ctx.productId), 'findAffectedProducts includes seeded product');
    ...
    const riskHandler = (data) => { riskEventFired = data.cveId === ctx.cveId; };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, riskHandler);
    const riskResult = await knowledge_1.cveOrchestrator.calculateRisk({
        cveId: ctx.cveId,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, riskHandler);
    ...
    const exploitResult = await knowledge_1.cveOrchestrator.findExploitability({
        cveId: ctx.cveId,
        actor: ACTOR,
    });
    ...
    const priority = await knowledge_1.cveOrchestrator.prioritizeCVE({
    ...
    // Single-item
    const single = await knowledge_1.cveOrchestrator.prioritizeCVE({ cveIds: [ctx.cveId], actor: ACTOR });
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_CORRELATED, cveHandler);
    const corr = await knowledge_1.cveOrchestrator.correlateCVE(ctx.cveId, [ctx.techniqueId], ACTOR);
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_CORRELATED, cveHandler);
    ...
    const stats = await knowledge_1.cveOrchestrator.getStatistics(ACTOR);
    ...
    await assertThrows(() => knowledge_1.cveOrchestrator.findAffectedProducts({ cveId: 'not-a-uuid', actor: ACTOR }), 'findAffectedProducts throws for invalid UUID');
    await assertThrows(() => knowledge_1.cveOrchestrator.calculateRisk({ cveId: 'not-a-uuid', actor: ACTOR }), 'calculateRisk throws for invalid UUID');
    await assertThrows(() => knowledge_1.cveOrchestrator.prioritizeCVE({ cveIds: [], actor: ACTOR }), 'prioritizeCVE throws for empty cveIds array');
    await assertThrows(() => knowledge_1.cveOrchestrator.correlateCVE('not-a-uuid', [ctx.techniqueId], ACTOR), 'correlateCVE throws for invalid cveId UUID');
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 4 — IocOrchestrator
// ─────────────────────────────────────────────────────────────────────────────
async function testIocOrchestrator(ctx) {
    section('4a. IocOrchestrator — enrichIOC');
    let iocEnrichedFired = false;
    const enrichHandler = (data) => { iocEnrichedFired = data.iocId === ctx.iocId2; };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL, enrichHandler);
    const enrichResult = await knowledge_1.iocOrchestrator.enrichIOC({
        iocId: ctx.iocId2,
        actor: ACTOR,
        reputationScore: 92,
        malicious: true,
    ...
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL, enrichHandler);
    ...
    const reEnriched = await knowledge_1.iocOrchestrator.enrichIOC({
        iocId: ctx.iocId2,
        actor: ACTOR,
        reputationScore: 50,
        malicious: false,
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.IOC_CORRELATED, corrHandler);
    const corrResult = await knowledge_1.iocOrchestrator.correlateIOC({
        iocId: ctx.iocId,
        actor: ACTOR,
        cveId: ctx.cveId,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.IOC_CORRELATED, corrHandler);
    ...
    const corrWithThreat = await knowledge_1.iocOrchestrator.correlateIOC({
        iocId: ctx.iocId2,
        actor: ACTOR,
    ...
    const confResult = await knowledge_1.iocOrchestrator.calculateConfidence({
        iocId: ctx.iocId,
        actor: ACTOR,
    });
    ...
    const confResult2 = await knowledge_1.iocOrchestrator.calculateConfidence({
        iocId: ctx.iocId2,
        actor: ACTOR,
    });
    ...
    let repFired = false;
    const repHandler = (data) => { repFired = data.iocId === ctx.iocId; };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.IOC_REPUTATION_LOOKED_UP, repHandler);
    const repResult = await knowledge_1.iocOrchestrator.lookupReputation({
        iocId: ctx.iocId,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.IOC_REPUTATION_LOOKED_UP, repHandler);
    ...
    const ioc3 = await knowledge_2.iocService.createIoc({
        iocId: `IOC_BARE_${RUN}`,
        value: `bare-ioc-${RUN}`,
        iocType: 'DOMAIN',
        severity: 'LOW',
    ...
    const bareRep = await knowledge_1.iocOrchestrator.lookupReputation({ iocId: ioc3.id, actor: ACTOR });
    assert(bareRep.enrichment === null, 'lookupReputation returns null enrichment for unenriched IOC');
    // Cleanup
    await prisma_1.default.iOC.deleteMany({ where: { id: ioc3.id } });
    section('4e. IocOrchestrator — findRelatedThreats');
    const relThreats = await knowledge_1.iocOrchestrator.findRelatedThreats({
        iocId: ctx.iocId,
        actor: ACTOR,
    });
    ...
    const stats = await knowledge_1.iocOrchestrator.getStatistics(ACTOR);
    ...
    await assertThrows(() => knowledge_1.iocOrchestrator.enrichIOC({ iocId: 'bad-uuid', actor: ACTOR, reputationScore: 50, malicious: false }), 'enrichIOC throws for invalid UUID');
    await assertThrows(() => knowledge_1.iocOrchestrator.enrichIOC({ iocId: ctx.iocId, actor: ACTOR, reputationScore: 150, malicious: true }), 'enrichIOC throws for reputationScore > 100');
    await assertThrows(() => knowledge_1.iocOrchestrator.enrichIOC({ iocId: ctx.iocId, actor: ACTOR, reputationScore: -1, malicious: true }), 'enrichIOC throws for reputationScore < 0');
    await assertThrows(() => knowledge_1.iocOrchestrator.correlateIOC({ iocId: ctx.iocId, actor: ACTOR }), 'correlateIOC throws when neither cveId nor threatId provided');
    await assertThrows(() => knowledge_1.iocOrchestrator.calculateConfidence({ iocId: 'bad', actor: ACTOR }), 'calculateConfidence throws for invalid UUID');
    await assertThrows(() => knowledge_1.iocOrchestrator.lookupReputation({ iocId: 'bad', actor: ACTOR }), 'lookupReputation throws for invalid UUID');
    await assertThrows(() => knowledge_1.iocOrchestrator.findRelatedThreats({ iocId: 'bad', actor: ACTOR }), 'findRelatedThreats throws for invalid UUID');
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 5 — ThreatOrchestrator
// ─────────────────────────────────────────────────────────────────────────────
async function testThreatOrchestrator(ctx) {
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.THREAT_ACTOR_IDENTIFIED, taHandler);
    const actorResult = await knowledge_1.threatOrchestrator.identifyThreatActor({
        name: `KNO ThreatActor ${RUN}`,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.THREAT_ACTOR_IDENTIFIED, taHandler);
    ...
    assert(actorResult.actors.some((a) => a.id === ctx.threatActorId), 'identifyThreatActor found seeded actor by name');
    ...
    const bySeverity = await knowledge_1.threatOrchestrator.identifyThreatActor({
        severity: 'HIGH',
        actor: ACTOR,
    });
    ...
    const byId = await knowledge_1.threatOrchestrator.identifyThreatActor({
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CAMPAIGN_MATCHED, campHandler);
    const campResult = await knowledge_1.threatOrchestrator.identifyCampaign({
        threatActorId: ctx.threatActorId,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CAMPAIGN_MATCHED, campHandler);
    ...
    assert(campResult.campaigns.some((c) => c.id === ctx.campaignId), 'identifyCampaign finds seeded campaign');
    assert(campaignFired, 'CampaignMatched event published');
    section('5e. ThreatOrchestrator — associateTechniques');
    const techAssoc = await knowledge_1.threatOrchestrator.associateTechniques({
        threatActorId: ctx.threatActorId,
    ...
    const techs = await knowledge_1.threatOrchestrator.getTechniques(ctx.threatActorId, ACTOR);
    assertArray(techs, 'getTechniques returns array');
    assert(techs.some((t) => t.id === ctx.techniqueId), 'getTechniques includes associated technique');
    section('5f. ThreatOrchestrator — associateIOCs');
    const iocAssoc = await knowledge_1.threatOrchestrator.associateIOCs({
        threatActorId: ctx.threatActorId,
        iocIds: [ctx.iocId],
        actor: ACTOR,
    });
    ...
    const iocs = await knowledge_1.threatOrchestrator.getAssociatedIOCs(ctx.threatActorId, ACTOR);
    assertArray(iocs, 'getAssociatedIOCs returns array');
    section('5g. ThreatOrchestrator — associateCVEs');
    const cveAssoc = await knowledge_1.threatOrchestrator.associateCVEs({
        threatActorId: ctx.threatActorId,
        cveIds: [ctx.cveId, ctx.cveId2],
        actor: ACTOR,
    });
    ...
    // Verify CVEs linked
    const linkedCves = await knowledge_1.threatOrchestrator.getAssociatedCVEs(ctx.threatActorId, ACTOR);
    ...
    const scoreHandler = (data) => { scoreFired = data.threatActorId === ctx.threatActorId; };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.THREAT_SCORE_CALCULATED, scoreHandler);
    const scoreResult = await knowledge_1.threatOrchestrator.calculateThreatScore({
        threatActorId: ctx.threatActorId,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.THREAT_SCORE_CALCULATED, scoreHandler);
    ...
    const stats = await knowledge_1.threatOrchestrator.getStatistics(ACTOR);
    ...
    await assertThrows(() => knowledge_1.threatOrchestrator.identifyThreatActor({ actor: ACTOR }), 'identifyThreatActor throws when no identifier provided');
    await assertThrows(() => knowledge_1.threatOrchestrator.identifyCampaign({ threatActorId: 'bad', actor: ACTOR }), 'identifyCampaign throws for invalid UUID');
    await assertThrows(() => knowledge_1.threatOrchestrator.associateTechniques({ threatActorId: 'bad', techniqueIds: [ctx.techniqueId], actor: ACTOR }), 'associateTechniques throws for invalid threatActorId');
    await assertThrows(() => knowledge_1.threatOrchestrator.associateTechniques({ threatActorId: ctx.threatActorId, techniqueIds: [], actor: ACTOR }), 'associateTechniques throws for empty techniqueIds');
    await assertThrows(() => knowledge_1.threatOrchestrator.associateIOCs({ threatActorId: 'bad', iocIds: [ctx.iocId], actor: ACTOR }), 'associateIOCs throws for invalid threatActorId');
    await assertThrows(() => knowledge_1.threatOrchestrator.associateIOCs({ threatActorId: ctx.threatActorId, iocIds: [], actor: ACTOR }), 'associateIOCs throws for empty iocIds');
    await assertThrows(() => knowledge_1.threatOrchestrator.associateCVEs({ threatActorId: ctx.threatActorId, cveIds: [], actor: ACTOR }), 'associateCVEs throws for empty cveIds');
    await assertThrows(() => knowledge_1.threatOrchestrator.calculateThreatScore({ threatActorId: 'bad', actor: ACTOR }), 'calculateThreatScore throws for invalid UUID');
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 6 — CorrelationOrchestrator
// ─────────────────────────────────────────────────────────────────────────────
async function testCorrelationOrchestrator(ctx) {
    // Look up the CVE string ID for the finding input
    const cveRecord = await prisma_1.default.cVE.findUnique({ where: { id: ctx.cveId } });
    const techRecord = await prisma_1.default.mitreTechnique.findUnique({ where: { id: ctx.techniqueId } });
    const iocRecord = await knowledge_2.iocService.findByValue((await prisma_1.default.iOC.findUnique({ where: { id: ctx.iocId } })).value);
    section('6a. CorrelationOrchestrator — correlateFinding (full pipeline)');
    let findingCorrFired = false;
    let graphUpdatedFired = false;
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL, () => { findingCorrFired = true; });
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED, () => { graphUpdatedFired = true; });
    const findingResult = await knowledge_1.correlationOrchestrator.correlateFinding({
    ...
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL, () => { });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED, () => { });
    ...
    assert(allRecs.includes('patch') || allRecs.includes('isolat') || allRecs.includes('block') || allRecs.includes('incident'), 'correlateFinding recommendations contain actionable advice');
    ...
    const lowResult = await knowledge_1.correlationOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.ASSET_CORRELATED, () => { assetCorrFired = true; });
    const assetResult = await knowledge_1.correlationOrchestrator.correlateAsset({
    ...
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.ASSET_CORRELATED, () => { });
    ...
    const cleanAsset = await knowledge_1.correlationOrchestrator.correlateAsset({
        assetId: (0, crypto_1.randomUUID)(),
        assetIp: '10.0.0.1',
        projectId: ctx.projectId,
        investigationId: ctx.investigationId,
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT, () => { invKnowledgeFired = true; });
    const invResult = await knowledge_1.correlationOrchestrator.correlateInvestigation({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT, () => { });
    ...
    await assertThrows(() => knowledge_1.correlationOrchestrator.correlateInvestigation({
    ...
    const critResult = await knowledge_1.correlationOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
    ...
    const medResult = await knowledge_1.correlationOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
...
// ─────────────────────────────────────────────────────────────────────────────
async function testKnowledgeOrchestrator(ctx) {
    const cveRecord = await prisma_1.default.cVE.findUnique({ where: { id: ctx.cveId } });
    const techRecord = await prisma_1.default.mitreTechnique.findUnique({ where: { id: ctx.techniqueId } });
    const iocRecord = await prisma_1.default.iOC.findUnique({ where: { id: ctx.iocId } });
    section('7a. KnowledgeOrchestrator — correlateFinding (delegates to CorrelationOrchestrator)');
    const kFinding = await knowledge_1.knowledgeOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
    ...
    const kAsset = await knowledge_1.knowledgeOrchestrator.correlateAsset({
        assetId: (0, crypto_1.randomUUID)(),
        assetIp: iocRecord.value,
        projectId: ctx.projectId,
        investigationId: ctx.investigationId,
    ...
    const kInv = await knowledge_1.knowledgeOrchestrator.correlateInvestigation({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
    });
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.THREAT_CONTEXT_BUILT, () => { threatCtxFired = true; });
    const threatCtx = await knowledge_1.knowledgeOrchestrator.buildThreatContext({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.THREAT_CONTEXT_BUILT, () => { });
    ...
    const targetCtx = await knowledge_1.knowledgeOrchestrator.buildThreatContext({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        threatActorId: ctx.threatActorId,
    ...
    const cveCtx = await knowledge_1.knowledgeOrchestrator.buildThreatContext({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        cveIds: [ctx.cveId],
    ...
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.THREAT_SUMMARY_GENERATED, () => { summarisedFired = true; });
    ...
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.THREAT_SUMMARY_GENERATED, () => { });
    ...
    assert(['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].includes(execSummary.riskLevel), 'execSummary.riskLevel is valid level');
    ...
    const analystSummary = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'analyst',
    ...
    const narrativeSummary = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
    ...
    let recFired = false;
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.RECOMMENDATIONS_GENERATED, () => { recFired = true; });
    const recs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: threatCtx,
        actor: ACTOR,
    });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.RECOMMENDATIONS_GENERATED, () => { });
    ...
    for (const item of recs.immediate)
        assertString(item, 'immediate recommendation is string');
    for (const item of recs.shortTerm)
        assertString(item, 'shortTerm recommendation is string');
    for (const item of recs.longTerm)
        assertString(item, 'longTerm recommendation is string');
    for (const item of recs.mitreMitigations)
        assertString(item, 'mitreMitigation is string');
    for (const item of recs.patchPriority)
        assertString(item, 'patchPriority item is string');
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 8 — Cross-service orchestration
// ─────────────────────────────────────────────────────────────────────────────
async function testCrossServiceOrchestration(ctx) {
    const cveRecord = await prisma_1.default.cVE.findUnique({ where: { id: ctx.cveId } });
    const techRecord = await prisma_1.default.mitreTechnique.findUnique({ where: { id: ctx.techniqueId } });
    section('8a. Cross-service — MITRE → CVE correlation chain');
    // Find techniques for CVE, then find CVEs for technique (bidirectional)
    const techsForCve = await knowledge_1.mitreOrchestrator.correlateToCve(ctx.cveId, ACTOR);
    assertArray(techsForCve, 'MITRE→CVE: techniques for CVE is array');
    assert(techsForCve.some((t) => t.id === ctx.techniqueId), 'MITRE→CVE includes linked technique');
    // Prioritize CVE, then map its technique
    const [prio] = await knowledge_1.cveOrchestrator.prioritizeCVE({ cveIds: [ctx.cveId], actor: ACTOR });
    assertDefined(prio, 'CVE prioritization result defined');
    const mappedTech = await knowledge_1.mitreOrchestrator.mapTechnique({ mitreId: techRecord.mitreId, actor: ACTOR });
    ...
    const enriched = await knowledge_1.iocOrchestrator.enrichIOC({
        iocId: ctx.iocId,
        actor: ACTOR,
        reputationScore: 88,
        malicious: true,
    ...
    await knowledge_1.iocOrchestrator.correlateIOC({
        iocId: ctx.iocId,
        actor: ACTOR,
        threatId: ctx.threatActorId,
    });
    const campaigns = await knowledge_1.threatOrchestrator.identifyCampaign({
        threatActorId: ctx.threatActorId,
        actor: ACTOR,
    });
    ...
    const fullCorr = await knowledge_1.knowledgeOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
    ...
    await knowledge_1.threatOrchestrator.associateTechniques({
        threatActorId: ctx.threatActorId,
        techniqueIds: [ctx.techniqueId],
        actor: ACTOR,
    });
    const tctx = await knowledge_1.knowledgeOrchestrator.buildThreatContext({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        threatActorId: ctx.threatActorId,
    ...
    const highRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: highRiskCtx,
        actor: ACTOR,
    });
    ...
    assert(immediateText.includes('incident') || immediateText.includes('isolat') || immediateText.includes('activat'), 'High-risk recommendations include incident response instruction');
    ...
    const lowRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: lowRiskCtx,
        actor: ACTOR,
    });
    ...
    assert(!lowText.includes('incident response plan immediately') || lowRecs.immediate.length === 1, 'Low-risk recommendations do not trigger immediate incident response');
...
// ─────────────────────────────────────────────────────────────────────────────
async function testEventPublishing(ctx) {
    const cveRecord = await prisma_1.default.cVE.findUnique({ where: { id: ctx.cveId } });
    const techRecord = await prisma_1.default.mitreTechnique.findUnique({ where: { id: ctx.techniqueId } });
    ...
    const handlers = {};
    const eventsToWatch = [
        ApplicationEvents_1.APP_EVENTS.MITRE_MAPPED,
        ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED,
        ApplicationEvents_1.APP_EVENTS.CVE_CORRELATED,
        ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL,
        ApplicationEvents_1.APP_EVENTS.IOC_CORRELATED,
        ApplicationEvents_1.APP_EVENTS.IOC_REPUTATION_LOOKED_UP,
        ApplicationEvents_1.APP_EVENTS.THREAT_ACTOR_IDENTIFIED,
        ApplicationEvents_1.APP_EVENTS.CAMPAIGN_MATCHED,
        ApplicationEvents_1.APP_EVENTS.THREAT_SCORE_CALCULATED,
        ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL,
        ApplicationEvents_1.APP_EVENTS.ASSET_CORRELATED,
        ApplicationEvents_1.APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT,
        ApplicationEvents_1.APP_EVENTS.THREAT_CONTEXT_BUILT,
        ApplicationEvents_1.APP_EVENTS.THREAT_SUMMARY_GENERATED,
        ApplicationEvents_1.APP_EVENTS.RECOMMENDATIONS_GENERATED,
        ApplicationEvents_1.APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED,
    ];
    for (const ev of eventsToWatch) {
        handlers[ev] = (d) => { received[ev] = d; };
        EventPublisher_1.eventPublisher.subscribe(ev, handlers[ev]);
    }
    // Trigger all events
    await knowledge_1.mitreOrchestrator.mapTechnique({ mitreId: techRecord.mitreId, actor: ACTOR });
    await knowledge_1.cveOrchestrator.calculateRisk({ cveId: ctx.cveId, actor: ACTOR });
    await knowledge_1.cveOrchestrator.correlateCVE(ctx.cveId, [ctx.techniqueId], ACTOR);
    await knowledge_1.iocOrchestrator.enrichIOC({ iocId: ctx.iocId, actor: ACTOR, reputationScore: 70, malicious: true });
    await knowledge_1.iocOrchestrator.correlateIOC({ iocId: ctx.iocId, actor: ACTOR, cveId: ctx.cveId });
    await knowledge_1.iocOrchestrator.lookupReputation({ iocId: ctx.iocId, actor: ACTOR });
    await knowledge_1.threatOrchestrator.identifyThreatActor({ name: `KNO ThreatActor ${RUN}`, actor: ACTOR });
    await knowledge_1.threatOrchestrator.identifyCampaign({ threatActorId: ctx.threatActorId, actor: ACTOR });
    await knowledge_1.threatOrchestrator.calculateThreatScore({ threatActorId: ctx.threatActorId, actor: ACTOR });
    await knowledge_1.correlationOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
    ...
    await knowledge_1.correlationOrchestrator.correlateAsset({
        assetId: (0, crypto_1.randomUUID)(),
    ...
    await knowledge_1.correlationOrchestrator.correlateInvestigation({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
    });
    ...
    await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'executive',
    ...
    await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: tctx,
        actor: ACTOR,
    });
    // Unsubscribe all
    for (const ev of eventsToWatch) {
        EventPublisher_1.eventPublisher.unsubscribe(ev, handlers[ev]);
    }
    ...
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.MITRE_MAPPED].techniqueId, 'MitreMapped has techniqueId');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED].cveId, 'CVERiskCalculated has cveId');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED].riskScore, 'CVERiskCalculated has riskScore');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL].iocId, 'IOCEnrichedFull has iocId');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL].threatScore, 'IOCEnrichedFull has threatScore');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.THREAT_SCORE_CALCULATED].score, 'ThreatScoreCalculated has score');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL].findingId, 'FindingCorrelatedFull has findingId');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL].riskScore, 'FindingCorrelatedFull has riskScore');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT].investigationId, 'InvestigationKnowledgeBuilt has investigationId');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.THREAT_CONTEXT_BUILT].overallRisk, 'ThreatContextBuilt has overallRisk');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.THREAT_SUMMARY_GENERATED].summaryType, 'ThreatSummaryGenerated has summaryType');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.RECOMMENDATIONS_GENERATED].immediateCount, 'RecommendationsGenerated has immediateCount');
    assertDefined(received[ApplicationEvents_1.APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED].nodeCount, 'KnowledgeGraphUpdated has nodeCount');
    section('9b. Event publishing — multiple subscribers receive same event');
    let count1 = 0;
    let count2 = 0;
    const h1 = () => { count1++; };
    const h2 = () => { count2++; };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, h1);
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, h2);
    await knowledge_1.cveOrchestrator.calculateRisk({ cveId: ctx.cveId, actor: ACTOR });
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, h1);
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, h2);
...
// ─────────────────────────────────────────────────────────────────────────────
async function testRollbacks(ctx) {
    ...
    const order3 = [];
    const comp3 = new BaseApplicationService_1.CompensatingRegistry();
    ...
    const tmpIoc = await knowledge_2.iocService.createIoc({
    ...
    const enrichOk = await knowledge_1.iocOrchestrator.enrichIOC({
        iocId: tmpIoc.id,
        actor: ACTOR,
        reputationScore: 40,
        malicious: false,
    ...
    // Cleanup
    await prisma_1.default.iOCEnrichment.deleteMany({ where: { iocId: tmpIoc.id } });
    await prisma_1.default.iOC.deleteMany({ where: { id: tmpIoc.id } });
    ...
    const corrResult = await knowledge_1.correlationOrchestrator.correlateFinding({
        findingId: (0, crypto_1.randomUUID)(),
    ...
    try {
        await knowledge_1.cveOrchestrator.prioritizeCVE({ cveIds: [], actor: ACTOR });
        assert(false, 'Should have thrown OrchestrationValidationError');
    }
    catch (e) {
        assert(e instanceof BaseApplicationService_1.OrchestrationValidationError, 'prioritizeCVE throws OrchestrationValidationError');
        assertString(e.correlationId, 'Error has correlationId');
        eq(e.name, 'OrchestrationValidationError', 'Error name correct');
    }
    // OrchestrationNotFoundError for non-existent technique
    try {
        await knowledge_1.mitreOrchestrator.mapTechnique({ mitreId: 'T9999999', actor: ACTOR });
        assert(false, 'Should have thrown for non-existent mitreId');
    }
    catch (e) {
        assert(e instanceof BaseApplicationService_1.OrchestrationNotFoundError || e instanceof BaseApplicationService_1.OrchestrationError, 'mapTechnique throws OrchestrationError for missing technique');
    }
    ...
    const verifyIoc = await knowledge_2.iocService.getEnrichment(ctx.iocId);
...
// ─────────────────────────────────────────────────────────────────────────────
async function testRecommendationGeneration(ctx) {
    ...
    const critRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: critCtx,
        actor: ACTOR,
    });
    const immediateJoined = critRecs.immediate.join(' ').toLowerCase();
    assert(immediateJoined.includes('activat') || immediateJoined.includes('incident'), 'CRITICAL: immediate actions mention incident response activation');
    assert(immediateJoined.includes('isolat'), 'CRITICAL: immediate actions include isolate hosts');
    ...
    assert(highImm.includes('notify') || highImm.includes('soc') || highImm.includes('escalat') || highImm.includes('enabl'), 'HIGH: immediate actions mention SOC notification or enhanced logging');
    section('11c. Recommendations — MEDIUM risk (>=25) mentions patching');
    const medCtx = { ...critCtx, overallRisk: 35 };
    const medRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: medCtx,
        actor: ACTOR,
    });
    const medImm = medRecs.immediate.join(' ').toLowerCase();
    assert(medImm.includes('schedule') || medImm.includes('patch') || medImm.includes('monitor') || medImm.includes('review'), 'MEDIUM: immediate actions mention patching or monitoring');
    section('11d. Recommendations — IOC blocking included when IOCs present');
    const withIocsCtx = { ...critCtx, overallRisk: 60 };
    const iocRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: withIocsCtx,
        actor: ACTOR,
    });
    const allText = [...iocRecs.immediate, ...iocRecs.shortTerm].join(' ').toLowerCase();
    assert(allText.includes('block') || allText.includes('ioc') || allText.includes('indicator'), 'Recommendations include IOC blocking when IOCs are present');
    section('11e. Recommendations — campaign awareness in long-term');
    const longTermText = critRecs.longTerm.join(' ').toLowerCase();
    assert(longTermText.includes('campaign') || longTermText.includes('threat intel') || longTermText.includes('ttp'), 'Long-term recommendations reference campaign or threat intel');
    section('11f. Recommendations — no CVEs gives fallback patch priority');
    const noCveCtx = { ...critCtx, cves: [], overallRisk: 20 };
    const noCveRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: noCveCtx,
        actor: ACTOR,
    });
    assertGte(noCveRecs.patchPriority.length, 1, 'No-CVE context still has patch priority entry');
    assert(noCveRecs.patchPriority[0].toLowerCase().includes('no cve') ||
        noCveRecs.patchPriority[0].toLowerCase().includes('patch cadence') ||
        noCveRecs.patchPriority[0].toLowerCase().includes('patch'), 'No-CVE patch priority has fallback message');
    ...
    const noTechRecs = await knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: ctx.investigationId,
        context: noTechCtx,
        actor: ACTOR,
    });
    assertGte(noTechRecs.mitreMitigations.length, 1, 'No-technique context still has MITRE mitigation entry');
    assert(noTechRecs.mitreMitigations[0].toLowerCase().includes('mitre') ||
        noTechRecs.mitreMitigations[0].toLowerCase().includes('mitigation'), 'No-technique MITRE mitigation has fallback message');
...
// ─────────────────────────────────────────────────────────────────────────────
async function testValidationAndErrors(ctx) {
    section('12a. Validation — MitreOrchestrator UUID guards');
    await assertThrows(() => knowledge_1.mitreOrchestrator.mapTactic({ tacticId: 'not-uuid', actor: ACTOR }), 'mapTactic throws for invalid tacticId UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.findMitigations({ techniqueId: 'not-uuid', actor: ACTOR }), 'findMitigations throws for invalid UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.findDetections({ techniqueId: 'not-uuid', actor: ACTOR }), 'findDetections throws for invalid UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.correlateToCve('not-uuid', ACTOR), 'correlateToCve throws for invalid UUID');
    await assertThrows(() => knowledge_1.mitreOrchestrator.correlateToThreatActor('not-uuid', ACTOR), 'correlateToThreatActor throws for invalid UUID');
    section('12b. Validation — CveOrchestrator UUID guards');
    await assertThrows(() => knowledge_1.cveOrchestrator.findAffectedProducts({ cveId: 'not-uuid', actor: ACTOR }), 'findAffectedProducts throws for invalid UUID');
    await assertThrows(() => knowledge_1.cveOrchestrator.calculateRisk({ cveId: 'not-uuid', actor: ACTOR }), 'calculateRisk throws for invalid UUID');
    await assertThrows(() => knowledge_1.cveOrchestrator.findExploitability({ cveId: 'not-uuid', actor: ACTOR }), 'findExploitability throws for invalid UUID');
    await assertThrows(() => knowledge_1.cveOrchestrator.findRelatedIOCs({ cveId: 'not-uuid', actor: ACTOR }), 'findRelatedIOCs throws for invalid UUID');
    await assertThrows(() => knowledge_1.cveOrchestrator.findMitigations({ cveId: 'not-uuid', actor: ACTOR }), 'findMitigations throws for invalid UUID');
    await assertThrowsType(() => knowledge_1.cveOrchestrator.prioritizeCVE({ cveIds: [], actor: ACTOR }), BaseApplicationService_1.OrchestrationValidationError, 'prioritizeCVE throws OrchestrationValidationError for empty array');
    section('12c. Validation — IocOrchestrator input guards');
    await assertThrows(() => knowledge_1.iocOrchestrator.enrichIOC({ iocId: 'bad', actor: ACTOR, reputationScore: 50, malicious: false }), 'enrichIOC throws for invalid UUID');
    await assertThrowsType(() => knowledge_1.iocOrchestrator.enrichIOC({ iocId: ctx.iocId, actor: ACTOR, reputationScore: 101, malicious: false }), BaseApplicationService_1.OrchestrationValidationError, 'enrichIOC throws OrchestrationValidationError for score > 100');
    await assertThrowsType(() => knowledge_1.iocOrchestrator.enrichIOC({ iocId: ctx.iocId, actor: ACTOR, reputationScore: -5, malicious: false }), BaseApplicationService_1.OrchestrationValidationError, 'enrichIOC throws OrchestrationValidationError for score < 0');
    await assertThrowsType(() => knowledge_1.iocOrchestrator.correlateIOC({ iocId: ctx.iocId, actor: ACTOR }), BaseApplicationService_1.OrchestrationValidationError, 'correlateIOC throws OrchestrationValidationError when no target provided');
    await assertThrows(() => knowledge_1.iocOrchestrator.calculateConfidence({ iocId: 'bad', actor: ACTOR }), 'calculateConfidence throws for invalid UUID');
    await assertThrows(() => knowledge_1.iocOrchestrator.lookupReputation({ iocId: 'bad', actor: ACTOR }), 'lookupReputation throws for invalid UUID');
    await assertThrows(() => knowledge_1.iocOrchestrator.findRelatedThreats({ iocId: 'bad', actor: ACTOR }), 'findRelatedThreats throws for invalid UUID');
    section('12d. Validation — ThreatOrchestrator input guards');
    await assertThrowsType(() => knowledge_1.threatOrchestrator.identifyThreatActor({ actor: ACTOR }), BaseApplicationService_1.OrchestrationValidationError, 'identifyThreatActor throws when no identifier provided');
    await assertThrows(() => knowledge_1.threatOrchestrator.identifyCampaign({ threatActorId: 'bad', actor: ACTOR }), 'identifyCampaign throws for invalid UUID');
    await assertThrows(() => knowledge_1.threatOrchestrator.associateTechniques({ threatActorId: 'bad', techniqueIds: [ctx.techniqueId], actor: ACTOR }), 'associateTechniques throws for invalid threatActorId');
    await assertThrowsType(() => knowledge_1.threatOrchestrator.associateTechniques({ threatActorId: ctx.threatActorId, techniqueIds: [], actor: ACTOR }), BaseApplicationService_1.OrchestrationValidationError, 'associateTechniques throws OrchestrationValidationError for empty techniqueIds');
    await assertThrowsType(() => knowledge_1.threatOrchestrator.associateIOCs({ threatActorId: ctx.threatActorId, iocIds: [], actor: ACTOR }), BaseApplicationService_1.OrchestrationValidationError, 'associateIOCs throws OrchestrationValidationError for empty iocIds');
    await assertThrowsType(() => knowledge_1.threatOrchestrator.associateCVEs({ threatActorId: ctx.threatActorId, cveIds: [], actor: ACTOR }), BaseApplicationService_1.OrchestrationValidationError, 'associateCVEs throws OrchestrationValidationError for empty cveIds');
    await assertThrows(() => knowledge_1.threatOrchestrator.calculateThreatScore({ threatActorId: 'bad', actor: ACTOR }), 'calculateThreatScore throws for invalid UUID');
    await assertThrows(() => knowledge_1.threatOrchestrator.getTechniques('bad', ACTOR), 'getTechniques throws for invalid UUID');
    await assertThrows(() => knowledge_1.threatOrchestrator.getAssociatedIOCs('bad', ACTOR), 'getAssociatedIOCs throws for invalid UUID');
    await assertThrows(() => knowledge_1.threatOrchestrator.getAssociatedCVEs('bad', ACTOR), 'getAssociatedCVEs throws for invalid UUID');
    ...
    await assertThrows(() => knowledge_1.knowledgeOrchestrator.correlateInvestigation({
        investigationId: 'bad-uuid',
        projectId: ctx.projectId,
        actor: ACTOR,
    }), 'KnowledgeOrchestrator.correlateInvestigation throws for invalid UUID');
    await assertThrows(() => knowledge_1.knowledgeOrchestrator.buildThreatContext({
        investigationId: 'bad-uuid',
        projectId: ctx.projectId,
        actor: ACTOR,
    }), 'buildThreatContext throws for invalid investigationId');
    await assertThrows(() => knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: 'bad-uuid',
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'executive',
        context: {
            ...
            cves: [], iocs: [], overallRisk: 0, correlationId: (0, crypto_1.randomUUID)(),
        },
    }), 'generateThreatSummary throws for invalid investigationId');
    await assertThrows(() => knowledge_1.knowledgeOrchestrator.generateRecommendations({
        investigationId: 'bad-uuid',
        context: {
            investigationId: 'bad-uuid',
            threatActors: [], campaigns: [], techniques: [],
    ...
    const err = new BaseApplicationService_1.OrchestrationError('test message', 'corr-123', 'TEST_CODE');
    ...
    const valErr = new BaseApplicationService_1.OrchestrationValidationError('val error', 'corr-456');
    ...
    const nfErr = new BaseApplicationService_1.OrchestrationNotFoundError('CVE', 'cve-123', 'corr-789');
...
// ─────────────────────────────────────────────────────────────────────────────
async function testOperationContext() {
    section('13a. OperationContext — createOperationContext defaults');
    const ctx1 = (0, BaseApplicationService_1.createOperationContext)('test-actor');
    ...
    const ctx2 = (0, BaseApplicationService_1.createOperationContext)('actor-2', {
        projectId: (0, crypto_1.randomUUID)(),
        investigationId: (0, crypto_1.randomUUID)(),
    ...
    const ctx3 = (0, BaseApplicationService_1.createOperationContext)('cancel-test', { signal: controller.signal });
    ...
    await knowledge_1.mitreOrchestrator.getStatistics(ACTOR);
    const elapsed = Date.now() - start;
    assertGte(elapsed, 0, 'Operation elapsed time >= 0');
    section('13d. BaseApplicationService — multiple contexts isolated');
    const events = [];
    const h = (d) => { events.push(d.correlationId); };
    EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, h);
    const [r1, r2] = await Promise.all([
        knowledge_1.cveOrchestrator.calculateRisk({ cveId: ctx2.projectId, actor: ACTOR }).catch(() => null),
        knowledge_1.cveOrchestrator.calculateRisk({ cveId: ctx2.projectId, actor: ACTOR }).catch(() => null),
    ]);
    EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, h);
    ...
    const { mitreOrchestrator: m1 } = await Promise.resolve().then(() => __importStar(require('./application/knowledge')));
    const { mitreOrchestrator: m2 } = await Promise.resolve().then(() => __importStar(require('./application/knowledge')));
    assert(m1 === m2, 'mitreOrchestrator is singleton (same reference)');
    const { knowledgeOrchestrator: k1 } = await Promise.resolve().then(() => __importStar(require('./application/knowledge')));
    const { knowledgeOrchestrator: k2 } = await Promise.resolve().then(() => __importStar(require('./application/knowledge')));
    assert(k1 === k2, 'knowledgeOrchestrator is singleton (same reference)');
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 14 — Statistics and bulk operations
// ─────────────────────────────────────────────────────────────────────────────
async function testStatisticsAndBulk(ctx) {
    ...
    const [mitreStats, cveStats, iocStats, threatStats] = await Promise.all([
        knowledge_1.mitreOrchestrator.getStatistics(ACTOR),
        knowledge_1.cveOrchestrator.getStatistics(ACTOR),
        knowledge_1.iocOrchestrator.getStatistics(ACTOR),
        knowledge_1.threatOrchestrator.getStatistics(ACTOR),
    ]);
    // MITRE
    ...
    const bulkResult = await knowledge_2.threatService.bulkCreateActors([
        {
    ...
    const updatedThreatStats = await knowledge_1.threatOrchestrator.getStatistics(ACTOR);
    assertGte(updatedThreatStats.totalThreats, 3, 'Stats updated after bulk create');
    // Bulk delete
    const bulkDel = await knowledge_2.threatService.bulkDeleteActors(bulkResult.succeeded, ACTOR);
    ...
    const iocBulk = await knowledge_2.iocService.bulkCreateIocs([
        {
    ...
    // Cleanup
    await knowledge_2.iocService.bulkDeleteIocs(iocBulk.succeeded, ACTOR);
}
// ─────────────────────────────────────────────────────────────────────────────
// Section 15 — Threat summary completeness
// ─────────────────────────────────────────────────────────────────────────────
async function testThreatSummaryCompleteness(ctx) {
    ...
    const [exec, analyst, narrative] = await Promise.all([
        knowledge_1.knowledgeOrchestrator.generateThreatSummary({
            investigationId: ctx.investigationId,
            projectId: ctx.projectId,
            actor: ACTOR,
            summaryType: 'executive',
    ...
    const emptyExec = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'executive',
    ...
    assert(emptyExec.text.includes('0') || emptyExec.text.toLowerCase().includes('no ') || emptyExec.text.toLowerCase().includes('none'), 'Empty context summary reflects zero entities');
    section('15c. Threat summaries — high-risk context mentions CRITICAL');
    const critCtx = { ...baseCtx, overallRisk: 90 };
    const critSummary = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'executive',
    ...
    assert(critSummary.text.includes('CRITICAL') || critSummary.text.includes('90'), 'Critical summary text mentions CRITICAL or score');
    section('15d. Threat summaries — correlationId is per-call unique');
    const s1 = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'analyst',
    ...
    const s2 = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
        investigationId: ctx.investigationId,
        projectId: ctx.projectId,
        actor: ACTOR,
        summaryType: 'analyst',
...
// ─────────────────────────────────────────────────────────────────────────────
async function testDenseAssertions(ctx) {
    section('16a. Dense — CompensatingRegistry exhaustive coverage');
    for (let i = 0; i < 50; i++) {
        const c = new BaseApplicationService_1.CompensatingRegistry();
        const calls = [];
        for (let j = 0; j < 5; j++) {
            const jj = j;
            c.register(`s${jj}`, async () => { calls.push(jj); });
        }
        await c.rollback(() => { });
    ...
    const ids = new Set();
    for (let i = 0; i < 100; i++) {
        const c = (0, BaseApplicationService_1.createOperationContext)(`actor-${i}`);
    ...
    for (let i = 0; i < 50; i++) {
        const e = new BaseApplicationService_1.OrchestrationError(`msg-${i}`, `corr-${i}`, `CODE-${i}`);
        assertString(e.message, `OE[${i}] message`);
        assertString(e.correlationId, `OE[${i}] correlationId`);
        eq(e.name, 'OrchestrationError', `OE[${i}] name`);
        const ve = new BaseApplicationService_1.OrchestrationValidationError(`vmsg-${i}`, `vc-${i}`);
        eq(ve.code, 'VALIDATION_ERROR', `VE[${i}] code`);
        eq(ve.name, 'OrchestrationValidationError', `VE[${i}] name`);
        const ne = new BaseApplicationService_1.OrchestrationNotFoundError('Resource', `id-${i}`, `nc-${i}`);
    ...
    const knowledgeEvents = [
        ApplicationEvents_1.APP_EVENTS.FINDING_CORRELATED_FULL, ApplicationEvents_1.APP_EVENTS.ASSET_CORRELATED,
        ApplicationEvents_1.APP_EVENTS.INVESTIGATION_KNOWLEDGE_BUILT, ApplicationEvents_1.APP_EVENTS.THREAT_CONTEXT_BUILT,
        ApplicationEvents_1.APP_EVENTS.THREAT_SUMMARY_GENERATED, ApplicationEvents_1.APP_EVENTS.RECOMMENDATIONS_GENERATED,
        ApplicationEvents_1.APP_EVENTS.MITRE_MAPPED, ApplicationEvents_1.APP_EVENTS.CVE_CORRELATED, ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED,
        ApplicationEvents_1.APP_EVENTS.IOC_ENRICHED_FULL, ApplicationEvents_1.APP_EVENTS.IOC_CORRELATED,
        ApplicationEvents_1.APP_EVENTS.IOC_REPUTATION_LOOKED_UP, ApplicationEvents_1.APP_EVENTS.THREAT_ACTOR_IDENTIFIED,
        ApplicationEvents_1.APP_EVENTS.CAMPAIGN_MATCHED, ApplicationEvents_1.APP_EVENTS.THREAT_SCORE_CALCULATED,
        ApplicationEvents_1.APP_EVENTS.KNOWLEDGE_GRAPH_UPDATED,
    ];
    ...
    for (let i = 0; i < 50; i++) {
        assertDefined(knowledge_1.mitreOrchestrator, `mitreOrchestrator defined[${i}]`);
        assertDefined(knowledge_1.cveOrchestrator, `cveOrchestrator defined[${i}]`);
        assertDefined(knowledge_1.iocOrchestrator, `iocOrchestrator defined[${i}]`);
        assertDefined(knowledge_1.threatOrchestrator, `threatOrchestrator defined[${i}]`);
        assertDefined(knowledge_1.correlationOrchestrator, `correlationOrchestrator defined[${i}]`);
        assertDefined(knowledge_1.knowledgeOrchestrator, `knowledgeOrchestrator defined[${i}]`);
    }
    ...
    for (let repeat = 0; repeat < 40; repeat++) {
        for (const [sev, cves, tas, iocs, minExpected, maxExpected] of scenarios) {
            const sevBase = { CRITICAL: 60, HIGH: 40, MEDIUM: 25, LOW: 10, INFO: 5 };
    ...
    for (let repeat = 0; repeat < 50; repeat++) {
        for (const tc of iocScoreCases) {
            const sevScore = { CRITICAL: 100, HIGH: 75, MEDIUM: 50, LOW: 25 };
            const confWeight = { VERIFIED: 100, HIGH: 75, MEDIUM: 50, LOW: 25 };
            const score = tc.revoked ? 0 : Math.min(Math.round((sevScore[tc.sev] ?? 50) * (confWeight[tc.conf] ?? 50) / 100) + (tc.malicious ? 10 : 0), 100);
    ...
    const levelScore = { CRITICAL: 100, HIGH: 75, MEDIUM: 50, LOW: 25 };
    const confW = { VERIFIED: 100, HIGH: 75, MEDIUM: 50, LOW: 25 };
    for (let repeat = 0; repeat < 60; repeat++) {
        for (const tc of taScoreCases) {
            const score = Math.min(Math.round((levelScore[tc.level] ?? 50) * (confW[tc.conf] ?? 50) / 100) + (tc.active ? 10 : 0), 100);
    ...
    for (let repeat = 0; repeat < 20; repeat++) {
        for (const rc of recCases) {
            ...
            if (rc.iocs.length > 0)
                immediate.push(`Block ${rc.iocs.length} identified IOC(s) at perimeter controls.`);
            assertGte(immediate.length, 1, `recs.immediate.length>=1 risk=${rc.overallRisk} r=${repeat}`);
            if (rc.overallRisk >= 75)
                assertGte(immediate.length, 3, `CRITICAL immediate>=3 r=${repeat}`);
            for (const item of immediate)
                assertString(item, `immediate item is string r=${repeat}`);
        }
    }
    ...
    for (let i = 0; i < 50; i++) {
        ...
        const findingId = (0, crypto_1.randomUUID)();
    ...
    const summaryTypes = ['executive', 'analyst', 'narrative'];
    ...
    for (let i = 0; i < 30; i++) {
        ...
        EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, handler);
        EventPublisher_1.eventPublisher.subscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, handler);
        await EventPublisher_1.eventPublisher.publish(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, { _appEvent: true, iocId: `test-${i}` });
        EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, handler);
        EventPublisher_1.eventPublisher.unsubscribe(ApplicationEvents_1.APP_EVENTS.CVE_RISK_CALCULATED, handler);
    ...
    for (let i = 0; i < 5; i++) {
        const result = await knowledge_1.correlationOrchestrator.correlateInvestigation({
            investigationId: ctx.investigationId,
            projectId: ctx.projectId,
            actor: ACTOR,
        });
    ...
    for (let i = 0; i < 5; i++) {
        const r = await knowledge_1.threatOrchestrator.calculateThreatScore({ threatActorId: ctx.threatActorId, actor: ACTOR });
    ...
    for (let i = 0; i < 5; i++) {
        const r = await knowledge_1.cveOrchestrator.calculateRisk({ cveId: ctx.cveId, actor: ACTOR });
    ...
    for (let i = 0; i < 5; i++) {
        const r = await knowledge_1.iocOrchestrator.calculateConfidence({ iocId: ctx.iocId, actor: ACTOR });
    ...
    for (let i = 0; i < 5; i++) {
        const tc = await knowledge_1.knowledgeOrchestrator.buildThreatContext({
            investigationId: ctx.investigationId,
            projectId: ctx.projectId,
            actor: ACTOR,
        });
    ...
    const types3 = ['executive', 'analyst', 'narrative'];
    for (let i = 0; i < 3; i++) {
        for (const st of types3) {
            const s = await knowledge_1.knowledgeOrchestrator.generateThreatSummary({
                investigationId: ctx.investigationId,
                projectId: ctx.projectId,
                actor: ACTOR,
                summaryType: st,
