...
async function main() {
    ...
    try {
        ...
        const fetchedRole = await core_1.roleRepository.findById(testRole.id);
        assert(!!fetchedRole, 'Role findById works');
        // C. Permission CRUD
        testPermission = await core_1.permissionRepository.create({
            name: `test.perm.${RUN}`,
    ...
    try {
        ...
        const byEmail = await core_1.userRepository.findByEmail(testUser.email);
        assert(byEmail?.id === testUser.id, 'UserRepository.findByEmail resolves correctly');
        const byUsername = await core_1.userRepository.findByUsername(testUser.username);
        assert(byUsername?.id === testUser.id, 'UserRepository.findByUsername resolves correctly');
        const activeUsers = await core_1.userRepository.findActiveUsers();
        ...
        await core_1.userRoleRepository.assignRole(testUser.id, testRole.id);
        const withRoles = await core_1.userRepository.findWithRoles(testUser.id);
        ...
        const withProjects = await core_1.userRepository.findWithProjects(testUser.id);
        ...
        const roleByName = await core_1.roleRepository.findByName(testRole.name);
        assert(roleByName?.id === testRole.id, 'RoleRepository.findByName resolves correctly');
        // Assign permission first
        await core_1.rolePermissionRepository.assignPermission(testRole.id, testPermission.id);
        const roleWithPerms = await core_1.roleRepository.findWithPermissions(testRole.id);
        ...
        const systemRoles = await core_1.roleRepository.findSystemRoles();
        ...
        const permsByResource = await core_1.permissionRepository.findByResource(testPermission.resource);
        assert(permsByResource.some(p => p.id === testPermission.id), 'PermissionRepository.findByResource resolves correctly');
        const permsByAction = await core_1.permissionRepository.findByAction(testPermission.action);
        assert(permsByAction.some(p => p.id === testPermission.id), 'PermissionRepository.findByAction resolves correctly');
        const permByResAndAct = await core_1.permissionRepository.findByResourceAndAction(testPermission.resource, testPermission.action);
        ...
        const slugDirect = await core_1.projectRepository.findBySlug(`Project-${RUN}`);
        ...
        const slugHyphen = await core_1.projectRepository.findBySlug(`project-${RUN}`);
        ...
        const projectWithMeta = await core_1.projectRepository.create({
            ownerId: testUser.id,
            name: `Special Project ${RUN}`,
            metadata: { slug: `special-slug-${RUN}` }
        });
        const slugMeta = await core_1.projectRepository.findBySlug(`special-slug-${RUN}`);
        assert(slugMeta?.id === projectWithMeta.id, 'ProjectRepository.findBySlug matches JSON metadata path');
        // clean up projectWithMeta
        await core_1.projectRepository.delete(projectWithMeta.id);
        const byOwner = await core_1.projectRepository.findByOwner(testUser.id);
        assert(byOwner.some(p => p.id === testProject.id), 'ProjectRepository.findByOwner resolves correctly');
        const withInvs = await core_1.projectRepository.findWithInvestigations(testProject.id);
        ...
        const activeProjects = await core_1.projectRepository.findActiveProjects();
        ...
        const invByProj = await core_1.investigationRepository.findByProject(testProject.id);
        assert(invByProj.some(i => i.id === testInvestigation.id), 'InvestigationRepository.findByProject resolves correctly');
        const invByStatus = await core_1.investigationRepository.findByStatus(testInvestigation.status);
        assert(invByStatus.some(i => i.id === testInvestigation.id), 'InvestigationRepository.findByStatus resolves correctly');
        const openInvs = await core_1.investigationRepository.findOpen();
        ...
        const testAsset = await prisma_1.default.asset.create({
            data: {
                projectId: testProject.id,
        ...
        const testFinding = await prisma_1.default.finding.create({
            data: {
                projectId: testProject.id,
                investigationId: testInvestigation.id,
                title: `Finding-${RUN}`,
        ...
        const withAssets = await core_1.investigationRepository.findWithAssets(testInvestigation.id);
        ...
        const withFindings = await core_1.investigationRepository.findWithFindings(testInvestigation.id);
        ...
        await prisma_1.default.asset.delete({ where: { id: testAsset.id } });
        await prisma_1.default.finding.delete({ where: { id: testFinding.id } });
        // Complete check (resolved/closed)
        const completeInvsBefore = await core_1.investigationRepository.findComplete();
        assert(!completeInvsBefore.some(i => i.id === testInvestigation.id), 'Investigation is not complete yet');
        // Update status to RESOLVED
        await core_1.investigationRepository.update(testInvestigation.id, { status: 'RESOLVED' });
        const completeInvsAfter = await core_1.investigationRepository.findComplete();
        ...
        await core_1.investigationRepository.update(testInvestigation.id, { status: 'OPEN' });
    }
    catch (e) {
    ...
    try {
        // UserRole Repository
        const uRolesBefore = await core_1.userRoleRepository.getUserRoles(testUser.id);
        ...
        const assignedAgain = await core_1.userRoleRepository.assignRole(testUser.id, testRole.id);
        assert(assignedAgain.userId === testUser.id && assignedAgain.roleId === testRole.id, 'UserRoleRepository.assignRole returns mapping');
        // Remove role
        await core_1.userRoleRepository.removeRole(testUser.id, testRole.id);
        const uRolesAfterRemove = await core_1.userRoleRepository.getUserRoles(testUser.id);
        ...
        await core_1.userRoleRepository.assignRole(testUser.id, testRole.id);
        const uRolesRestored = await core_1.userRoleRepository.getUserRoles(testUser.id);
        ...
        const rolePermsBefore = await core_1.rolePermissionRepository.getPermissions(testRole.id);
        assert(rolePermsBefore.some(rp => rp.permissionId === testPermission.id), 'Role has active permission mapping before');
        // Revoke
        await core_1.rolePermissionRepository.revokePermission(testRole.id, testPermission.id);
        const rolePermsAfter = await core_1.rolePermissionRepository.getPermissions(testRole.id);
        ...
        // Re-assign
        await core_1.rolePermissionRepository.assignPermission(testRole.id, testPermission.id);
        const rolePermsRestored = await core_1.rolePermissionRepository.getPermissions(testRole.id);
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            const u = await core_1.userRepository.create({
        ...
        const pRes = await core_1.userRepository.paginate({ page: 1, limit: 3 }, { username: { startsWith: 'user_infra_' }, email: { endsWith: `${RUN}@netfusion.test` } }, [{ field: 'username', direction: 'asc' }]);
        ...
        const pResPage2 = await core_1.userRepository.paginate({ page: 2, limit: 3 }, { username: { startsWith: 'user_infra_' }, email: { endsWith: `${RUN}@netfusion.test` } }, [{ field: 'username', direction: 'asc' }]);
        ...
        // B. Sorting desc
        const sortedDesc = await core_1.userRepository.findMany({
        ...
        const softDeletedUser = await core_1.userRepository.softDelete(targetUser.id, 'infra-test');
        ...
        const checkExists = await core_1.userRepository.exists({ id: targetUser.id, deletedAt: null });
        assert(checkExists === false, 'exists returns false for soft deleted record when filtering');
        // Restore
        const restoredUser = await core_1.userRepository.restore(targetUser.id);
        ...
        for (const u of extraUsers) {
            await core_1.userRepository.delete(u.id);
        }
    }
    // ───────────────────────────────────────────────────────────────────────────
    ...
    try {
        const committedUser = await core_1.userRepository.transaction(async (tx) => {
            const u = await core_1.userRepository.create({
            ...
            await core_1.projectRepository.create({
                ownerId: u.id,
                name: `TX Project ${RUN}`,
                status: 'ACTIVE'
            }, tx);
        ...
        const uExists = await core_1.userRepository.exists({ id: committedUser.id });
        const pExists = await core_1.projectRepository.exists({ name: `TX Project ${RUN}` });
        assert(uExists && pExists, 'Committed transaction modifications are persisted');
        // Cleanup
        const prj = await core_1.projectRepository.findOne({ name: `TX Project ${RUN}` });
        if (prj)
            await core_1.projectRepository.delete(prj.id);
        await core_1.userRepository.delete(committedUser.id);
    }
    catch (e) {
    ...
    try {
        await core_1.userRepository.transaction(async (tx) => {
            const u = await core_1.userRepository.create({
            ...
            await core_1.userRepository.create({
                email: `tx-roll-${RUN}@netfusion.test`,
        ...
        const uExists = await core_1.userRepository.exists({ email: `tx-roll-${RUN}@netfusion.test` });
    ...
    try {
        ...
        class AssetRepositoryForLockCheck extends BaseRepository {
            constructor() {
                super('asset');
            }
        }
    ...
    try {
        ...
        // Let's verify this.
        const tempUser = await core_1.userRepository.create({
        ...
        const tempRole = await core_1.roleRepository.create({
            name: `cascade-role-${RUN}`,
            displayName: `Cascade Role ${RUN}`,
            description: 'Temporary role for cascade test',
            isSystem: false
        });
        const ur = await core_1.userRoleRepository.assignRole(tempUser.id, tempRole.id);
        assert(!!ur.id, 'UserRole mapping created successfully');
        // Delete User
        await core_1.userRepository.delete(tempUser.id);
        ...
        const urExists = await core_1.userRoleRepository.exists({ id: ur.id });
        assert(urExists === false, 'UserRole mapping cascade-deleted when user is physically deleted');
        // Clean up Role
        await core_1.roleRepository.delete(tempRole.id);
    }
    catch (e) {
    ...
    try {
        ...
        if (testRole) {
            await core_1.roleRepository.delete(testRole.id);
            ok('Test Role physically deleted');
        }
        if (testPermission) {
            await core_1.permissionRepository.delete(testPermission.id);
