...
async function setupFixtures() {
    ...
    const prov = await prisma_1.default.provider.create({
        data: {
...
async function testConversationService() {
    section('1. ConversationService');
    // 1.1 Create
    const conv = await ai_1.conversationService.createConversation({
        title: `conv-${RUN}`,
        projectId: testProject.id,
    ...
    try {
        await ai_1.conversationService.createConversation({ projectId: testProject.id });
        fail('1.07 createConversation missing title should throw');
    }
    catch {
        ok('1.07 validation rejects missing title');
    }
    // 1.3 Invalid UUID validation
    try {
        await ai_1.conversationService.createConversation({
            title: 'x', projectId: 'not-a-uuid', createdBy: 'sys', updatedBy: 'sys',
        });
        fail('1.08 invalid projectId UUID should throw');
    }
    catch {
        ok('1.08 validation rejects invalid projectId UUID');
    }
    // 1.4 Add message
    const msg = await ai_1.conversationService.addMessage(conv.id, {
        role: 'user',
        content: 'Hello AI!',
        createdBy: testUser.id,
        updatedBy: testUser.id,
    ...
    const convAfterMsg = await ai_1.conversationService.findConversation(conv.id);
    assert((convAfterMsg?.contextSize ?? 0) > 0, '1.13 contextSize updated after message');
    // 1.6 Add assistant message
    const aiMsg = await ai_1.conversationService.addMessage(conv.id, {
        role: 'assistant',
    ...
    const reply = await ai_1.conversationService.addMessage(conv.id, {
        role: 'user',
    ...
    try {
        await ai_1.conversationService.addMessage(conv.id, { role: '', content: '', createdBy: 'x', updatedBy: 'x' });
        fail('1.16 empty role/content should throw');
    }
    catch {
        ok('1.16 message validation rejects empty role/content');
    }
    // 1.9 Search messages
    const found = await ai_1.conversationService.searchMessages('Hello');
    ...
    // 1.10 Update summary
    const withSummary = await ai_1.conversationService.updateSummary(conv.id, 'Test summary', testUser.id);
    assert(withSummary.summary === 'Test summary', '1.19 summary updated');
    // 1.11 Add / remove tag
    const tagged = await ai_1.conversationService.addTag(conv.id, 'security', testUser.id);
    assert(tagged.tags.includes('security'), '1.20 tag added');
    const deduped = await ai_1.conversationService.addTag(conv.id, 'security', testUser.id);
    assert(deduped.tags.filter((t) => t === 'security').length === 1, '1.21 tags deduplicated');
    const untagged = await ai_1.conversationService.removeTag(conv.id, 'security', testUser.id);
    assert(!untagged.tags.includes('security'), '1.22 tag removed');
    // 1.12 Recalculate context size
    const size = await ai_1.conversationService.recalculateContextSize(conv.id);
    assert(typeof size === 'number' && size > 0, '1.23 recalculateContextSize returns positive number');
    // 1.13 Stats
    const stats = await ai_1.conversationService.getConversationStats(conv.id);
    ...
    // 1.14 Find helpers
    const byProject = await ai_1.conversationService.findByProject(testProject.id);
    assert(byProject.some((c) => c.id === conv.id), '1.28 findByProject includes conversation');
    const byUser = await ai_1.conversationService.findByUser(testUser.id);
    assert(byUser.some((c) => c.id === conv.id), '1.29 findByUser includes conversation');
    const byInvestigation = await ai_1.conversationService.findByInvestigation(testInvestigation.id);
    assert(byInvestigation.some((c) => c.id === conv.id), '1.30 findByInvestigation includes conversation');
    const active = await ai_1.conversationService.findActive();
    assert(active.some((c) => c.id === conv.id), '1.31 findActive includes conversation');
    // 1.15 Lifecycle transitions
    const archived = await ai_1.conversationService.archiveConversation(conv.id, testUser.id);
    assert(archived.status === 'ARCHIVED', '1.32 archiveConversation sets ARCHIVED');
    const reactivated = await ai_1.conversationService.reactivateConversation(conv.id, testUser.id);
    assert(reactivated.status === 'ACTIVE', '1.33 reactivateConversation sets ACTIVE');
    const completed = await ai_1.conversationService.completeConversation(conv.id, testUser.id);
    assert(completed.status === 'COMPLETED', '1.34 completeConversation sets COMPLETED');
    // 1.16 Event publishing
    let eventFired = false;
    const handler = () => { eventFired = true; };
    EventPublisher_1.eventPublisher.subscribe('ConversationCreated', handler);
    const convB = await ai_1.conversationService.createConversation({
    ...
    EventPublisher_1.eventPublisher.unsubscribe('ConversationCreated', handler);
    // 1.17 Soft delete
    await ai_1.conversationService.deleteConversation(conv.id, testUser.id);
    const deleted = await ai_1.conversationService.findConversation(conv.id);
    assert(deleted?.deletedAt !== null, '1.36 deleteConversation sets deletedAt');
    // 1.18 Deletion validation
    try {
        await ai_1.conversationService.deleteConversation('not-a-uuid', 'actor');
        fail('1.37 deleteConversation invalid UUID should throw');
    }
    catch {
        ok('1.37 deleteConversation validates UUID');
    }
    // cleanup extra
    await prisma_1.default.conversation.delete({ where: { id: convB.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 2 — SessionMemoryService
// ═══════════════════════════════════════════════════════════════════════════════
async function testSessionMemoryService() {
    section('2. SessionMemoryService');
    ...
    // 2.1 Create
    const mem = await ai_1.sessionMemoryService.createMemory({
    ...
    try {
        await ai_1.sessionMemoryService.createMemory({ projectId: testProject.id });
        fail('2.07 missing conversationId should throw');
    }
    catch {
        ok('2.07 validation rejects missing conversationId');
    }
    // 2.3 Add entry
    const entry = await ai_1.sessionMemoryService.addEntry(mem.id, {
        memoryType: 'FACT',
        state: 'ACTIVE',
        title: 'First fact',
    ...
    const memAfterEntry = await ai_1.sessionMemoryService.findMemory(mem.id);
    assert((memAfterEntry?.contextSize ?? 0) > 0, '2.14 contextSize updated after entry');
    // 2.4 Add second entry
    const entry2 = await ai_1.sessionMemoryService.addEntry(mem.id, {
    ...
    // 2.5 Update entry
    const updated = await ai_1.sessionMemoryService.updateEntry(entry.id, {
        importanceScore: 90.0,
        updatedBy: testUser.id,
    });
    assert(updated.importanceScore === 90.0, '2.16 updateEntry persists new importanceScore');
    // 2.6 Search
    const found = await ai_1.sessionMemoryService.searchEntries('attacker');
    ...
    const avgImportance = await ai_1.sessionMemoryService.calculateAverageImportance(mem.id);
    ...
    const avgConfidence = await ai_1.sessionMemoryService.calculateAverageConfidence(mem.id);
    ...
    // 2.8 Stats
    const stats = await ai_1.sessionMemoryService.getMemoryStats(mem.id);
    ...
    // 2.9 Find helpers
    const byProject = await ai_1.sessionMemoryService.findByProject(testProject.id);
    assert(byProject.some((m) => m.id === mem.id), '2.27 findByProject');
    const byUser = await ai_1.sessionMemoryService.findByUser(testUser.id);
    assert(byUser.some((m) => m.id === mem.id), '2.28 findByUser');
    const byInv = await ai_1.sessionMemoryService.findByInvestigation(testInvestigation.id);
    assert(byInv.some((m) => m.id === mem.id), '2.29 findByInvestigation');
    const activeMemories = await ai_1.sessionMemoryService.findActive();
    assert(activeMemories.some((m) => m.id === mem.id), '2.30 findActive');
    const entries = await ai_1.sessionMemoryService.findEntries(mem.id);
    assert(entries.length === 2, '2.31 findEntries returns 2');
    // 2.10 Lifecycle transitions
    const archived = await ai_1.sessionMemoryService.archiveMemory(mem.id, testUser.id);
    assert(archived.status === 'ARCHIVED', '2.32 archiveMemory sets ARCHIVED');
    const activated = await ai_1.sessionMemoryService.activateMemory(mem.id, testUser.id);
    assert(activated.status === 'ACTIVE', '2.33 activateMemory sets ACTIVE');
    // 2.11 Delete entry
    await ai_1.sessionMemoryService.deleteEntry(entry2.id, testUser.id);
    const entriesAfterDelete = await ai_1.sessionMemoryService.findEntries(mem.id);
    assert(entriesAfterDelete.every((e) => e.id !== entry2.id), '2.34 deleteEntry soft-deletes');
    // 2.12 Soft delete memory
    await ai_1.sessionMemoryService.deleteMemory(mem.id, testUser.id);
    const deletedMem = await ai_1.sessionMemoryService.findMemory(mem.id);
    assert(deletedMem?.deletedAt !== null, '2.35 deleteMemory sets deletedAt');
    // cleanup
    await prisma_1.default.conversation.delete({ where: { id: conv.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 3 — ContextWindowService
// ═══════════════════════════════════════════════════════════════════════════════
async function testContextWindowService() {
    section('3. ContextWindowService');
    // 3.1 Create window
    const win = await ai_1.contextWindowService.createWindow({
        projectId: testProject.id,
        investigationId: testInvestigation.id,
        userId: testUser.id,
        windowName: `win-${RUN}`,
    ...
    // 3.2 Validation
    try {
        await ai_1.contextWindowService.createWindow({ projectId: 'bad-uuid', createdBy: 'x', updatedBy: 'x' });
        fail('3.05 invalid projectId UUID should throw');
    }
    catch {
        ok('3.05 UUID validation on createWindow');
    }
    // 3.3 Add entry
    const entry = await ai_1.contextWindowService.addEntry(win.id, {
        source: 'alert',
        priority: 'HIGH',
    ...
    // Context size updated
    const winAfterEntry = await ai_1.contextWindowService.findWindow(win.id);
    assert((winAfterEntry?.contextSize ?? 0) > 0, '3.11 contextSize updated after entry');
    // 3.4 Add second entry
    const entry2 = await ai_1.contextWindowService.addEntry(win.id, {
        source: 'finding',
        priority: 'MEDIUM',
    ...
    // 3.5 Update entry
    const updated = await ai_1.contextWindowService.updateEntry(entry.id, {
        importanceScore: 95.0,
        updatedBy: testUser.id,
    });
    assert(updated.importanceScore === 95.0, '3.13 updateEntry persists change');
    // 3.6 Rank by importance
    const ranked = await ai_1.contextWindowService.rankEntriesByImportance(win.id);
    ...
    const rankedConf = await ai_1.contextWindowService.rankEntriesByConfidence(win.id);
    ...
    // 3.8 Stats
    const stats = await ai_1.contextWindowService.getWindowStats(win.id);
    ...
    // 3.9 Find helpers
    const byUser = await ai_1.contextWindowService.findByUser(testUser.id);
    assert(byUser.some((w) => w.id === win.id), '3.23 findByUser includes window');
    const activeWins = await ai_1.contextWindowService.findActive();
    assert(activeWins.some((w) => w.id === win.id), '3.24 findActive includes window');
    const foundEntries = await ai_1.contextWindowService.findEntries(win.id);
    assert(foundEntries.length === 2, '3.25 findEntries returns 2');
    // 3.10 Lifecycle transitions
    const archWin = await ai_1.contextWindowService.archiveWindow(win.id, testUser.id);
    assert(archWin.status === 'ARCHIVED', '3.26 archiveWindow sets ARCHIVED');
    const actWin = await ai_1.contextWindowService.activateWindow(win.id, testUser.id);
    assert(actWin.status === 'ACTIVE', '3.27 activateWindow sets ACTIVE');
    // 3.11 Delete entry
    await ai_1.contextWindowService.deleteEntry(entry2.id, testUser.id);
    const afterDelete = await ai_1.contextWindowService.findEntries(win.id);
    assert(afterDelete.every((e) => e.id !== entry2.id), '3.28 deleteEntry soft-deletes');
    // 3.12 Soft delete window
    await ai_1.contextWindowService.deleteWindow(win.id, testUser.id);
    const deletedWin = await ai_1.contextWindowService.findWindow(win.id);
    assert(deletedWin?.deletedAt !== null, '3.29 deleteWindow sets deletedAt');
}
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 4 — PromptAssemblyService
// ═══════════════════════════════════════════════════════════════════════════════
async function testPromptAssemblyService() {
    section('4. PromptAssemblyService');
    // 4.1 Create prompt
    ...
    try {
        await ai_1.promptAssemblyService.createPrompt({
            projectId: testProject.id,
        });
        fail('4.06 missing investigationId should throw');
    }
    catch {
        ok('4.06 validation rejects missing investigationId');
    }
    // 4.3 Add section
    const section1 = await ai_1.promptAssemblyService.addSection(prompt.id, {
        title: 'Alert Context',
    ...
    const section2 = await ai_1.promptAssemblyService.addSection(prompt.id, {
    ...
    // 4.5 Update section
    const updSection = await ai_1.promptAssemblyService.updateSection(section1.id, {
        priority: 90,
        updatedBy: testUser.id,
    });
    ...
    // 4.7 Token usage
    const tokenEst = await ai_1.promptAssemblyService.calculateTokenUsage(prompt.id);
    ...
    // 4.9 Search sections
    const foundSections = await ai_1.promptAssemblyService.searchSections('Alert');
    ...
    // 4.10 Find helpers
    const byProject = await ai_1.promptAssemblyService.findByProject(testProject.id);
    assert(byProject.some((p) => p.id === prompt.id), '4.25 findByProject');
    const foundSects = await ai_1.promptAssemblyService.findSections(prompt.id);
    assert(foundSects.length === 2, '4.26 findSections returns 2');
    // 4.11 Lifecycle transitions
    const published = await ai_1.promptAssemblyService.publishPrompt(prompt.id, testUser.id);
    assert(published.status === 'ACTIVE', '4.27 publishPrompt sets ACTIVE');
    const pubList = await ai_1.promptAssemblyService.findPublished();
    assert(pubList.some((p) => p.id === prompt.id), '4.28 findPublished includes prompt');
    const archived = await ai_1.promptAssemblyService.archivePrompt(prompt.id, testUser.id);
    assert(archived.status === 'ARCHIVED', '4.29 archivePrompt sets ARCHIVED');
    const drafted = await ai_1.promptAssemblyService.draftPrompt(prompt.id, testUser.id);
    assert(drafted.status === 'DRAFT', '4.30 draftPrompt sets DRAFT');
    // 4.12 Delete section
    await ai_1.promptAssemblyService.deleteSection(section2.id, testUser.id);
    const afterDel = await ai_1.promptAssemblyService.findSections(prompt.id);
    assert(afterDel.every((s) => s.id !== section2.id), '4.31 deleteSection soft-deletes');
    // 4.13 Soft delete prompt
    await ai_1.promptAssemblyService.deletePrompt(prompt.id, testUser.id);
    const deleted = await ai_1.promptAssemblyService.findPrompt(prompt.id);
...
async function testReasoningService() {
    ...
    // 5.2 Validation
    try {
        await ai_1.reasoningService.createSession({ projectId: testProject.id });
        fail('5.07 missing investigationId should throw');
    }
    catch {
        ok('5.07 validation rejects missing investigationId');
    }
    // 5.3 Add step 1
    const step1 = await ai_1.reasoningService.addStep(session.id, {
        stepNumber: 1,
        stage: 'COLLECTION',
    ...
    // 5.4 Add step 2
    const step2 = await ai_1.reasoningService.addStep(session.id, {
        stepNumber: 2,
        stage: 'ANALYSIS',
    ...
    const sessionAfterSteps = await ai_1.reasoningService.findSession(session.id);
    assert((sessionAfterSteps?.overallConfidence ?? 0) > 0, '5.14 overallConfidence updated after steps');
    // 5.5 Update step
    const updStep = await ai_1.reasoningService.updateStep(step1.id, {
        confidence: 0.9,
        updatedBy: testUser.id,
    });
    assert(updStep.confidence === 0.9, '5.15 updateStep persists confidence change');
    // 5.6 Link evidence to step
    const linkedStep = await ai_1.reasoningService.linkEvidenceToStep(step1.id, testInvestigation.id, testUser.id);
    assert(linkedStep.evidenceIds.includes(testInvestigation.id), '5.16 linkEvidenceToStep adds evidenceId');
    // Deduplication
    const linkedAgain = await ai_1.reasoningService.linkEvidenceToStep(step1.id, testInvestigation.id, testUser.id);
    ...
    const linkedFinding = await ai_1.reasoningService.linkFindingToStep(step2.id, testInvestigation.id, testUser.id);
    ...
    const conf = await ai_1.reasoningService.calculateConfidence(session.id);
    ...
    // 5.11 Find helpers
    const byStatus = await ai_1.reasoningService.findByStatus('ACTIVE');
    assert(byStatus.some((s) => s.id === session.id), '5.27 findByStatus includes session');
    const steps = await ai_1.reasoningService.findSteps(session.id);
    ...
    const completed = await ai_1.reasoningService.completeSession(session.id, 'Threat confirmed: APT-29', testUser.id);
    ...
    const completedList = await ai_1.reasoningService.findCompleted();
    assert(completedList.some((s) => s.id === session.id), '5.32 findCompleted includes session');
    // 5.13 Create a session to fail
    const failSession = await ai_1.reasoningService.createSession({
        projectId: testProject.id,
        investigationId: testInvestigation.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const failed2 = await ai_1.reasoningService.failSession(failSession.id, 'system');
    assert(failed2.status === 'FAILED', '5.33 failSession sets FAILED');
    // 5.14 Soft delete
    await ai_1.reasoningService.deleteSession(session.id, testUser.id);
    const deletedSess = await ai_1.reasoningService.findSession(session.id);
    assert(deletedSess?.deletedAt !== null, '5.34 deleteSession sets deletedAt');
    // cleanup
    await prisma_1.default.reasoning.delete({ where: { id: failSession.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
...
async function testProviderService() {
    ...
    const prov = await ai_1.providerService.registerProvider({
    ...
    try {
        await ai_1.providerService.registerProvider({ endpoint: 'x' });
        fail('6.06 missing providerName should throw');
    }
    catch {
        ok('6.06 validation rejects missing providerName');
    }
    // 6.3 Register model
    const model = await ai_1.providerService.registerModel(prov.id, {
        modelName: 'gpt-4o',
        alias: 'gpt4-turbo',
        streaming: true,
        toolCalling: true,
    ...
    const embModel = await ai_1.providerService.registerModel(prov.id, {
    ...
    const caps = await ai_1.providerService.getCapabilities(prov.id);
    ...
    const enabled = await ai_1.providerService.findEnabled();
    assert(enabled.some((p) => p.id === prov.id), '6.21 findEnabled includes provider');
    const healthy = await ai_1.providerService.findHealthy();
    assert(healthy.some((p) => p.id === prov.id), '6.22 findHealthy includes provider');
    // 6.7 Find models
    const models = await ai_1.providerService.findModels(prov.id);
    assert(models.length >= 2, '6.23 findModels returns >= 2');
    const modelByName = await ai_1.providerService.findModelByName(prov.id, 'gpt-4o');
    assert(modelByName?.id === model.id, '6.24 findModelByName returns correct model');
    // 6.8 Provider selection — priority strategy
    const selected = await ai_1.providerService.selectProvider({ strategy: 'priority' });
    assert(selected !== null, '6.25 selectProvider returns provider');
    const selectedHealth = await ai_1.providerService.selectProvider({ strategy: 'health' });
    assert(selectedHealth !== null, '6.26 selectProvider health strategy');
    const selectedRandom = await ai_1.providerService.selectProvider({ strategy: 'random' });
    assert(selectedRandom !== null, '6.27 selectProvider random strategy');
    // 6.9 Capability-filtered selection
    const streamingProvider = await ai_1.providerService.selectProvider({ requireStreaming: true });
    assert(streamingProvider !== null, '6.28 selectProvider requireStreaming');
    const toolCallingProvider = await ai_1.providerService.selectProvider({ requireToolCalling: true });
    assert(toolCallingProvider !== null, '6.29 selectProvider requireToolCalling');
    // 6.10 Health score update
    const updated = await ai_1.providerService.updateHealthScore(prov.id, 75.0, 'system');
    assert(updated.healthScore === 75.0, '6.30 updateHealthScore persists value');
    // 6.11 Provider stats
    const stats = await ai_1.providerService.getProviderStats(prov.id);
    ...
    // 6.12 Disable / enable
    const disabled = await ai_1.providerService.disableProvider(prov.id, 'system');
    assert(disabled.enabled === false, '6.35 disableProvider sets enabled = false');
    const reenabled = await ai_1.providerService.enableProvider(prov.id, 'system');
    assert(reenabled.enabled === true, '6.36 enableProvider sets enabled = true');
    // 6.13 Get model capabilities
    const modelCaps = await ai_1.providerService.getModelCapabilities(model.id);
    assert(modelCaps?.streaming === true, '6.37 getModelCapabilities returns model');
    // 6.14 Soft delete
    await ai_1.providerService.deleteProvider(prov.id, 'system');
    const deleted = await ai_1.providerService.findProvider(prov.id);
...
async function testExecutionService() {
    ...
    const exec = await ai_1.executionService.submitExecution({
    ...
    try {
        await ai_1.executionService.submitExecution({ systemPrompt: 'x' });
        fail('7.06 missing providerId should throw');
    }
    catch {
        ok('7.06 validation rejects missing providerId');
    }
    // 7.3 Disabled provider check
    await prisma_1.default.provider.update({
        where: { id: testProvider.id },
        data: { enabled: false },
    });
    try {
        await ai_1.executionService.submitExecution({
            providerId: testProvider.id,
            systemPrompt: 'x',
            userPrompt: 'y',
            createdBy: 'system',
        ...
        ok('7.07 disabled provider rejected on submit');
    }
    // Re-enable
    await prisma_1.default.provider.update({ where: { id: testProvider.id }, data: { enabled: true } });
    // 7.4 Lifecycle transitions
    const started = await ai_1.executionService.startExecution(exec.id, testUser.id);
    assert(started.status === 'ACTIVE', '7.08 startExecution sets ACTIVE');
    const completed = await ai_1.executionService.completeExecution(exec.id, testUser.id);
    assert(completed.status === 'COMPLETED', '7.09 completeExecution sets COMPLETED');
    // 7.5 Record usage
    const usage = await ai_1.executionService.recordUsage(exec.id, {
        promptTokens: 450,
    ...
    // 7.6 Calculate cost
    const cost = await ai_1.executionService.calculateCost(exec.id);
    assert(Math.abs(cost - 0.00285) < 0.0001, '7.17 calculateCost returns correct value');
    // 7.7 Usage stats
    const stats = await ai_1.executionService.getUsageStats(exec.id);
    ...
    const exec2 = await ai_1.executionService.submitExecution({
        providerId: testProvider.id,
        systemPrompt: 'sys',
        userPrompt: 'usr',
        createdBy: 'system',
    ...
    const failedExec = await ai_1.executionService.failExecution(exec2.id, 'system');
    assert(failedExec.status === 'FAILED', '7.21 failExecution sets FAILED');
    const failedList = await ai_1.executionService.findFailed();
    assert(failedList.some((e) => e.id === exec2.id), '7.22 findFailed includes execution');
    const exec3 = await ai_1.executionService.submitExecution({
        providerId: testProvider.id,
        systemPrompt: 'sys',
        userPrompt: 'usr',
        createdBy: 'system',
    ...
    const cancelledExec = await ai_1.executionService.cancelExecution(exec3.id, 'system');
    assert(cancelledExec.status === 'FAILED', '7.23 cancelExecution sets FAILED');
    // 7.9 Find helpers
    const byProvider = await ai_1.executionService.findByProvider(testProvider.id);
    assert(byProvider.some((e) => e.id === exec.id), '7.24 findByProvider');
    const byStatus = await ai_1.executionService.findByStatus('COMPLETED');
    assert(byStatus.some((e) => e.id === exec.id), '7.25 findByStatus COMPLETED');
    const completedList = await ai_1.executionService.findCompleted();
    assert(completedList.some((e) => e.id === exec.id), '7.26 findCompleted');
    // 7.10 Aggregate project usage
    const aggregate = await ai_1.executionService.aggregateProjectUsage(testProject.id);
    ...
    // 7.11 Soft delete
    await ai_1.executionService.deleteExecution(exec.id, testUser.id);
    const deleted = await ai_1.executionService.findExecution(exec.id);
    assert(deleted?.deletedAt !== null, '7.32 deleteExecution sets deletedAt');
    // cleanup
    await prisma_1.default.execution.delete({ where: { id: exec2.id } }).catch(() => { });
    await prisma_1.default.execution.delete({ where: { id: exec3.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
...
async function testStreamingService() {
    ...
    const exec = await prisma_1.default.execution.create({
        data: {
            providerId: testProvider.id,
            systemPrompt: 'system',
            userPrompt: 'user',
    ...
    // 8.2 Validation
    try {
        await ai_1.streamingService.createSession({ executionId: 'bad-uuid', createdBy: 'x', updatedBy: 'x' });
        fail('8.04 invalid executionId UUID should throw');
    }
    catch {
        ok('8.04 UUID validation on createSession');
    }
    // 8.3 Append chunk 1
    const chunk1 = await ai_1.streamingService.appendChunk(stream.id, {
        sequenceNumber: 1,
    ...
    // 8.4 Append chunk 2
    const chunk2 = await ai_1.streamingService.appendChunk(stream.id, {
        sequenceNumber: 2,
    ...
    const progress = await ai_1.streamingService.getProgress(stream.id);
    assert(typeof progress === 'number', '8.11 getProgress returns number');
    assert(progress >= 0, '8.12 progress >= 0');
    // 8.6 Find chunks
    const chunks = await ai_1.streamingService.findChunks(stream.id);
    ...
    const content = await ai_1.streamingService.reconstructContent(stream.id);
    ...
    const statsActive = await ai_1.streamingService.getStreamingStats(stream.id);
    ...
    const chunk3 = await ai_1.streamingService.appendChunk(stream.id, {
        sequenceNumber: 3,
    ...
    const streamAfterFinal = await ai_1.streamingService.findSession(stream.id);
    ...
    try {
        ...
        ok('8.22 cannot append chunk to COMPLETED session');
    }
    // 8.11 Stats after completion
    const statsCompleted = await ai_1.streamingService.getStreamingStats(stream.id);
    ...
    const stream2 = await ai_1.streamingService.createSession({
        executionId: exec2.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const failed = await ai_1.streamingService.failSession(stream2.id, 'system');
    assert(failed.status === 'FAILED', '8.28 failSession sets FAILED');
    // 8.15 Create a fresh session to cancel
    const exec3 = await prisma_1.default.execution.create({
        data: {
            providerId: testProvider.id,
            systemPrompt: 'sys',
            userPrompt: 'usr',
    ...
    const stream3 = await ai_1.streamingService.createSession({
        executionId: exec3.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const cancelled = await ai_1.streamingService.cancelSession(stream3.id, 'system');
    assert(cancelled.status === 'FAILED', '8.29 cancelSession sets FAILED');
    // 8.16 Soft delete
    await ai_1.streamingService.deleteSession(stream.id, testUser.id);
    const deleted = await ai_1.streamingService.findSession(stream.id);
    assert(deleted?.deletedAt !== null, '8.30 deleteSession sets deletedAt');
    // cleanup
    await prisma_1.default.execution.delete({ where: { id: exec.id } }).catch(() => { });
    await prisma_1.default.execution.delete({ where: { id: exec2.id } }).catch(() => { });
    await prisma_1.default.execution.delete({ where: { id: exec3.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
...
async function testIntegration() {
    ...
    const userMsg = await ai_1.conversationService.addMessage(conv.id, {
        role: 'user',
    ...
    const ctxWin = await ai_1.contextWindowService.createWindow({
        projectId: testProject.id,
        investigationId: testInvestigation.id,
        conversationId: conv.id,
        userId: testUser.id,
    ...
    await ai_1.contextWindowService.addEntry(ctxWin.id, {
        source: 'alert',
        priority: 'CRITICAL',
    ...
    const reasoning = await ai_1.reasoningService.createSession({
        projectId: testProject.id,
        investigationId: testInvestigation.id,
    ...
    await ai_1.reasoningService.addStep(reasoning.id, {
        stepNumber: 1,
        stage: 'TRIAGE',
    ...
    await ai_1.promptAssemblyService.publishPrompt(prompt.id, testUser.id);
    await prisma_1.default.promptAssembly.update({
        where: { id: prompt.id },
        data: { reasoningId: reasoning.id },
    });
    const linkedPrompt = await ai_1.promptAssemblyService.findPrompt(prompt.id);
    ...
    await ai_1.executionService.startExecution(exec.id, testUser.id);
    ...
    await ai_1.executionService.completeExecution(exec.id, testUser.id);
    const usage = await ai_1.executionService.recordUsage(exec.id, {
        promptTokens: 1200,
    ...
    const completedReasoning = await ai_1.reasoningService.completeSession(reasoning.id, 'Lateral movement confirmed — APT behavior pattern', testUser.id);
    ...
    await ai_1.conversationService.addMessage(conv.id, {
        role: 'assistant',
        content: 'THREAT LEVEL: CRITICAL. Lateral movement confirmed.',
        createdBy: 'system',
        updatedBy: 'system',
    });
    const convStats = await ai_1.conversationService.getConversationStats(conv.id);
    assert(convStats.messageCount === 2, '9.22 Conversation has 2 messages');
    // 9.2 Provider selection for streaming execution
    const streamingProv = await ai_1.providerService.selectProvider({ requireStreaming: true });
    ...
    ['ConversationCreated', 'SessionMemoryCreated', 'ContextWindowCreated',
        ...
        'StreamingSessionCreated'].forEach((e) => EventPublisher_1.eventPublisher.subscribe(e, counter));
    const fullConv = await ai_1.conversationService.createConversation({
    ...
    // cleanup
    await prisma_1.default.conversation.delete({ where: { id: fullConv.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
...
async function testEdgeCases() {
    ...
    const missing = await ai_1.conversationService.findConversation('a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11');
    assert(missing === null, '10.01 findConversation returns null for unknown id');
    // 10.2 SessionMemoryService — average on empty memory
    const emptyConv = await prisma_1.default.conversation.create({
        data: {
            title: `edge-conv-${RUN}`,
            projectId: testProject.id,
            createdBy: 'system',
    ...
    const emptyMem = await ai_1.sessionMemoryService.createMemory({
        conversationId: emptyConv.id,
        projectId: testProject.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const emptyAvgImportance = await ai_1.sessionMemoryService.calculateAverageImportance(emptyMem.id);
    assert(emptyAvgImportance === 0.0, '10.02 calculateAverageImportance = 0 on empty memory');
    const emptyAvgConf = await ai_1.sessionMemoryService.calculateAverageConfidence(emptyMem.id);
    assert(emptyAvgConf === 0.0, '10.03 calculateAverageConfidence = 0 on empty memory');
    // 10.3 ContextWindowService — empty stats
    const emptyWin = await ai_1.contextWindowService.createWindow({
        projectId: testProject.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const emptyWinStats = await ai_1.contextWindowService.getWindowStats(emptyWin.id);
    ...
    const emptyReasoning = await ai_1.reasoningService.createSession({
        projectId: testProject.id,
        investigationId: testInvestigation.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const emptyConf = await ai_1.reasoningService.calculateConfidence(emptyReasoning.id);
    assert(emptyConf === 0.0, '10.06 calculateConfidence = 0 on session with no steps');
    const emptyRisk = await ai_1.reasoningService.calculateOverallRisk(emptyReasoning.id);
    ...
    const noUsageExec = await ai_1.executionService.submitExecution({
        providerId: testProvider.id,
        systemPrompt: 'sys',
        userPrompt: 'usr',
        createdBy: 'system',
    ...
    const nullUsage = await ai_1.executionService.getUsageStats(noUsageExec.id);
    assert(nullUsage === null, '10.08 getUsageStats returns null when no usage recorded');
    const zeroCost = await ai_1.executionService.calculateCost(noUsageExec.id);
    ...
    const noStream = await ai_1.streamingService.findByExecution('a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11');
    ...
    const largePrompt = await ai_1.promptAssemblyService.createPrompt({
        investigationId: testInvestigation.id,
        projectId: testProject.id,
        systemPrompt: 'sys',
        userPrompt: 'usr',
    ...
    await ai_1.promptAssemblyService.addSection(largePrompt.id, {
        title: 'Big Section',
    ...
    const tightBudget = await ai_1.promptAssemblyService.checkTokenBudget(largePrompt.id);
    ...
    try {
        await ai_1.conversationService.searchMessages('');
        fail('10.12 empty search query should throw');
    }
    catch {
        ok('10.12 searchMessages rejects empty query');
    }
    // 10.9 SessionMemoryService — search empty query throws
    try {
        await ai_1.sessionMemoryService.searchEntries('');
        fail('10.13 empty searchEntries query should throw');
    }
    catch {
        ok('10.13 searchEntries rejects empty query');
    }
    // 10.10 PromptAssemblyService — search empty query throws
    try {
        await ai_1.promptAssemblyService.searchSections('');
        fail('10.14 empty searchSections query should throw');
    }
    catch {
        ok('10.14 searchSections rejects empty query');
    }
    ...
    const result = await ai_1.providerService.selectProvider({ strategy: 'priority' });
    ...
    const clampSession = await ai_1.reasoningService.createSession({
        projectId: testProject.id,
        investigationId: testInvestigation.id,
        createdBy: 'system',
        updatedBy: 'system',
    });
    const clampedStep = await ai_1.reasoningService.addStep(clampSession.id, {
        stepNumber: 1,
        stage: 'TEST',
        inputSummary: 'x',
        outputSummary: 'y',
    ...
    const negativeStep = await ai_1.reasoningService.addStep(clampSession.id, {
        stepNumber: 2,
        stage: 'TEST',
        inputSummary: 'x',
        outputSummary: 'y',
    ...
    const provForClamp = await ai_1.providerService.registerProvider({
    ...
    const overHundred = await ai_1.providerService.updateHealthScore(provForClamp.id, 150.0, 'system');
    assert(overHundred.healthScore <= 100.0, '10.18 healthScore clamped to max 100');
    const belowZero = await ai_1.providerService.updateHealthScore(provForClamp.id, -50.0, 'system');
    ...
    try {
        await ai_1.conversationService.findByProject('not-a-uuid');
        fail('10.20 findByProject invalid UUID should throw');
    }
    catch {
        ok('10.20 findByProject validates UUID');
    }
    // 10.15 ExecutionService — invalid UUID on findByProvider
    try {
        await ai_1.executionService.findByProvider('not-a-uuid');
        fail('10.21 findByProvider invalid UUID should throw');
    }
    catch {
        ok('10.21 findByProvider validates UUID');
    }
    // cleanup
    await prisma_1.default.conversation.delete({ where: { id: emptyConv.id } }).catch(() => { });
    await prisma_1.default.execution.delete({ where: { id: noUsageExec.id } }).catch(() => { });
}
// ═══════════════════════════════════════════════════════════════════════════════
...
async function cleanup() {
    ...
    try {
        ...
        await prisma_1.default.reasoningStep.deleteMany({
            where: { reasoning: { OR: [
                        { project: { name: { contains: RUN } } },
                        { sessionName: { contains: RUN } },
                    ] } },
        }).catch(() => { });
        await prisma_1.default.reasoning.deleteMany({
            where: { OR: [
                    { project: { name: { contains: RUN } } },
                    { sessionName: { contains: RUN } },
                ] },
        ...
        // Seed provider
        await prisma_1.default.provider.delete({ where: { id: testProvider.id } }).catch(() => { });
        // Core fixtures
        await prisma_1.default.investigation.delete({ where: { id: testInvestigation.id } }).catch(() => { });
        await prisma_1.default.project.delete({ where: { id: testProject.id } }).catch(() => { });
