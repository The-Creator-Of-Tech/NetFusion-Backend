...
async function main() {
    ...
    try {
        const playbook = await workflow_1.playbookRepository.create({
            projectId: testProject.id,
            name: `Playbook ${RUN}`,
            severity: 'HIGH',
            status: 'DRAFT',
        ...
        // Update
        const updatedPlaybook = await workflow_1.playbookRepository.update(playbook.id, {
        ...
        // Soft delete
        const softDeleted = await workflow_1.playbookRepository.softDelete(playbook.id, 'test');
        assert(softDeleted.deletedAt !== null, 'Playbook softDeleted timestamp set');
        // Restore
        const restored = await workflow_1.playbookRepository.restore(playbook.id);
        assert(restored.deletedAt === null, 'Playbook restored successfully');
        // Clean up
        await workflow_1.playbookRepository.delete(playbook.id);
    ...
    try {
        const rule = await workflow_1.ruleRepository.create({
            projectId: testProject.id,
            name: `Rule ${RUN}`,
            severity: 'CRITICAL',
            status: 'ACTIVE',
        ...
        await workflow_1.ruleRepository.delete(rule.id);
    }
    catch (e) {
    ...
    try {
        ...
        const exists = await workflow_1.playbookRepository.exists({ name: `Tx Playbook ${RUN}` });
    ...
    if (!nodeServerAvailable) {
        ...
        for (const { file, pattern } of fastSmokeTests) {
            try {
                ...
                const output = (0, child_process_1.execSync)(`python ${file}`, {
                    encoding: 'utf8',
                    stdio: 'pipe',
                    timeout: 60000,
                    env: execEnv,
