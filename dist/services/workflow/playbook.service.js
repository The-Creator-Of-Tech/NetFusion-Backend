...
class PlaybookService extends BaseService_1.BaseService {
    constructor(playbookRepo = workflow_1.playbookRepository) {
        super();
    ...
    async createPlaybook(data, tx) {
        ...
        const runInTx = async (transaction) => {
            ...
            await EventPublisher_1.eventPublisher.publish('PlaybookCreated', { playbook });
            return playbook;
        };
     ...
     */
    async updatePlaybook(id, data, tx) {
        ...
        const runInTx = async (transaction) => {
            ...
            await EventPublisher_1.eventPublisher.publish('PlaybookUpdated', { playbook: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
     ...
     */
    async deletePlaybook(id, actor, tx) {
        this.validateUuid(id, 'playbookId');
        const runInTx = async (transaction) => {
            const existing = await this.playbookRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            await EventPublisher_1.eventPublisher.publish('PlaybookDeleted', { playbook: deleted });
            return deleted;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
     ...
     */
    async executePlaybook(id, actor, tx) {
        this.validateUuid(id, 'playbookId');
        const runInTx = async (transaction) => {
            const existing = await this.playbookRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            if (existing.status === 'DRAFT') {
                playbook = await this.playbookRepo.update(id, { status: 'ACTIVE', updatedBy: actor }, transaction);
            }
            const steps = await this.playbookRepo.findWithSteps(id, transaction);
            await EventPublisher_1.eventPublisher.publish('PlaybookExecutionStarted', {
                playbook,
                actor,
     ...
     */
    async enablePlaybook(id, actor, tx) {
        this.validateUuid(id, 'playbookId');
        const runInTx = async (transaction) => {
            const existing = await this.playbookRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            const updated = await this.playbookRepo.update(id, { enabled: true, updatedBy: actor }, transaction);
            await EventPublisher_1.eventPublisher.publish('PlaybookEnabled', { playbook: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
     ...
     */
    async disablePlaybook(id, actor, tx) {
        this.validateUuid(id, 'playbookId');
        const runInTx = async (transaction) => {
            const existing = await this.playbookRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            const updated = await this.playbookRepo.update(id, { enabled: false, updatedBy: actor }, transaction);
            await EventPublisher_1.eventPublisher.publish('PlaybookDisabled', { playbook: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
     ...
     */
    async archivePlaybook(id, actor, tx) {
        this.validateUuid(id, 'playbookId');
        const runInTx = async (transaction) => {
            const existing = await this.playbookRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            const updated = await this.playbookRepo.update(id, { status: 'ARCHIVED', updatedBy: actor }, transaction);
            await EventPublisher_1.eventPublisher.publish('PlaybookArchived', { playbook: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
     ...
     */
    async bulkCreatePlaybooks(items, actor, tx) {
        const succeeded = [];
        const failed = [];
        ...
        await EventPublisher_1.eventPublisher.publish('PlaybooksBulkCreated', { succeeded, failed });
        return { succeeded, failed };
    }
    /**
     * Bulk soft-delete playbooks by IDs.
     */
    async bulkDeletePlaybooks(ids, actor, tx) {
        const succeeded = [];
        const failed = [];
        for (const id of ids) {
            try {
        ...
        await EventPublisher_1.eventPublisher.publish('PlaybooksBulkDeleted', { succeeded, failed });
        return { succeeded, failed };
    }
}
exports.PlaybookService = PlaybookService;
exports.playbookService = new PlaybookService();
