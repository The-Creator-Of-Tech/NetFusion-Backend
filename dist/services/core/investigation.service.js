...
class InvestigationService extends BaseService_1.BaseService {
    constructor(investigationRepo = core_1.investigationRepository, timelineRepo = investigation_1.timelineRepository, activityLogRepo = core_1.activityLogRepository, notificationRepo = core_1.notificationRepository, userRepo = core_1.userRepository, projectRepo = core_1.projectRepository, assetRepo = investigation_1.assetRepository, findingRepo = investigation_1.findingRepository, evidenceRepo = investigation_1.evidenceRepository) {
        super();
    ...
    async createInvestigation(data, tx) {
        this.validateRequired(data, ['title', 'projectId', 'ownerId']);
        ...
        const runInTx = async (transaction) => {
            ...
            const createdByVal = data.createdBy || 'system';
            const updatedByVal = data.updatedBy || 'system';
            ...
            await EventPublisher_1.eventPublisher.publish('InvestigationOpened', { investigation: inv });
            return inv;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
    async updateInvestigation(id, data, tx) {
        ...
        const runInTx = async (transaction) => {
            ...
            await EventPublisher_1.eventPublisher.publish('InvestigationUpdated', { investigation: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
    async closeInvestigation(id, tx) {
        this.validateUuid(id, 'investigationId');
        const runInTx = async (transaction) => {
            const existing = await this.investigationRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            await EventPublisher_1.eventPublisher.publish('InvestigationClosed', { investigation: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
    async reopenInvestigation(id, tx) {
        this.validateUuid(id, 'investigationId');
        const runInTx = async (transaction) => {
            const existing = await this.investigationRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
            ...
            await EventPublisher_1.eventPublisher.publish('InvestigationReopened', { investigation: updated });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
    async assignOwner(id, ownerId, tx) {
        this.validateUuid(id, 'investigationId');
        ...
        const runInTx = async (transaction) => {
            ...
            await EventPublisher_1.eventPublisher.publish('InvestigationOwnerAssigned', { investigation: updated, owner: user });
            return updated;
        };
        return tx ? runInTx(tx) : prisma_1.default.$transaction(runInTx);
    }
    async changeStatus(id, status, tx) {
        return this.updateInvestigation(id, { status }, tx);
    }
    async changePriority(id, priority, tx) {
        return this.updateInvestigation(id, { priority }, tx);
    }
    async deleteInvestigation(id, tx) {
        this.validateUuid(id, 'investigationId');
        const runInTx = async (transaction) => {
            const existing = await this.investigationRepo.findById(id, transaction);
            if (!existing || existing.deletedAt) {
    ...
    async findInvestigation(id, tx) {
        this.validateUuid(id, 'investigationId');
        return this.investigationRepo.findById(id, tx);
    }
    async calculateStatistics(id, tx) {
        this.validateUuid(id, 'investigationId');
        const runInTx = async (transaction) => {
