...
async function main() {
    ...
    let testProvider = undefined;
    let testModel1 = undefined;
    let testModel2 = undefined;
    let testExecution = undefined;
    let testUsage = undefined;
    let testStreaming = undefined;
    let testChunk1 = undefined;
    let testChunk2 = undefined;
    let testConversation = undefined;
    let testMessage1 = undefined;
    let testMessage2 = undefined;
    let testMemory = undefined;
    let testMemoryEntry1 = undefined;
    let testMemoryEntry2 = undefined;
    let testContext = undefined;
    let testContextEntry1 = undefined;
    let testContextEntry2 = undefined;
    let testPrompt = undefined;
    let testSection1 = undefined;
    let testSection2 = undefined;
    let testReasoning = undefined;
    let testStep1 = undefined;
    let testStep2 = undefined;
    ...
    try {
        testProvider = await ai_1.providerRepository.create({
        ...
        testModel1 = await prisma_1.default.providerModel.create({
            data: {
        ...
        testModel2 = await prisma_1.default.providerModel.create({
            data: {
                providerId: testProvider.id,
                modelName: 'model-b',
                streaming: false,
        ...
        // findEnabled
        const enabledList = await ai_1.providerRepository.findEnabled();
        assert(enabledList.some(p => p.id === testProvider.id), 'findEnabled resolves correctly');
        // findHealthy
        const healthyList = await ai_1.providerRepository.findHealthy();
        assert(healthyList.some(p => p.id === testProvider.id), 'findHealthy resolves correctly');
        // findModels
        const modelsList = await ai_1.providerRepository.findModels(testProvider.id);
        assert(modelsList.length === 2, 'findModels resolves correct count');
        // findModelByName
        const modelA = await ai_1.providerRepository.findModelByName(testProvider.id, 'model-a');
        assert(modelA?.modelName === 'model-a', 'findModelByName resolves correctly');
        // findCapabilities
        const caps = await ai_1.providerRepository.findCapabilities(testProvider.id);
    ...
    try {
        testExecution = await ai_1.executionRepository.create({
            providerId: testProvider.id,
        ...
        testUsage = await prisma_1.default.executionUsage.create({
            data: {
        ...
        // findByProvider
        const byProv = await ai_1.executionRepository.findByProvider(testProvider.id);
        assert(byProv.some(ex => ex.id === testExecution.id), 'findByProvider resolves correctly');
        // findByStatus
        const byStatus = await ai_1.executionRepository.findByStatus('COMPLETED');
        assert(byStatus.some(ex => ex.id === testExecution.id), 'findByStatus resolves correctly');
        // findCompleted
        const completedList = await ai_1.executionRepository.findCompleted();
        assert(completedList.some(ex => ex.id === testExecution.id), 'findCompleted resolves correctly');
        // findUsage
        const usageObj = await ai_1.executionRepository.findUsage(testExecution.id);
        assert(usageObj?.id === testUsage.id, 'findUsage resolves correctly');
        // calculateCost
        const cost = await ai_1.executionRepository.calculateCost(testExecution.id);
    ...
    try {
        testStreaming = await ai_1.streamingRepository.create({
            executionId: testExecution.id,
            projectId: testProject.id,
            investigationId: testInvestigation.id,
            userId: testUser.id,
        ...
        testChunk1 = await prisma_1.default.streamingChunk.create({
            data: {
        ...
        testChunk2 = await prisma_1.default.streamingChunk.create({
            data: {
                streamingId: testStreaming.id,
                sequenceNumber: 2,
                content: 'chunk-2',
        ...
        // findByExecution
        const byEx = await ai_1.streamingRepository.findByExecution(testExecution.id);
        assert(byEx?.id === testStreaming.id, 'findByExecution resolves correctly');
        // findActive
        const activeSessions = await ai_1.streamingRepository.findActive();
        assert(activeSessions.some(s => s.id === testStreaming.id), 'findActive resolves correctly');
        // findChunks
        const chunks = await ai_1.streamingRepository.findChunks(testStreaming.id);
        assert(chunks.length === 2 && chunks[0].sequenceNumber === 1, 'findChunks returns ordered chunks');
        // calculateProgress
        const progress = await ai_1.streamingRepository.calculateProgress(testStreaming.id);
    ...
    try {
        testConversation = await ai_1.conversationRepository.create({
            projectId: testProject.id,
            investigationId: testInvestigation.id,
            userId: testUser.id,
        ...
        testMessage1 = await prisma_1.default.conversationMessage.create({
            data: {
        ...
        testMessage2 = await prisma_1.default.conversationMessage.create({
            data: {
                conversationId: testConversation.id,
                role: 'assistant',
        ...
        // findByUser
        const byUserList = await ai_1.conversationRepository.findByUser(testUser.id);
        assert(byUserList.some(c => c.id === testConversation.id), 'findByUser resolves correctly');
        // findByProject
        const byProjList = await ai_1.conversationRepository.findByProject(testProject.id);
        assert(byProjList.some(c => c.id === testConversation.id), 'findByProject resolves correctly');
        // findByInvestigation
        const byInvList = await ai_1.conversationRepository.findByInvestigation(testInvestigation.id);
        assert(byInvList.some(c => c.id === testConversation.id), 'findByInvestigation resolves correctly');
        // findActive
        const activeList = await ai_1.conversationRepository.findActive();
        assert(activeList.some(c => c.id === testConversation.id), 'findActive resolves correctly');
        // findWithMessages
        const convWithMsgs = await ai_1.conversationRepository.findWithMessages(testConversation.id);
        assert(convWithMsgs?.messages?.length === 2, 'findWithMessages resolves with nested messages');
        // searchMessages
        const results = await ai_1.conversationRepository.searchMessages('trigger');
    ...
    try {
        testMemory = await ai_1.sessionMemoryRepository.create({
            conversationId: testConversation.id,
            projectId: testProject.id,
            investigationId: testInvestigation.id,
            userId: testUser.id,
        ...
        testMemoryEntry1 = await prisma_1.default.memoryEntry.create({
            data: {
        ...
        testMemoryEntry2 = await prisma_1.default.memoryEntry.create({
            data: {
                memoryId: testMemory.id,
        ...
        // findByUser
        const byUserMem = await ai_1.sessionMemoryRepository.findByUser(testUser.id);
        assert(byUserMem.some(m => m.id === testMemory.id), 'findByUser resolves correctly');
        // findByProject
        const byProjMem = await ai_1.sessionMemoryRepository.findByProject(testProject.id);
        assert(byProjMem.some(m => m.id === testMemory.id), 'findByProject resolves correctly');
        // findActive
        const activeMem = await ai_1.sessionMemoryRepository.findActive();
        assert(activeMem.some(m => m.id === testMemory.id), 'findActive resolves correctly');
        // findEntries
        const entries = await ai_1.sessionMemoryRepository.findEntries(testMemory.id);
        assert(entries.length === 2, 'findEntries resolves correct entries');
        // searchEntries
        const searchRes = await ai_1.sessionMemoryRepository.searchEntries('ransomware');
    ...
    try {
        testContext = await ai_1.contextWindowRepository.create({
            investigationId: testInvestigation.id,
            conversationId: testConversation.id,
            projectId: testProject.id,
            userId: testUser.id,
        ...
        testContextEntry1 = await prisma_1.default.contextEntry.create({
            data: {
        ...
        testContextEntry2 = await prisma_1.default.contextEntry.create({
            data: {
                contextId: testContext.id,
                source: 'finding',
                priority: 'MEDIUM',
        ...
        // findByUser
        const byUserCtx = await ai_1.contextWindowRepository.findByUser(testUser.id);
        assert(byUserCtx.some(w => w.id === testContext.id), 'findByUser resolves correctly');
        // findByConversation
        const byConvCtx = await ai_1.contextWindowRepository.findByConversation(testConversation.id);
        assert(byConvCtx.some(w => w.id === testContext.id), 'findByConversation resolves correctly');
        // findActive
        const activeCtx = await ai_1.contextWindowRepository.findActive();
        assert(activeCtx.some(w => w.id === testContext.id), 'findActive resolves correctly');
        // findEntries
        const entriesList = await ai_1.contextWindowRepository.findEntries(testContext.id);
        assert(entriesList.length === 2, 'findEntries resolves correct context entries');
        // calculateContextSize
        const size = await ai_1.contextWindowRepository.calculateContextSize(testContext.id);
    ...
    try {
        testPrompt = await ai_1.promptAssemblyRepository.create({
            contextId: testContext.id,
            investigationId: testInvestigation.id,
            projectId: testProject.id,
            userId: testUser.id,
        ...
        testSection1 = await prisma_1.default.promptSection.create({
            data: {
        ...
        testSection2 = await prisma_1.default.promptSection.create({
            data: {
                promptId: testPrompt.id,
        ...
        // findPublished
        const published = await ai_1.promptAssemblyRepository.findPublished();
        assert(published.some(p => p.id === testPrompt.id), 'findPublished resolves correctly');
        // findByProject
        const byProj = await ai_1.promptAssemblyRepository.findByProject(testProject.id);
        assert(byProj.some(p => p.id === testPrompt.id), 'findByProject resolves correctly');
        // findSections
        const sections = await ai_1.promptAssemblyRepository.findSections(testPrompt.id);
        assert(sections.length === 2, 'findSections resolves correct count');
        // searchSections
        const search = await ai_1.promptAssemblyRepository.searchSections('parsed');
    ...
    try {
        testReasoning = await ai_1.reasoningRepository.create({
            projectId: testProject.id,
            investigationId: testInvestigation.id,
            userId: testUser.id,
        ...
        testStep1 = await prisma_1.default.reasoningStep.create({
            data: {
        ...
        testStep2 = await prisma_1.default.reasoningStep.create({
            data: {
                reasoningId: testReasoning.id,
                stepNumber: 2,
        ...
        // findByExecution
        const byExec = await ai_1.reasoningRepository.findByExecution(testExecution.id);
        assert(byExec.some(r => r.id === testReasoning.id), 'findByExecution resolves correctly via JSON path metadata');
        // findByStatus
        const byStatus = await ai_1.reasoningRepository.findByStatus('ACTIVE');
        assert(byStatus.some(r => r.id === testReasoning.id), 'findByStatus resolves correctly');
        // findSteps
        const steps = await ai_1.reasoningRepository.findSteps(testReasoning.id);
        assert(steps.length === 2 && steps[0].stepNumber === 1, 'findSteps resolves ordered steps');
        // calculateConfidence
        const avgConfidence = await ai_1.reasoningRepository.calculateConfidence(testReasoning.id);
    ...
    try {
        ...
        const dummyConv = await ai_1.conversationRepository.create({
            projectId: testProject.id,
            investigationId: testInvestigation.id,
            userId: testUser.id,
            title: `Dummy ${RUN}`,
        ...
        const softDeleted = await ai_1.conversationRepository.softDelete(dummyConv.id, 'infra-test');
        assert(softDeleted.deletedAt !== null, 'softDelete sets deletedAt timestamp');
        const restored = await ai_1.conversationRepository.restore(dummyConv.id);
        assert(restored.deletedAt === null, 'restore resets deletedAt to null');
        await ai_1.conversationRepository.delete(dummyConv.id);
        // B. Optimistic Locking on Conversation
        const convForLock = await ai_1.conversationRepository.create({
            projectId: testProject.id,
            investigationId: testInvestigation.id,
            userId: testUser.id,
            title: `Lock ${RUN}`,
        ...
        const lockedUpdate = await ai_1.conversationRepository.update(convForLock.id, {
        ...
        try {
            await ai_1.conversationRepository.update(convForLock.id, {
        ...
        await ai_1.conversationRepository.delete(convForLock.id);
        // C. Transactions & Rollback
        try {
            await ai_1.conversationRepository.transaction(async (tx) => {
                await ai_1.conversationRepository.create({
                    projectId: testProject.id,
                    investigationId: testInvestigation.id,
                    userId: testUser.id,
                    title: `Tx Fail ${RUN}`,
        ...
        const checkTx = await ai_1.conversationRepository.exists({ title: `Tx Fail ${RUN}` });
    ...
    try {
        if (testChunk1)
            await prisma_1.default.streamingChunk.delete({ where: { id: testChunk1.id } });
        if (testChunk2)
            await prisma_1.default.streamingChunk.delete({ where: { id: testChunk2.id } });
        if (testStreaming)
            await ai_1.streamingRepository.delete(testStreaming.id);
        if (testUsage)
            await prisma_1.default.executionUsage.delete({ where: { id: testUsage.id } });
        if (testExecution)
            await ai_1.executionRepository.delete(testExecution.id);
        if (testModel1)
            await prisma_1.default.providerModel.delete({ where: { id: testModel1.id } });
        if (testModel2)
            await prisma_1.default.providerModel.delete({ where: { id: testModel2.id } });
        if (testProvider)
            await ai_1.providerRepository.delete(testProvider.id);
        if (testStep1)
            await prisma_1.default.reasoningStep.delete({ where: { id: testStep1.id } });
        if (testStep2)
            await prisma_1.default.reasoningStep.delete({ where: { id: testStep2.id } });
        if (testReasoning)
            await ai_1.reasoningRepository.delete(testReasoning.id);
        if (testSection1)
            await prisma_1.default.promptSection.delete({ where: { id: testSection1.id } });
        if (testSection2)
            await prisma_1.default.promptSection.delete({ where: { id: testSection2.id } });
        if (testPrompt)
            await ai_1.promptAssemblyRepository.delete(testPrompt.id);
        if (testContextEntry1)
            await prisma_1.default.contextEntry.delete({ where: { id: testContextEntry1.id } });
        if (testContextEntry2)
            await prisma_1.default.contextEntry.delete({ where: { id: testContextEntry2.id } });
        if (testContext)
            await ai_1.contextWindowRepository.delete(testContext.id);
        if (testMemoryEntry1)
            await prisma_1.default.memoryEntry.delete({ where: { id: testMemoryEntry1.id } });
        if (testMemoryEntry2)
            await prisma_1.default.memoryEntry.delete({ where: { id: testMemoryEntry2.id } });
        if (testMemory)
            await ai_1.sessionMemoryRepository.delete(testMemory.id);
        if (testMessage1)
            await prisma_1.default.conversationMessage.delete({ where: { id: testMessage1.id } });
        if (testMessage2)
            await prisma_1.default.conversationMessage.delete({ where: { id: testMessage2.id } });
        if (testConversation)
            await ai_1.conversationRepository.delete(testConversation.id);
