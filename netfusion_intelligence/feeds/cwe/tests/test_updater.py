"""
Tests for CweUpdater — version comparison, activation, and rollback.
"""
...
@pytest.fixture
def updater(repo):
    return CweUpdater(repo)


def _make_version(feed_id: str, version_id: str, checksum: str = "abc123", status=DatasetStatus.ACTIVE):
...
class TestCweUpdaterCompareVersions:
    ...
    def test_same_checksum_no_update_needed(self, updater, repo):
        dv = _make_version("mitre_cwe_xml", "v001", checksum="abc123")
        repo.save_dataset_version(dv)
        repo.set_active_dataset_version("mitre_cwe_xml", "v001")
    ...
    def test_different_checksum_requires_update(self, updater, repo):
        dv = _make_version("mitre_cwe_xml", "v001", checksum="oldchecksum")
        repo.save_dataset_version(dv)
        repo.set_active_dataset_version("mitre_cwe_xml", "v001")
        result = updater.compare_versions("newchecksum")
        assert result["update_required"] is True
...
class TestCweUpdaterActivateDataset:

    def test_activate_sets_active_version(self, updater, repo):
        dv = _make_version("mitre_cwe_xml", "v002", status=DatasetStatus.CREATED)
        repo.save_dataset_version(dv)
    ...
    def test_activate_deactivates_previous_version(self, updater, repo):
        dv1 = _make_version("mitre_cwe_xml", "v001")
        dv2 = _make_version("mitre_cwe_xml", "v002", status=DatasetStatus.CREATED)
...
class TestCweUpdaterRollback:
    ...
    def test_rollback_to_specific_version(self, updater, repo):
        dv1 = _make_version("mitre_cwe_xml", "v001")
        dv2 = _make_version("mitre_cwe_xml", "v002")
        repo.save_dataset_version(dv1)
        repo.save_dataset_version(dv2)
        repo.set_active_dataset_version("mitre_cwe_xml", "v002")
    ...
    def test_rollback_unknown_version_raises(self, updater, repo):
        dv = _make_version("mitre_cwe_xml", "v001")
        repo.save_dataset_version(dv)
