...
export class AssetService extends BaseService {
  ...
  async createAsset(data: Prisma.AssetUncheckedCreateInput, tx?: any): Promise<Asset> {
    ...
    const runInTx = async (transaction: any) => {
      ...
      const normalizedHostname = this.normalizeHostname(data.hostname as string | undefined);
      const normalizedIp       = this.normalizeIp(data.currentIp as string | undefined);
      if (normalizedHostname || normalizedIp) {
        const filter: any = { investigationId: data.investigationId, deletedAt: null };
        if (normalizedHostname) filter.hostname  = normalizedHostname;
        if (normalizedIp)       filter.currentIp = normalizedIp;
        const existing = await this.assetRepo.findOne(filter, transaction);
        if (existing) throw new Error(`Duplicate asset: hostname/IP already exists in this investigation.`);
      }
      ...
      await this.timelineSvc.recordCreation(
        asset.projectId, asset.investigationId, 'Asset', asset.id, data.createdBy as string, transaction,
      );
      ...
      await eventPublisher.publish('AssetCreated', { asset });
      return asset;
    };
  ...
  async updateAsset(id: string, data: Prisma.AssetUncheckedUpdateInput, tx?: any): Promise<Asset> {
    this.validateUuid(id, 'assetId');
    const runInTx = async (transaction: any) => {
      const existing = await this.assetRepo.findById(id, transaction);
      if (!existing || existing.deletedAt) throw new Error(`Asset "${id}" not found.`);

      const updated = await this.assetRepo.update(id, {
        ...data,
        hostname:  data.hostname  != null ? this.normalizeHostname(data.hostname  as string) : undefined,
        currentIp: data.currentIp != null ? this.normalizeIp(data.currentIp as string)       : undefined,
      }, transaction);

      await this.timelineSvc.recordUpdate(
        updated.projectId, updated.investigationId, 'Asset', id,
        'fields updated', (data.updatedBy as string) ?? 'system', transaction,
      );
      await eventPublisher.publish('AssetUpdated', { asset: updated });
      return updated;
    };
    return tx ? runInTx(tx) : prisma.$transaction(runInTx);
  }
  ...
  async mergeAssets(sourceId: string, targetId: string, actor: string, tx?: any): Promise<Asset> {
    ...
    const runInTx = async (transaction: any) => {
      ...
      if (!source || source.deletedAt) throw new Error(`Source asset "${sourceId}" not found.`);
      if (!target || target.deletedAt) throw new Error(`Target asset "${targetId}" not found.`);

      // Re-point evidence and findings from source → target
      await prisma.evidence.updateMany({ where: { assetId: sourceId },  data: { assetId: targetId } });
      await prisma.finding.updateMany({ where: { assetId: sourceId },  data: { assetId: targetId } });

      // Soft-delete source
      await this.assetRepo.softDelete(sourceId, actor, transaction);

      // Merge metadata
      const mergedMeta = { ...(target.metadata as any ?? {}), ...(source.metadata as any ?? {}), mergedFrom: sourceId };
      ...
      await eventPublisher.publish('AssetsMerged', { source, target: merged });
      return merged;
    };
    return tx ? runInTx(tx) : prisma.$transaction(runInTx);
  }
  ...
  async enrichAsset(id: string, enrichment: Record<string, any>, actor: string, tx?: any): Promise<Asset> {
    this.validateUuid(id, 'assetId');
    const runInTx = async (transaction: any) => {
      const existing = await this.assetRepo.findById(id, transaction);
      if (!existing || existing.deletedAt) throw new Error(`Asset "${id}" not found.`);
      const merged = { ...(existing.metadata as any ?? {}), ...enrichment };
      ...
      await eventPublisher.publish('AssetEnriched', { asset: enriched });
      return enriched;
    };
    return tx ? runInTx(tx) : prisma.$transaction(runInTx);
  }
  ...
  async calculateRiskScore(id: string, tx?: any): Promise<number> {
    ...
    const runInTx = async (transaction: any) => {
      ...
      if (!asset || asset.deletedAt) throw new Error(`Asset "${id}" not found.`);

      let score = 0;
      const severityWeights: Record<string, number> = { CRITICAL: 40, HIGH: 25, MEDIUM: 15, LOW: 8, INFO: 2 };
  ...
  async associateEvidence(assetId: string, evidenceId: string, actor: string, tx?: any): Promise<void> {
    this.validateUuid(assetId,   'assetId');
    this.validateUuid(evidenceId, 'evidenceId');
    const runInTx = async (transaction: any) => {
      await prisma.evidence.update({ where: { id: evidenceId }, data: { assetId, updatedBy: actor } });
      ...
      await eventPublisher.publish('AssetEvidenceLinked', { assetId, evidenceId });
    };
    return tx ? runInTx(tx) : prisma.$transaction(runInTx);
  }

  async associateFindings(assetId: string, findingIds: string[], actor: string, tx?: any): Promise<void> {
    this.validateUuid(assetId, 'assetId');
    const runInTx = async (transaction: any) => {
      for (const fid of findingIds) {
        await prisma.finding.update({ where: { id: fid }, data: { assetId, updatedBy: actor } });
      }
      const asset = await this.assetRepo.findById(assetId, transaction);
      if (!asset) throw new Error(`Asset "${assetId}" not found.`);
      await this.timelineSvc.record({
  ...
  async summarizeAsset(id: string, tx?: any): Promise<Record<string, any>> {
    this.validateUuid(id, 'assetId');
    const runInTx = async (transaction: any) => {
      const [asset, findings, evidence] = await Promise.all([
        this.assetRepo.findById(id, transaction),
  ...
  private normalizeHostname(h: string | undefined | null): string | undefined {
    return h ? h.trim().toLowerCase() : undefined;
  }
  private normalizeIp(ip: string | undefined | null): string | undefined {
