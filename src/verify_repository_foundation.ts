 ...
 */
class TestSettingRepository extends BaseRepository<SystemSetting> {
  constructor() {
    super('systemSetting');
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const updatedCount = await repo.updateMany(
      { key: { in: [batchKey1, batchKey2, batchKey3] } },
      { value: 'batch-updated' }
    );
    ...
    // Physical cleanup
    await prisma.systemSetting.deleteMany({ where: { key: { in: [batchKey1, batchKey2, batchKey3] } } });
  } catch (e) {
  ...
  try {
    ...
    // Clean up
    await prisma.systemSetting.deleteMany({ where: { key: { in: pKeys } } });
  } catch (e) {
