...
import { cveService } from '../../services/knowledge';

// ─────────────────────────────────────────────────────────────────────────────
// Types
// ─────────────────────────────────────────────────────────────────────────────

export interface FindAffectedProductsInput {
  cveId: string;
  actor: string;
}
...
export interface FindExploitabilityInput {
  cveId: string;
  actor: string;
}

export interface FindRelatedIoCsInput {
  cveId: string;
  actor: string;
}

export interface FindCveMitigationsInput {
  cveId: string;
  actor: string;
}

export interface PrioritizeCvesInput {
  cveIds: string[];
  actor: string;
  /** If true, exploited CVEs are ranked higher regardless of CVSS */
  exploitedFirst?: boolean;
}
...
export class CveOrchestrator extends BaseApplicationService {
  constructor() {
   ...
   */
  async findAffectedProducts(
    input: FindAffectedProductsInput,
    ...
    const products = await cveService.getAffectedProducts(input.cveId);
  ...
  async calculateRisk(
    ...
    parentCtx?: OperationContext,
  ): Promise<{ cveId: string; riskScore: number; correlationId: string }> {
    const ctx = parentCtx ?? createOperationContext(input.actor);
    this.validateUuid(input.cveId, 'cveId', ctx);
    this.logInfo(ctx, `Calculating risk for CVE: ${input.cveId}`);

    const riskScore = await cveService.calculateCveRisk(input.cveId);

    await this.publishEvent(APP_EVENTS.CVE_RISK_CALCULATED, ctx, {
      cveId: input.cveId,
      riskScore,
    });
   ...
   */
  async findExploitability(
    input: FindExploitabilityInput,
    parentCtx?: OperationContext,
  ): Promise<{
    cveId: string;
    exploited: boolean;
    patched: boolean;
    cvssDetails: any | null;
    cvssScore: number;
    ...
    const cvssDetails = await cveService.getCvssDetails(input.cveId);
   ...
   */
  async findRelatedIOCs(
    input: FindRelatedIoCsInput,
    parentCtx?: OperationContext,
  ): Promise<any[]> {
    const ctx = parentCtx ?? createOperationContext(input.actor);
    this.validateUuid(input.cveId, 'cveId', ctx);
    ...
    const techniques = await cveService.findByTechnique(input.cveId).catch(() => []);
  ...
  async findMitigations(
    ...
    const affectedProducts = await cveService.getAffectedProducts(input.cveId);
  ...
  async getStatistics(actor: string, parentCtx?: OperationContext) {
    ...
    return cveService.getStatistics();
  }
  ...
  async correlateCVE(
    ...
    const updated = await cveService.correlateToTechniques(cveId, techniqueIds, actor);

    await this.publishEvent(APP_EVENTS.CVE_CORRELATED, ctx, {
      cveId,
      techniqueIds,
    });
...
export const cveOrchestrator = new CveOrchestrator();
